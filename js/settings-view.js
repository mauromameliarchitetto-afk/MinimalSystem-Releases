...
async function settingsNotificationsHtml() {
  ...
  //
  // 'unsupported' su piattaforma nativa è un caso DIVERSO da "permesso non
  // ancora concesso", e va tenuto separato (secondo test su dispositivo,
  // stesso giorno): significa che window.Capacitor.Plugins.PushNotifications
  // non esiste, cioè l'APK installato è stato costruito prima che
  // @capacitor/push-notifications entrasse nel progetto. Gli aggiornamenti
  // automatici (OTA) sostituiscono solo i file web, mai i moduli nativi:
  // nessun pulsante qui può rimediare — mostrarne uno produce solo un
  // errore ad ogni tocco. L'unica azione reale è installare un APK nuovo.
  const systemLine = !isNative
    ...
    : systemPerm === 'granted'
      ...
      : systemPerm === 'denied'
        ...
        : systemPerm === 'not_configured'
          ? '<p class="helper-text" style="margin:8px 0 0;color:var(--fisico-forte);">⚠️ Questa versione dell\'app è stata costruita senza la configurazione delle notifiche push: non possono essere ricevute finché non viene installato un APK che la includa.</p>'
        : systemPerm === 'unsupported'
          ? '<p class="helper-text" style="margin:8px 0 0;color:var(--fisico-forte);">⚠️ Questa versione installata dell\'app non contiene il modulo delle notifiche push: gli aggiornamenti automatici rinnovano solo la parte web, non i moduli nativi. Per riceverle serve installare l\'APK più recente, dal banner "Nuova versione disponibile" che l\'app mostra quando ne esiste uno.</p>'
  ...
  return `
    ...
    <p class="helper-text" style="margin:8px 0 0;">Se disattivata, la notifica mostra solo "Nuovo messaggio": né il testo, né chi scrive, né la storia — utile su un dispositivo condiviso.</p>
    ${systemLine}
...
/* -------------------------------------------------------------------- Chat
   ...
   bloccati (user_blocks — qui solo aggregato in un'unica vista, mai una
   seconda tabella: gli stessi dati già scritti dai bottoni
   "Blocca"/"Sblocca" dentro il modale chat).

   Il blocco NON è più per storia: blocca la persona ovunque, chat e
   richieste di ingresso comprese (blocco unico, vedi
   20260930050000_ugc_moderation.sql). L'elenco qui sotto perde quindi la
   colonna "in quale storia", che non esiste più. */
...
async function listMyChatBlocks() {
  ...
  return (data || []).map(r => ({ blocked_user_id: r.blocked_id }));
}
...
async function settingsChatHtml() {
  ...
  const blockedHtml = blocks.length
    ? blocks.map(b => `<div class="row-between" style="margin-bottom:6px;">
        <span class="helper-text" style="margin:0;">${escapeHtml(blockedNames[b.blocked_user_id] || 'Utente')}</span>
        <button type="button" class="btn btn-ghost btn-sm" data-settingsunblock="${b.blocked_user_id}">Sblocca</button>
      </div>`).join('')
...
function wireSettingsEvents() {
  ...
  root.addEventListener('click', async e => {
    ...
    if (unblockBtn) {
      const blockedUserId = unblockBtn.dataset.settingsunblock;
      try { await unblockChatUser(null, blockedUserId); await renderSettingsView(); }
      catch (err) { toast('Errore: ' + describeError(err)); }
      return;
    }
  });
