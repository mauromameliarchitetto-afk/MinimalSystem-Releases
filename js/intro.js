...
(function () {
  ...
  function revealFirstFrame() {
    if (firstFrameRevealed || finished || !playRequested) return;
    firstFrameRevealed = true;
    layer.classList.add('intro-frame-ready');
    if (window.RMStartup) window.RMStartup.hide();
  }
  function watchDecodedFrames() {
    if (typeof video.requestVideoFrameCallback !== 'function') return;
    frameCallbackId = video.requestVideoFrameCallback(function () {
      frameCallbackId = null;
      // Un vero frame decodificato, non una soglia su currentTime.
      revealFirstFrame();
    });
  }
  function revealLegacyFrame() {
    if (typeof video.requestVideoFrameCallback === 'function' || video.readyState < 2 || !playRequested) return;
  ...
  function onError() {
    if (isNative && window.RMStartup && window.RMStartup.isChecking()) return; // preload fallito: gestito al tentativo dopo il controllo aggiornamenti
  ...
  function attemptPlay(viaGesture) {
    ...
    playRequested = true;
    if (window.RMStartup && isNative) window.RMStartup.show('Preparazione introduzione…');
    if (frameCallbackId === null && !firstFrameRevealed) watchDecodedFrames();
    armWatchdog();
    video.muted = false;
    video.volume = 1;
    var p;
  ...
  if (isNative) {
    // Prima controllo/download/applicazione aggiornamenti; poi il video
    // della versione effettivamente in esecuzione, con audio integrale.
    if (window.RMStartup) window.RMStartup.ready.then(function () { attemptPlay(false); });
    else attemptPlay(false);
  } else {
