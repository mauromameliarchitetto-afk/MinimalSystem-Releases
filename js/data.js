...
const TRAIT_LISTS = {
  ...
  capacitaCombattive: ['Tattica Militare', 'Ascia', 'Spada', 'Lancia', 'Arco', 'Spadone',
    ...
    // e la migrazione SQL 20260912000001_rename_legacy_trait_to_resistenza) — una
...
// Evidenziazione visiva pura (Fase 5, "evidenziazione tiri"): segnala che un
// singolo dado è uscito al proprio valore massimo/minimo. Non è una regola
// di gioco (nessun successo/critico automatico inventato) — solo un
// riscontro visivo, applicabile a qualunque tiro di un singolo dado.
function rollExtremeClass(value, sides) {
  if (!Number.isFinite(value) || !Number.isFinite(sides) || sides <= 1) return null;
  if (value === sides) return 'roll-max';
  if (value === 1) return 'roll-min';
  return null;
}
