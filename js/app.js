...
function ensureDannoAttivaFields(r, isTecnica) {
  ...
  // Effetti di stato AGGIUNTIVI oltre al primo (effettoNome/effettoTratto
  // sopra): una Tecnica o un'Abilità possono davvero avere più di un
  // effetto di stato collegato, non solo quello "principale" — max 4 extra
  // (5 in totale), un tetto di buon senso più che un vincolo di gioco, vedi
  // effettoBlockHtml. bonusPct resta Abilità-only, stesso trattamento del
  // principale sopra.
  if (!Array.isArray(r.effettiExtra)) r.effettiExtra = [];
  r.effettiExtra = r.effettiExtra.slice(0, 4).map(ee => ({
    nome: typeof (ee && ee.nome) === 'string' ? ee.nome : '',
    tratto: typeof (ee && ee.tratto) === 'string' ? ee.tratto : '',
    bonusPct: (!isTecnica && typeof (ee && ee.bonusPct) === 'number' && !isNaN(ee.bonusPct)) ? ee.bonusPct : 0
  }));
...
function effettoTrattoSelectHtmlAttr(selAttr, customAttr, keyPrefix, tratto, dis) {
  const isOfficial = TRAIT_LISTS.capacitaCombattive.includes(tratto);
  const isCustom = !!tratto && !isOfficial;
  const opts = TRAIT_LISTS.capacitaCombattive.map(n =>
    `<option value="${escapeHtml(n)}" ${tratto === n ? 'selected' : ''}>${escapeHtml(n)}</option>`).join('');
  return `<select data-${selAttr}="${keyPrefix}" aria-label="Tratto di salvezza" ${dis}>
      <option value="" ${!tratto ? 'selected' : ''}>Tratto di salvezza…</option>
      ${opts}
      <option value="__custom__" ${isCustom ? 'selected' : ''}>Personalizzato…</option>
    </select>
    <input type="text" data-${customAttr}="${keyPrefix}" value="${escapeHtml(isCustom ? tratto : '')}" placeholder="Nome tratto" maxlength="40" aria-label="Nome tratto di salvezza personalizzato" class="${isCustom ? '' : 'hidden'}" ${dis}>`;
}
/* Un singolo effetto di stato (preset+nome libero, tratto di salvezza,
   bonus% ed eventuale hint) — usato sia per l'effetto principale
   (effettoNome/effettoTratto/effettoBonusPct sulla riga, mai rimovibile:
   "Nessun effetto" lo azzera) sia per ciascun effetto di r.effettiExtra
   (rimovibile). Attributi data-* parametrizzati per tenere separati i
   listener del principale (compatibilità con righe/salvataggi già
   esistenti) da quelli degli extra. */
function effettoEntryHtml(keyPrefix, nome, tratto, bonusPct, dis, opts) {
  ...
  // "sempre dovuto" — stesso campo tratto, riusato però come tratto di
  // RESISTENZA del bersaglio invece che di salvezza. Solo Bruciare e
  ...
  const statusMatch = statusEffectByName(nome);
  const effectCatalogNames = ['Rompere', 'Tramortire', ...STATUS_EFFECTS.map(s => s.label)];
  const effectIsCustom = !!nome && !effectCatalogNames.some(n => n.toLowerCase() === String(nome).trim().toLowerCase());
  const effectPresetOptions = [
    '<option value="">Nessun effetto</option>',
    ...effectCatalogNames.map(n => `<option value="${escapeHtml(n)}" ${!effectIsCustom && String(nome).toLowerCase() === n.toLowerCase() ? 'selected' : ''}>${escapeHtml(n)}</option>`),
    `<option value="__custom__" ${effectIsCustom ? 'selected' : ''}>Personalizzato…</option>`
  ].join('');
  // Accecare e' incluso: la meccanica esiste da sempre (riduzione del 70%
  // della Mira sui tiri "Per colpire", vedi il ramo attackTrait === 'Mira'
  // in combatAttackOutcomeHtml) ma mancava da questo elenco, quindi l'app
  // diceva al Narratore che era "solo un'etichetta". Chi leggeva quel
  // messaggio evitava di usarlo, oppure applicava il malus a mano SOPRA
  // quello gia' calcolato dal motore.
  // Sanguinare resta fuori ed e' corretto: non e' un effetto che il
  // Narratore sceglie: si innesca da solo dai tipi di danno
  // Perforare/Tagliare (combat_maybe_trigger_sanguinare, lato server) ed e'
  // indipendente dall'effetto scelto qui. Selezionarlo a mano non produce
  // davvero nulla.
  const wiredStatuses = new Set(['bruciare', 'avvelenare', 'elettrificare', 'stordire', 'immobilizzare', 'rallentare', 'confondere', 'corrodere', 'silenziare', 'congelare', 'accecare']);
  const statusHint = statusMatch
    ...
    : '';
  return `<div class="tec-effetto-entry">
      <div class="equip-bonus-row" style="margin-top:6px;">
        <select data-${opts.presetAttr}="${keyPrefix}" aria-label="Effetto di stato" ${dis}>${effectPresetOptions}</select>
        <input type="text" value="${escapeHtml(effectIsCustom ? nome : '')}" placeholder="Nome effetto personalizzato" maxlength="30" data-${opts.nomeAttr}="${keyPrefix}" class="${effectIsCustom ? '' : 'hidden'}" ${dis}>
        ${opts.removeAttr ? `<button type="button" class="btn btn-icon btn-sm btn-ghost" data-${opts.removeAttr}="${keyPrefix}" title="Rimuovi effetto" ${dis}>✕</button>` : ''}
      </div>
      ${nome ? `<div class="equip-bonus-row" style="margin-top:4px;">${effettoTrattoSelectHtmlAttr(opts.trattoSelAttr, opts.trattoCustomAttr, keyPrefix, tratto, dis)}</div>` : ''}
      ${(opts.showBonusPct && statusMatch) ? `<div class="equip-bonus-row" style="margin-top:4px;">
        <label style="white-space:nowrap;">Bonus % al tiro</label>
        <input type="number" min="-100" max="100" value="${Number(bonusPct) || 0}" data-${opts.bonusPctAttr}="${keyPrefix}" style="width:56px;" ${dis}>
      </div>` : ''}
      ${statusHint}
    </div>`;
}
/* Blocco "+ Effetto (Rompere, Tramortire, Bruciare...)" + tratto di
   ...
   sincronizzata a mano. Una Tecnica/Abilità può avere PIÙ di un effetto di
   stato collegato (es. Bruciare + Rallentare sullo stesso colpo): il primo
   resta sui campi storici effettoNome/effettoTratto/effettoBonusPct (mai
   rimovibile, "Nessun effetto" lo azzera), gli altri in r.effettiExtra
   (aggiungibili/rimovibili col tasto "+ Aggiungi effetto", vedi
   wireTecAbExtra). raggioHex resta un'unica proprietà della riga — decide
   il raggio dell'INTERO attacco (chi viene colpito), non di un singolo
   stato — quindi non è duplicato per ogni effetto extra. */
function effettoBlockHtml(keyPrefix, r, dis, isTecnica) {
  ...
  const extra = Array.isArray(r.effettiExtra) ? r.effettiExtra : [];
  const primaryHtml = effettoEntryHtml(keyPrefix, r.effettoNome, r.effettoTratto, r.effettoBonusPct, dis, {
    presetAttr: 'effettopreset', nomeAttr: 'effettonome', trattoSelAttr: 'effettotrattosel', trattoCustomAttr: 'effettotrattocustom',
    bonusPctAttr: 'effettobonuspct', showBonusPct: !isTecnica, removeAttr: null
  });
  const extraHtml = extra.map((ee, j) => effettoEntryHtml(`${keyPrefix}::${j}`, ee.nome, ee.tratto, ee.bonusPct, dis, {
    presetAttr: 'effettoextrapreset', nomeAttr: 'effettoextranome', trattoSelAttr: 'effettoextratrattosel', trattoCustomAttr: 'effettoextratrattocustom',
    bonusPctAttr: 'effettoextrabonuspct', showBonusPct: !isTecnica, removeAttr: 'deleffettoextra'
  })).join('');
  const anyStatusForRaggio = !isTecnica && (statusEffectByName(r.effettoNome) || extra.some(ee => statusEffectByName(ee.nome)));
  const raggioHtml = anyStatusForRaggio ? `<div class="equip-bonus-row" style="margin-top:4px;">
        <label style="white-space:nowrap;">Raggio (celle)</label>
  ...
  return `<details class="tec-effetto-details" ${(r.effettoNome || extra.length) ? 'open' : ''}>
      <summary class="tec-effetto-summary">+ Effetto (Rompere, Tramortire, Bruciare...)</summary>
      ${primaryHtml}
      ${extraHtml}
      ${raggioHtml}
      <button type="button" class="btn btn-ghost btn-sm" data-addeffettoextra="${keyPrefix}" style="margin-top:6px;" ${dis}>+ Aggiungi effetto</button>
    </details>`;
}
...
/* Fascia firma/release (#app-signature): a differenza di [data-toast-clear]
   (per-vista, misurato da updateToastReservedSpace ad ogni toast()) è
   SEMPRE presente, fuori da ogni .view — una sua propria variabile CSS
   indipendente, misurata una sola volta all'avvio e ad ogni resize (mai
   dentro updateToastReservedSpace: mescolarla in --toast-reserved
   romperebbe il contratto "0 quando la vista corrente non ne ha uno" già
   verificato da tests/e2e/toast-safe-area.spec.js). #toast la somma alla
   propria formula in css/style.css. */
function updateSignatureReservedSpace() {
  const sig = $('#app-signature');
  if (!sig) return;
  // Esclude il padding-bottom reale (già = --safe-bottom, vedi .app-signature
  // in css/style.css): la formula del toast aggiunge --safe-bottom per
  // conto proprio, sommare qui anche l'altezza intera lo conterebbe due
  // volte (stesso hotfix già applicato in updateToastReservedSpace).
  const rect = sig.getBoundingClientRect();
  const paddingBottom = parseFloat(getComputedStyle(sig).paddingBottom) || 0;
  const contentBottom = rect.bottom - paddingBottom;
  const visibleTop = Math.max(rect.top, 0);
  const visibleContentBottom = Math.min(contentBottom, window.innerHeight);
  const visible = Math.max(0, visibleContentBottom - visibleTop);
  document.documentElement.style.setProperty('--sig-bar-reserved', visible + 'px');
}
window.addEventListener('resize', updateSignatureReservedSpace);
if (window.visualViewport) window.visualViewport.addEventListener('resize', updateSignatureReservedSpace);

/* Terza fascia che il toast deve scavalcare, accanto a --toast-reserved e
   --sig-bar-reserved: la navigazione persistente (#rm-nav). Serve da
   quando la barra compare anche in copertina, l'unica vista con un
   [data-toast-clear] sul fondo (il payoff): senza, il toast lo copriva.
   Si riserva spazio SOLO quando la barra è davvero una fascia inferiore:
   da 900px in su diventa un rail laterale a tutta altezza, che in basso
   non toglie nulla. Il discrimine è geometrico e non ripete il breakpoint
   in JS — la fascia inferiore occupa tutta la larghezza, il rail no. */
function updateNavReservedSpace() {
  const nav = $('#rm-nav');
  if (!nav) return;
  const rect = nav.getBoundingClientRect();
  const isBottomBar = rect.height > 0 && rect.width >= window.innerWidth - 1;
  document.documentElement.style.setProperty('--nav-bar-reserved', (isBottomBar ? rect.height : 0) + 'px');
}
window.addEventListener('resize', updateNavReservedSpace);
if (window.visualViewport) window.visualViewport.addEventListener('resize', updateNavReservedSpace);


/* Componente condiviso firma/release (requisito trasversale del piano
   D&D-Beyond-like, PROMPT_CLAUDE.md): fonte unica per entrambi i valori,
   mai scritti a mano altrove. window.BRAND.productName (js/brand.js) è la
   firma approvata (nessun nome di persona: scelta esplicita dell'utente).
   APP_VERSION (js/version.js) è la STESSA variabile che checkForUpdate usa
   per il controllo aggiornamenti nativo: resta null su un checkout
   sorgente non passato da build:www (mai buildato), quindi qui il bottone
   versione resta nascosto invece di mostrare un numero inventato — la
   firma da sola resta comunque sempre visibile. */
function renderAppSignature() {
  const textEl = $('#app-signature-text');
  const verEl = $('#app-signature-version');
  if (!textEl || !verEl) return;
  textEl.textContent = (window.BRAND && window.BRAND.productName) || '';
...
/* Ricerca globale (Fase 4, piano D&D-Beyond-like): personaggi visibili
   nella sessione corrente — stesso elenco di "I tuoi personaggi"
   (visibleCharacters()), che per costruzione non contiene mai PNG (il
   sync cloud li esclude già a monte, vedi js/cloud-character.js:
   .eq('is_npc', false) — un PNG non è mai "un tuo personaggio" locale).
   searchRules() vive in js/rules.js (solo dati statici del regolamento,
   nessuna dipendenza da app.js). Le campagne (solo per un account
   Narratore) sono cercate a parte in performGlobalSearch, perché
   richiedono listMyCampaigns() (js/cloud-account.js, caricato dopo). */
function searchCharacters(query) {
  const q = (query || '').trim().toLocaleLowerCase('it');
  if (!q) return [];
  return visibleCharacters()
    .filter(c => (c.nome || '').toLocaleLowerCase('it').includes(q))
    .map(c => ({ type: 'character', id: c.id, title: c.nome || 'Senza nome' }));
}

/* Contenuti personalizzati (S12, piano UX "confronto D&D Beyond"): stessa
   fonte dati di renderCustomContentsView (listMyCustomContents,
   js/cloud-account.js), qui solo filtrata per titolo/corpo. L'oggetto
   completo resta agganciato al risultato (non solo id/titolo) perché
   aprirlo significa richiamare openCustomContentForm con l'item intero —
   rifetcharlo da capo al click sarebbe un giro a vuoto evitabile. */
function searchCustomContents(query, items) {
  const q = (query || '').trim().toLocaleLowerCase('it');
  if (!q) return [];
  return (items || [])
    .filter(c => (c.title || '').toLocaleLowerCase('it').includes(q) || (c.body || '').toLocaleLowerCase('it').includes(q))
    .map(c => ({ type: 'content', id: c.id, title: c.title || 'Senza titolo', item: c }));
}

const GLOBAL_SEARCH_CATEGORY_LABELS = { rule: 'Regole', character: 'Personaggi', campaign: 'Storie', content: 'Contenuti', setting: 'Ambientazioni' };

function globalSearchResultRowHtml(r) {
  if (r.type === 'rule') {
    return `<button type="button" class="row-between" data-searchresult="rule" data-chapterindex="${r.chapterIndex}" style="cursor:pointer;background:transparent;border:none;width:100%;padding:10px 0;text-align:left;border-bottom:1px solid var(--bordo-scuro);">
      <span style="display:flex;flex-direction:column;gap:2px;min-width:0;">
        <span>${escapeHtml(r.sectionTitle)} <span class="helper-text" style="margin:0;">— ${escapeHtml(r.chapterTitle)}</span></span>
        ${r.snippet ? `<span class="helper-text" style="margin:0;">${escapeHtml(r.snippet)}</span>` : ''}
      </span>
      <span aria-hidden="true">›</span>
    </button>`;
  }
  if (r.type === 'character') {
    return `<button type="button" class="row-between" data-searchresult="character" data-charid="${r.id}" style="cursor:pointer;background:transparent;border:none;width:100%;padding:10px 0;text-align:left;border-bottom:1px solid var(--bordo-scuro);">
      <span>${escapeHtml(r.title)} <span class="helper-text" style="margin:0;">— personaggio</span></span>
      <span aria-hidden="true">›</span>
    </button>`;
  }
  if (r.type === 'campaign') {
    return `<button type="button" class="row-between" data-searchresult="campaign" data-campaignid="${r.id}" style="cursor:pointer;background:transparent;border:none;width:100%;padding:10px 0;text-align:left;border-bottom:1px solid var(--bordo-scuro);">
      <span>${escapeHtml(r.title)} <span class="helper-text" style="margin:0;">— storia</span></span>
      <span aria-hidden="true">›</span>
    </button>`;
  }
  if (r.type === 'content') {
    return `<button type="button" class="row-between" data-searchresult="content" data-contentid="${r.id}" style="cursor:pointer;background:transparent;border:none;width:100%;padding:10px 0;text-align:left;border-bottom:1px solid var(--bordo-scuro);">
      <span>${escapeHtml(r.title)} <span class="helper-text" style="margin:0;">— contenuto personalizzato</span></span>
      <span aria-hidden="true">›</span>
    </button>`;
  }
  if (r.type === 'setting') {
    return `<button type="button" class="row-between" data-searchresult="setting" data-settingkey="${escapeHtml(r.settingKey)}" style="cursor:pointer;background:transparent;border:none;width:100%;padding:10px 0;text-align:left;border-bottom:1px solid var(--bordo-scuro);">
      <span>${escapeHtml(r.title)} <span class="helper-text" style="margin:0;">— ambientazione</span></span>
      <span aria-hidden="true">›</span>
    </button>`;
  }
  return '';
...
/* "Cronologia recente" (S12): solo per-dispositivo, nessuna sincronizzazione
   — stesso pattern di localStorage già usato per altre preferenze non
   legate al personaggio (es. ms_sheet_playmode_v1, S5). */
...
function pushRecentSearch(q) {
  ...
  if (!trimmed) return;
  try {
    let list = getRecentSearches().filter(x => x.toLocaleLowerCase('it') !== trimmed.toLocaleLowerCase('it'));
...
function renderRecentSearchesChips() {
  const el = $('#search-recent');
  if (!el) return;
  const list = getRecentSearches();
  if (!list.length) { el.classList.add('hidden'); el.innerHTML = ''; return; }
  el.classList.remove('hidden');
  el.innerHTML = `
    <div class="helper-text" style="margin:0 0 6px;">Ricerche recenti</div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
      ${list.map(q => `<button type="button" class="chip" data-recentsearch="${escapeHtml(q)}" style="cursor:pointer;">${escapeHtml(q)}</button>`).join('')}
      <button type="button" class="chip" data-clearrecentsearches="1" style="cursor:pointer;color:var(--fisico-forte);">Cancella</button>
    </div>
  `;
}

/* Risultati e categoria attiva tenuti in memoria (S12): un filtro per
   categoria è puro client-side su una ricerca già fatta, mai una nuova
   query — stesso principio già usato per il filtro di S9 sui riassunti. */
let lastGlobalSearchResults = [];
let globalSearchActiveCategory = 'all';
let globalSearchDebounceTimer = null;

function renderGlobalSearchCategories() {
  const el = $('#search-global-categories');
  if (!el) return;
  const counts = {};
  lastGlobalSearchResults.forEach(r => { counts[r.type] = (counts[r.type] || 0) + 1; });
  const cats = Object.keys(GLOBAL_SEARCH_CATEGORY_LABELS).filter(t => counts[t]);
  if (cats.length <= 1) { el.classList.add('hidden'); el.innerHTML = ''; return; }
  el.classList.remove('hidden');
  el.innerHTML = [
    `<button type="button" class="chip${globalSearchActiveCategory === 'all' ? ' active' : ''}" data-searchcategory="all">Tutti (${lastGlobalSearchResults.length})</button>`,
    ...cats.map(t => `<button type="button" class="chip${globalSearchActiveCategory === t ? ' active' : ''}" data-searchcategory="${t}">${GLOBAL_SEARCH_CATEGORY_LABELS[t]} (${counts[t]})</button>`)
  ].join('');
}
function renderGlobalSearchResultsList(q) {
  const resultsEl = $('#search-global-results');
  if (!resultsEl) return;
  const filtered = globalSearchActiveCategory === 'all'
    ? lastGlobalSearchResults
    : lastGlobalSearchResults.filter(r => r.type === globalSearchActiveCategory);
  resultsEl.innerHTML = filtered.length
    ? filtered.map(globalSearchResultRowHtml).join('')
    : `<p class="helper-text" style="margin:0;">Nessun risultato per "${escapeHtml(q)}".</p>`;
}

async function performGlobalSearch(query) {
  const resultsEl = $('#search-global-results');
  const categoriesEl = $('#search-global-categories');
  if (!resultsEl) return;
  const q = (query || '').trim();
  if (!q) {
    lastGlobalSearchResults = [];
    resultsEl.innerHTML = '';
    if (categoriesEl) { categoriesEl.classList.add('hidden'); categoriesEl.innerHTML = ''; }
    renderRecentSearchesChips();
    return;
  }
  const recentEl = $('#search-recent');
  if (recentEl) { recentEl.classList.add('hidden'); recentEl.innerHTML = ''; }
  const ruleResults = typeof searchRules === 'function' ? searchRules(q) : [];
  const charResults = searchCharacters(q);
  resultsEl.innerHTML = `<p class="helper-text" style="margin:0;">Ricerca in corso…</p>`;
  let campaignResults = [];
  if (typeof listMyCampaigns === 'function') {
    try {
      const mine = await listMyCampaigns();
      const qLower = q.toLocaleLowerCase('it');
      campaignResults = (mine || [])
        .filter(c => (c.name || '').toLocaleLowerCase('it').includes(qLower))
        .map(c => ({ type: 'campaign', id: c.id, title: c.name }));
    } catch (e) { /* nessuna storia da mostrare se la rete fallisce: non blocca gli altri risultati */ }
  }
  let contentResults = [];
  if (typeof listMyCustomContents === 'function') {
    try {
      const mineContents = await listMyCustomContents();
      contentResults = searchCustomContents(q, mineContents);
    } catch (e) { /* nessun contenuto da mostrare se la rete fallisce: non blocca gli altri risultati */ }
  }
  const settingResults = typeof searchLibrarySettings === 'function' ? searchLibrarySettings(q) : [];
  lastGlobalSearchResults = [...ruleResults, ...charResults, ...campaignResults, ...contentResults, ...settingResults];
  globalSearchActiveCategory = 'all';
  renderGlobalSearchCategories();
  renderGlobalSearchResultsList(q);
  pushRecentSearch(q);
}

/* Changelog: la release mostrata è sempre la stessa APP_VERSION della
   fascia firma/release, mai un valore duplicato a mano. Lo storico reale
   delle note di rilascio (dalle GitHub Release già pubblicate da
   .github/workflows/build-apk.yml) non è ancora collegato in questa fase
   — vedi il commento nella vista in index.html: nessuna nota inventata. */
/* ---- Changelog: storico reale delle release ----------------------------
   Fino al 16/09/2026 questa vista mostrava solo la versione in esecuzione e
   una riga che ammetteva di non avere lo storico. Le GitHub Release esistono
   da sempre: mancava solo la lettura.

   Riusa esattamente la strada gia' battuta da checkForUpdate(): stesso
   endpoint, stesso host (api.github.com e' gia' in connect-src nella CSP),
   stesso withTimeout, stesso repo risolto da resolveReleasesRepo().

   DUE VINCOLI CHE NON VANNO RIMOSSI

   1) Nessun collegamento di download, mai. Le policy degli store vietano
      canali di aggiornamento alternativi, ed e' il motivo per cui
      checkForUpdate() si spegne del tutto su canale store. Mostrare le note
      di rilascio non e' un canale di aggiornamento: offrire da qui l'APK o
      il bundle lo diventerebbe. Gli asset delle release non vengono quindi
      nemmeno letti. Per la stessa ragione questa vista NON e' protetta da
      isStoreChannel(): leggere uno storico e' lecito ovunque.
   2) Il corpo delle release e' testo esterno. Viene mostrato come testo
      semplice con escapeHtml, mai interpretato come HTML o Markdown. */
const CHANGELOG_CACHE_KEY = 'ms_changelog_cache_v1';
const CHANGELOG_CACHE_TTL = 5 * 60 * 1000;
const CHANGELOG_MAX_RELEASES = 20;
const CHANGELOG_BODY_PREVIEW_CHARS = 280;
...
/* Testo esterno reso come testo: escapeHtml prima, poi i soli a capo
   diventano <br>. Nessun Markdown interpretato — un corpo di release puo'
   contenere qualunque cosa, e questa vista non e' il posto per scoprirlo. */
function changelogBodyHtml(body) {
  const text = String(body || '').trim();
  if (!text) return '<p class="helper-text" style="margin:6px 0 0;">Nessuna nota per questa versione.</p>';
  const toHtml = t => escapeHtml(t).replace(/\n/g, '<br>');
  if (text.length <= CHANGELOG_BODY_PREVIEW_CHARS) {
    return `<p class="helper-text" style="margin:6px 0 0;">${toHtml(text)}</p>`;
  }
  const cut = text.slice(0, CHANGELOG_BODY_PREVIEW_CHARS).replace(/\s+\S*$/, '');
  return `<p class="helper-text" style="margin:6px 0 0;">${toHtml(cut)}…</p>
    <details style="margin-top:6px;">
      <summary class="helper-text" style="cursor:pointer;">Note complete</summary>
      <p class="helper-text" style="margin:6px 0 0;">${toHtml(text)}</p>
    </details>`;
}

function changelogEntryHtml(rel, runningVersion) {
  const tag = String(rel.tag_name || '').trim();
  const version = tag.replace(/^v/i, '');
  const isRunning = !!runningVersion && version === String(runningVersion);
  const title = String(rel.name || tag || 'Versione senza nome').trim();
  const date = changelogDateLabel(rel.published_at || rel.created_at);
  return `<div class="box" style="margin-bottom:10px;"><div class="box-bar"></div><div class="box-pad">
    <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;">
      <strong>${escapeHtml(title)}</strong>
      ${isRunning ? '<span class="helper-text" style="color:var(--magico);">· in esecuzione</span>' : ''}
    </div>
    ${date ? `<p class="helper-text" style="margin:2px 0 0;">${escapeHtml(date)}</p>` : ''}
    ${changelogBodyHtml(rel.body)}
  </div></div>`;
}
...
function readChangelogCache() {
  try {
    ...
    if (Date.now() - cached.at > CHANGELOG_CACHE_TTL) return null;
    return cached.releases;
  } catch (e) { return null; }
}

/* L'API pubblica di GitHub concede circa 60 richieste all'ora per indirizzo
   IP senza autenticazione: senza cache, aprire e chiudere questa vista
   qualche volta basterebbe a esaurirle e a far fallire ANCHE il controllo
   aggiornamenti, che usa lo stesso contingente. */
async function fetchChangelogReleases() {
  const cached = readChangelogCache();
  if (cached) return cached;
  const repo = await resolveReleasesRepo();
  const res = await withTimeout(
    fetch(`https://api.github.com/repos/${repo}/releases?per_page=${CHANGELOG_MAX_RELEASES}`),
    'Storico versioni'
  );
  if (!res.ok) throw new Error(`GitHub ha risposto ${res.status}`);
  const all = await res.json();
  if (!Array.isArray(all)) throw new Error('Risposta inattesa da GitHub');
  // Bozze e prerelease restano fuori: le prerelease sono build di collaudo
  // (vedi l'input "prerelease" di build-apk.yml) e l'updater le ignora
  // sempre. Uno storico che le elencasse mostrerebbe versioni che nessuno
  // ha mai ricevuto.
  const releases = all
  ...
  try {
    localStorage.setItem(CHANGELOG_CACHE_KEY, JSON.stringify({ at: Date.now(), releases }));
  } catch (e) { /* quota piena o storage negato: la cache e' un lusso, non un requisito */ }
  return releases;
}

async function renderChangelogView() {
  const el = $('#changelog-current-version');
  if (el) {
    el.textContent = (typeof APP_VERSION !== 'undefined' && APP_VERSION)
      ? `Versione in esecuzione: v${APP_VERSION}`
      : 'Versione in esecuzione: build di sviluppo (non pubblicata).';
  }
  const list = $('#changelog-list');
  if (!list) return;
  list.innerHTML = '<p class="helper-text" style="margin:0;">Carico lo storico…</p>';
  try {
    const releases = await fetchChangelogReleases();
    if (!releases.length) {
      list.innerHTML = '<p class="helper-text" style="margin:0;">Nessuna versione pubblicata finora.</p>';
      return;
    }
    const running = (typeof APP_VERSION !== 'undefined' && APP_VERSION) ? APP_VERSION : null;
    list.innerHTML = releases.map(r => changelogEntryHtml(r, running)).join('');
  } catch (e) {
    // Mai inventare uno storico quando la rete non risponde: si dice che non
    // si e' potuto leggerlo, e la versione in esecuzione resta comunque
    // visibile sopra.
    list.innerHTML = `<p class="helper-text" style="margin:0;">Non riesco a leggere lo storico delle versioni${(typeof describeError === 'function' && e) ? ` (${escapeHtml(describeError(e))})` : ''}. Riprova più tardi.</p>`;
  }
}
...
/* Viste dove la navigazione persistente (#rm-nav, piano UX "confronto D&D
   Beyond") NON compare: scheda e combattimento (schermate a piena
   immersione, dove sottrarre spazio verticale fisso costerebbe più di
   quanto una barra sempre uguale offrirebbe) e il wizard di creazione
   (ha già i propri passi/pulsanti). In copertina invece c'è: è la
   destinazione della voce "Home", e da lì si arriva a tutto il resto
   senza dover riaprire l'hamburger (richiesta esplicita dell'utente). */
const RM_NAV_HIDDEN_VIEWS = new Set(['sheet', 'combat', 'create', 'charview', 'onboarding']);
function updateRmNav(name) {
  const nav = $('#rm-nav');
  if (!nav) return;
  const show = !RM_NAV_HIDDEN_VIEWS.has(name);
  nav.classList.toggle('hidden', !show);
  document.body.classList.toggle('rm-nav-visible', show);
  // "rules" (il lettore del manuale) si raggiunge da dentro il Compendio:
  // entrambe le viste illuminano quindi la stessa voce di navigazione, che
  // ora punta a 'library'. Senza questa riga, aprendo il manuale la barra
  // perderebbe l'evidenziazione e si avrebbe la sensazione di essere
  // usciti dalla sezione.
  const activeMap = { cover: 'cover', home: 'home', list: 'charlist', campaigns: 'storie', account: 'storie', library: 'library', rules: 'library', search: 'search' };
  const activeTarget = activeMap[name];
  $$('.rm-nav-item').forEach(btn => btn.classList.toggle('active', !!activeTarget && btn.dataset.rmNav === activeTarget));
  updateNavReservedSpace();
}
...
function showViewDom(name) {
  ...
  if (name === 'combat') combatWakeLockOnCombatViewEnter(); else combatWakeLockRelease();
  // Presenza e canali realtime di Conversazioni (checkpoint C3, stesso
  // principio del tabellone di combattimento sopra): restano aperti solo
  // mentre view-conversations è davvero in primo piano, si chiudono su
  // ogni altra vista — evita canali/presence orfani quando si esce.
  // "campaignsheet" è esente dallo stop della Presence (non dai canali di
  // Conversazioni/chat privata, che restano estranei a quella vista): la
  // mappa di sessione (M2/M3, story-map-session.js) vive dentro la scheda
  // storia e riusa la STESSA Presence — fermarla mentre si entra nella
  // scheda storia la romperebbe ancora prima che la mappa possa usarla.
  if (name !== 'conversations') {
    if (typeof unsubscribeConversationsRealtime === 'function') unsubscribeConversationsRealtime();
    if (typeof unsubscribeDirectChatRealtime === 'function') unsubscribeDirectChatRealtime();
    if (name !== 'campaignsheet' && typeof stopPresence === 'function') stopPresence();
  }
  if (name !== 'campaignsheet') {
    if (typeof unsubscribeMapSessionRealtime === 'function') unsubscribeMapSessionRealtime();
  }
  ...
  // Modalità gioco (S5, piano UX "confronto D&D Beyond"): preferenza
  // persistente per-dispositivo (mai legata al personaggio), si somma
  // (mai sostituisce) alle restrizioni automatiche già esistenti sopra —
  // MAI applicata durante l'editing Narratore di un personaggio altrui/
  // di un PNG (narratorEditMode/narratorEditIsNpc): lì Identità/
  // Statistiche possono davvero servire, indipendentemente da una
  // preferenza personale impostata sui PROPRI personaggi.
  if (name === 'sheet') {
    const hiddenTabs = new Set();
    if (combatSheetRestricted || narratorEditIsNpc) {
      hiddenTabs.add('note');
      if (combatSheetRestricted) hiddenTabs.add('livelli');
    }
    if (sheetPlayModeOnly && !narratorEditIsNpc && !narratorEditMode) {
      SHEET_PLAYMODE_HIDDEN_TABS.forEach(t => hiddenTabs.add(t));
    }
    $$('#tabs .tab-btn').forEach(b => b.classList.toggle('hidden', hiddenTabs.has(b.dataset.tab)));
    updateSheetPlayModeButton();
  } else {
...
function refreshViewOnReturn(name) {
  ...
  else if (name === 'tome') { if (typeof renderTome === 'function') renderTome(); }
  ...
  else if (name === 'home') { if (typeof renderHomeView === 'function') renderHomeView(); }
}

/* Tabella unica di instradamento per le cinque destinazioni principali
   (piano UX "confronto D&D Beyond", S1/navigazione): condivisa dal menu
   hamburger esistente (.cm-item) e dalla nuova barra di navigazione
   persistente (.rm-nav-item) — stessa destinazione, stesso comportamento,
   un solo punto da mantenere. */
function goToMenuTarget(target) {
  if (target === 'cover') { showView('cover'); return; }
  if (target === 'home') { if (typeof renderHomeView === 'function') renderHomeView(); showView('home'); return; }
  if (target === 'storie') { goToStorieDestination(); return; }
  if (target === 'rules') { renderRules(); showView('rules'); return; }
  if (target === 'master') { renderMasterArea(); showView('master'); return; }
  if (target === 'premises') { renderPremisesArea(); showView('premises'); return; }
  if (target === 'account') { renderAccountArea(); showView('account'); return; }
  // Il Recap non è più una destinazione globale: vive nel tab "Recap"
  // della singola storia. 'previously' resta come alias (Home "Leggi il
  // recap") verso la storia del personaggio attivo.
  if (target === 'previously') { if (typeof openActiveCharacterStoryRecap === 'function') openActiveCharacterStoryRecap(); return; }
  if (target === 'notifications') { if (typeof renderNotificationsView === 'function') renderNotificationsView(); showView('notifications'); return; }
  if (target === 'campaigns') { if (typeof renderMyCampaignsBox === 'function') renderMyCampaignsBox(); showView('campaigns'); return; }
  if (target === 'charlist') {
    // character_tome_v1 (checkpoint T1): a flag spento (produzione)
    // rmFeatureEnabled non esiste ancora attivo o restituisce false, quindi
    // il comportamento resta identico a prima. Il Tomo richiama comunque
    // sempre openCharacter(id) per aprire la scheda, mai una copia.
    if (typeof rmFeatureEnabled === 'function' && rmFeatureEnabled('character_tome_v1') && typeof renderTome === 'function') {
      // showView PRIMA di renderTome (bug T1.1): #view-tome deve essere già
      // visibile quando renderTome calcola lo scroll iniziale verso
      // activeId, altrimenti track.clientWidth è ancora 0 (vista nascosta)
      // e il carosello si apre sempre sulla prima carta invece che
      // sull'ultimo personaggio aperto.
      showView('tome'); renderTome(); return;
    }
    renderCharList(); showView('list'); return;
  }
  if (target === 'newchar') { createCharacterFlow(); return; }
  if (target === 'library') { if (typeof renderLibraryView === 'function') renderLibraryView(); showView('library'); return; }
  if (target === 'search') { showView('search'); return; }
  if (target === 'customcontents') { if (typeof renderCustomContentsView === 'function') renderCustomContentsView(); showView('customcontents'); return; }
  if (target === 'plans') { if (typeof renderPlansView === 'function') renderPlansView(); showView('plans'); return; }
  if (target === 'settings') { if (typeof renderSettingsView === 'function') renderSettingsView(); showView('settings'); return; }
  if (target === 'repertoire') { if (typeof renderRepertoireView === 'function') renderRepertoireView(); showView('repertoire'); return; }
  if (target === 'support') { if (typeof renderSupportView === 'function') renderSupportView(); showView('support'); return; }
  if (target === 'chatpreview') { if (typeof renderChatPreview === 'function') renderChatPreview(); showView('chatpreview'); return; }
  if (target === 'conversations') {
    // Fail-closed (audit Play Store, punto 6): anche un collegamento diretto
    // (notifica, deep link, menu rimasto in cache) non apre Conversazioni se
    // il database non ha le dipendenze della chat privata.
    const ready = typeof rmFeatureBackendReady === 'function' ? rmFeatureBackendReady('chat_revision_v1') : Promise.resolve(false);
    ready.then(ok => {
      if (!ok) { if (typeof toast === 'function') toast('Conversazioni non disponibili al momento.'); return; }
      if (typeof renderConversationsView === 'function') renderConversationsView();
...
/* "Storie" nella navigazione persistente: destinazione unica per due
   pubblici diversi, mai un bottone che sparisce per metà utenza (a
   differenza di "Le tue storie" nell'hamburger, riservata a Narratore/
   admin — vedi renderMyCampaignsBox/#cm-item-campaigns). Un Narratore/
   admin possiede storie (#view-campaigns, dato reale già esistente); un
   Giocatore no — le SUE storie sono "a quali storie altrui i miei
   personaggi partecipano", dato già renderizzato da renderPlayerStoriesBox
   dentro Account, tab Giocatore (#account-giocatore-stories). Nessun nuovo
   dato: solo instradamento verso quello che già esiste, secondo il ruolo
   reale dell'account. */
async function goToStorieDestination() {
  let session = null;
  try { session = await currentCloudSession(); } catch (e) {}
  ...
  renderAccountArea();
  showView('account');
  const giocatoreTab = $('[data-accmode="giocatore"]');
  if (giocatoreTab) giocatoreTab.click();
  const storiesBox = $('#account-giocatore-stories');
  if (storiesBox) storiesBox.scrollIntoView({ block: 'center' });
}
...
// S20 (profilo/fiducia): aggiorna anche l'indicatore persistente in
// Account (#acc-connection-state, se presente in quel momento — la vista
// può non essere aperta) invece di una seconda coppia di listener a parte.
function updateConnectionStateIndicator() {
  const el = document.getElementById('acc-connection-state');
  if (!el) return;
  el.textContent = navigator.onLine ? '🟢 Il dispositivo è online.' : '🔴 Il dispositivo è offline.';
}
...
window.addEventListener('online', () => {
  ...
  // Riconnessione del tabellone di combattimento (FC4): prima solo la
  // sincronizzazione dei personaggi veniva riavviata qui, mai il canale
  // Realtime del combattimento — uno spettatore puro (nessun personaggio
  // proprio in quell'incontro) restava con un canale caduto in silenzio
  // finché non lasciava e rientrava nella vista. startCombatRealtimeWatch
  // è già idempotente (richiama stopCombatRealtimeWatch al suo interno),
  // sicuro da richiamare anche se il canale non era mai davvero caduto.
  if (typeof combatViewEncounterId !== 'undefined' && combatViewEncounterId
    && typeof startCombatRealtimeWatch === 'function' && typeof onCombatRealtimeChange === 'function') {
    startCombatRealtimeWatch(combatViewEncounterId, onCombatRealtimeChange);
    if (typeof refreshCombatBoard === 'function') refreshCombatBoard();
  }
  updateConnectionStateIndicator();
});
...
// Piano UX "confronto D&D Beyond" (S4, builder approfondito): "perché
// serve questo passo", una riga per scheda narrativa — nessun campo qui è
// obbligatorio (vedi .wiz-narrative-hint già esistente), quindi il testo
// spiega il VANTAGGIO di compilarlo ora, mai una finta obbligatorietà.
const WIZ_NARRATIVE_WHY = [
  'Un volto e un nome aiutano te e il tuo Narratore a immaginare subito il personaggio.',
  'Descrivere l\'aspetto lo rende riconoscibile al tavolo, oltre i numeri della scheda.',
  'Il vissuto del personaggio dà al Narratore spunti reali per costruire la storia.',
  'Sapere come si comporta aiuta a interpretarlo in modo coerente durante il gioco.',
  'Il passato spiega perché il personaggio è arrivato a essere così com\'è oggi.',
  'I legami con altri personaggi danno al Narratore agganci narrativi da usare.'
];
...
const WIZARD_STEPS = [
  ...
  { key: 'build', label: 'Classe', validate: () => true,
    why: 'La classe determina come cresceranno HP e MP, e quante Tecniche/Abilità avrai fin da subito.' },
  { key: 'primary', label: 'Statistiche primarie', validate: c => primaryRemaining(c) === 0,
    why: 'Le statistiche primarie determinano quanto il personaggio è efficace nelle azioni di gioco.' },
  { key: 'traits', label: 'Tratti', validate: c => allTraitsAtZero(c),
    why: 'I tratti definiscono cosa il personaggio sa fare bene: combattere, conoscere, relazionarsi.' },
  { key: 'tecab', label: 'Tecniche e Abilità', validate: c => wizardTecAbValid(c),
    why: 'Tecniche e Abilità sono le mosse speciali che potrai usare durante il gioco.' },
  { key: 'summary', label: 'Riepilogo', validate: c => wizardAllValid(c),
    why: 'Controlla tutto prima di confermare: dopo, classe e statistiche non saranno più modificabili da qui.' }
];
...
/* isResuming (piano UX "confronto D&D Beyond", S4 "autosalvataggio e
   ripresa espliciti"): true solo quando si riapre una bozza già esistente
   (lista personaggi, js/app.js più sotto — !cc.creationCompleted), mai
   quando createCharacterFlow() la chiama per un personaggio appena creato.
   Il salvataggio in sé non cambia in nessuno dei due casi (saveAll() gira
   comunque qui sotto, come sempre) — questo flag serve solo a mostrare un
   messaggio onesto ("hai ripreso una bozza") invece di uno silenzioso. */
function openCreationWizard(id, isResuming) {
  activeId = id;
  saveAll();
  ...
  if (isResuming) {
    const c = getActive();
    toast(c && c.nome ? `Hai ripreso la bozza di ${c.nome}` : 'Hai ripreso questa bozza — resta salvata in automatico');
  }
...
function renderWizardProgress() {
  ...
  if (titleEl && countEl) {
    ...
    if (whyEl) whyEl.textContent = step.key === 'narrative' ? WIZ_NARRATIVE_WHY[wizardNarrativeIndex] : (step.why || '');
  }
}
...
/* "Schermo acceso" (Fase 5, "evidenziazione tiri e schermo acceso"):
   Screen Wake Lock API standard, nessun plugin nativo aggiuntivo. Il
   sentinel va sempre ri-richiesto quando il documento torna visibile
   (il browser lo rilascia da solo appena l'app va in background — non è
   un bug nostro, è il comportamento previsto dalla spec) e va rilasciato
   uscendo dal combattimento: non ha senso tenere lo schermo acceso fuori
   da una sessione di gioco attiva. La preferenza è per-dispositivo
   (localStorage), non per-account: non è un dato dell'utente, è
   un'impostazione del device su cui gira l'app in quel momento. */
let combatWakeLockSentinel = null;
const COMBAT_WAKE_LOCK_PREF_KEY = 'rm_combat_keep_awake';
function combatWakeLockSupported() {
  return typeof navigator !== 'undefined' && typeof navigator.wakeLock !== 'undefined'
    && typeof navigator.wakeLock.request === 'function';
}
function combatWakeLockPreferred() {
  try { return localStorage.getItem(COMBAT_WAKE_LOCK_PREF_KEY) === '1'; } catch (e) { return false; }
}
function combatWakeLockSetPreferred(on) {
  try { localStorage.setItem(COMBAT_WAKE_LOCK_PREF_KEY, on ? '1' : '0'); } catch (e) {}
}
async function combatWakeLockAcquire() {
  if (!combatWakeLockSupported() || combatWakeLockSentinel) return;
  try {
    combatWakeLockSentinel = await navigator.wakeLock.request('screen');
    combatWakeLockSentinel.addEventListener('release', () => { combatWakeLockSentinel = null; });
  } catch (e) {
    // Negato dal browser (tab non visibile, risparmio energetico, ecc.):
    // nessun errore da mostrare, il toggle resta semplicemente inefficace
    // finché la richiesta non può essere ripetuta con successo.
    combatWakeLockSentinel = null;
  }
}
function combatWakeLockRelease() {
  if (!combatWakeLockSentinel) return;
  combatWakeLockSentinel.release().catch(() => {});
  combatWakeLockSentinel = null;
}
function combatWakeLockSyncToggleUI() {
  const row = $('#combat-wakelock-row'), toggle = $('#combat-wakelock-toggle');
  if (!row || !toggle) return;
  const supported = combatWakeLockSupported();
  row.classList.toggle('unsupported', !supported);
  toggle.disabled = !supported;
  toggle.checked = supported && combatWakeLockPreferred();
}
// Richiamata da showViewDom su ogni ingresso in #view-combat, e dal
// listener 'visibilitychange' qui sotto quando l'app torna in primo piano.
function combatWakeLockOnCombatViewEnter() {
  combatWakeLockSyncToggleUI();
  if (combatWakeLockPreferred()) combatWakeLockAcquire();
}
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState !== 'visible') return;
  if (typeof viewStack === 'undefined' || viewStack[viewStack.length - 1] !== 'combat') return;
  if (combatWakeLockPreferred()) combatWakeLockAcquire();
});
...
async function openCombatView(campaignId, isMaster, explicitEncounterId) {
  ...
  showView('combat');
  renderCombatBoard();
  if (typeof refreshCampaignChatEntryBadges === 'function') refreshCampaignChatEntryBadges(campaignId);
  await loadCombatEncounterForCampaign(campaignId, explicitEncounterId);
}

// explicitEncounterId (FC4, elenco incontri nel tab Combattimento della
// scheda storia): apre esattamente l'incontro scelto (anche 'ended'/
// 'archived', per rivederlo), invece di risolvere sempre l'ultimo
// staging/active della campagna — comportamento di default invariato
// quando non viene passato (es. il bottone generico "Vai al tabellone").
async function loadCombatEncounterForCampaign(campaignId, explicitEncounterId) {
  ...
  try {
    if (explicitEncounterId) {
      combatViewEncounterId = explicitEncounterId;
    } else {
      // 'archived' esclusa insieme a 'ended' (FC4): un incontro archiviato
      // è a tutti gli effetti concluso, non deve mai essere risolto come
      // "il combattimento in corso" di una campagna.
      const { data, error } = await withTimeout(
        sb.from('combat_encounters').select('*').eq('campaign_id', campaignId).not('status', 'in', '(ended,archived)')
...
function onCombatRealtimeChange() {
  refreshCombatBoard();
  // Registro di gioco (FC5): refreshCombatBoard riassegna combatState (da
  // cui il registro legge eventi/status), ma i "tiri" vivono in
  // lastCombatAttackLog, un fetch a parte — questo la tiene sincronizzata
  // in tempo reale mentre il registro è aperto, senza un secondo canale.
  if (typeof refreshCombatLogIfOpen === 'function') refreshCombatLogIfOpen();
}
...
async function refreshCombatBoard() {
  ...
  try {
    ...
    renderCombatBoard();
    // Ritratti in Storage: URL firmati generati al bisogno (mai salvati),
    // poi un secondo render solo se qualcosa è arrivato.
    if (typeof hydrateParticipantPortraits === 'function') {
      const boardAtRequest = combatState;
      hydrateParticipantPortraits(combatState && combatState.participants).then(changed => {
        if (changed && combatState === boardAtRequest) renderCombatBoard();
      }).catch(() => {});
    }
...
function combatParticipantName(p) {
  // displayName (FC4, etichetta locale del Narratore) ha sempre priorità
  // quando impostata: distingue copie della stessa creatura ("Goblin 1"/
  // "Goblin 2") indipendentemente dal fog-of-war — non è un dato del
  // personaggio, quindi non passa mai dalla redazione.
...
// Zoom (FC4): fattore locale al dispositivo, mai sincronizzato/persistito —
// ogni partecipante vede la mappa alla dimensione che preferisce, non è uno
// stato di gioco. Reimpostato a 1 ogni volta che si cambia incontro (vedi
// openCombatView) perché una griglia diversa può avere proporzioni diverse.
let combatMapZoom = 1;
const COMBAT_MAP_ZOOM_MIN = 0.6, COMBAT_MAP_ZOOM_MAX = 2.5, COMBAT_MAP_ZOOM_STEP = 0.2;
...
async function renderCombatMap() {
  ...
  // Zoom (FC4): moltiplica la larghezza cella invece di scalare via CSS
  // transform — le celle restano nitide (niente sfocatura/aliasing del
  // testo nelle etichette) e i target di click restano dove disegnati,
  // nessun secondo sistema di coordinate da sincronizzare col drag/tap.
  const cellW = (availW / (cols * 0.75 + 0.25)) * combatMapZoom;
  ...
  levelParticipants.forEach(p => {
    ...
    // escapeHtml sul ritratto non è cerimoniale: `portrait` è
    // characters.data.portrait, una stringa che il proprietario del
    // personaggio scrive via REST senza alcuna validazione di formato lato
    // RLS (il controllo è solo di proprietà, non di contenuto). Finisce
    // dentro un attributo style="..." di un template passato a innerHTML,
    // e la pedina è visibile a TUTTI i partecipanti al combattimento: una
    // doppia apice nel valore usciva dall'attributo e iniettava markup
    // nella pagina degli altri giocatori, non solo nella propria.
    const avatarStyle = portrait ? `background-image:url(${escapeHtml(portrait)});${portraitPosCss(p.portraitPos)}` : '';
    ...
    // Integrazione FASE FINALE, punti 5-6: desaturazione fuori turno/anello
    // di turno — dipende SOLO dallo stato autorevole dell'incontro
    // (combatIsCurrentTurn, già sincronizzato per tutti via il tabellone),
    // mai un flag locale.
    const inTurnClass = combatIsCurrentTurn(p) ? ' combat-token-in-turn' : '';
    ...
    // Icone di stato sulla pedina (FC4): stessa lista di combatEffectBadgesHtml
    // (barra ritratti), ma senza bottoni interattivi qui — toccarli
    // interferirebbe col tap/drag della pedina, che resta solo per
    // selezione/spostamento. Sola lettura a colpo d'occhio sulla mappa.
    const statusIconsHtml = combatTokenStatusIconsHtml(p);
    // Bottone "apri scheda/azioni" sulla pedina (FC4): prima non c'era
    // ALCUN modo di aprire la scheda toccando direttamente il token (solo
    // dalla barra ritratti in basso, vedi audit) — bottone separato invece
    // di sovraccaricare il tap sul corpo della pedina, che resta identico
    // a prima (seleziona per spostamento/risolve un bersaglio pendente).
    const menuBtnHtml = `<button type="button" class="combat-token-menu-btn" data-tokenmenu="${p.characterId}" aria-label="Apri scheda o azioni" title="Apri scheda o azioni">⋮</button>`;
    html += `<div class="combat-token${mineClass}${selectedClass}${targetableClass}${multiPickedClass}${boostGlowClass}${inTurnClass}" data-tokenchar="${p.characterId}"
    ...
    // Overlay NON clippato: .combat-token ha clip-path esagonale (per
    // l'anello colorato via box-shadow inset, vedi commento sopra) — quel
    // clip-path ritaglia via qualunque discendente posizionato fuori dal
    // box (angoli, o interamente sopra/sotto). Etichetta nome e barra di
    // movimento (bottom:100%/top:100%, entrambe fuori dal box per
    // costruzione) erano quindi SEMPRE invisibili — bug preesistente
    // scoperto durante FC4, non introdotto da FC4. Le icone/bottone di FC4
    // vivevano già in questo fratello separato per lo stesso motivo:
    // spostate qui anche etichetta e barra di movimento, stessa
    // posizione/dimensione del token ma senza il clip-path che le
    // riguardava.
    html += `<div class="combat-token-overlay" style="left:${tokenCenterLeft}px;top:${tokenCenterTop}px;width:${tokenW}px;height:${tokenH}px;">
...
/* Versione compatta di combatEffectBadgesHtml per la pedina sulla mappa
   (FC4, "visualizzazione status" — prima solo nella barra ritratti, mai
   sopra il token stesso): stesse icone, nessun bottone interattivo (il
   tap sulla pedina resta solo per selezione/spostamento) e al massimo 4
   per non coprire la faccia della pedina. */
function combatTokenStatusIconsHtml(p) {
  const effects = combatEffectsForChar(p.characterId);
  if (!effects.length) return '';
  const icons = effects.slice(0, 4).map(e => {
    const status = percentContestStatusInfo(e.status_key);
    const isStatMod = Array.isArray(e.stat_mods) && e.stat_mods.length;
    return status ? status.icon : (isStatMod ? (e.phase === 'malus' ? '💥' : '💊') : combatEffectIcon(e.effect_kind));
  });
  return `<div class="combat-token-status-icons" aria-hidden="true">${icons.join('')}</div>`;
}
...
function combatRosterCardHtml(p) {
  ...
  const avatarStyle = portrait ? ` style="background-image:url(${escapeHtml(portrait)});${portraitPosCss(p.portraitPos)}"` : '';
...
function openCombatOwnActionsMenu(characterId) {
  ...
  if (isActive && (isMaster || mine) && combatIsCurrentTurn(p)) {
    ...
    // Correttivo (FC4): torna al partecipante precedente SOLO dentro lo
    // stesso round (combat_previous_turn rifiuta di attraversare un cambio
    // di round già applicato, vedi la migrazione) — utile per un "passa
    // turno" per errore, non un vero annullamento generale.
    buttons.push(`<button class="btn btn-ghost" id="combat-own-previous-turn">⏮ Turno precedente</button>`);
  }
  // Iniziativa manuale (FC4): corregge il valore di QUESTO partecipante e
  // ricalcola l'ordine — disponibile solo al Narratore, in qualunque
  // momento (prima o dopo il tiro automatico), mai un secondo modo di
  // avviare l'iniziativa di tutti (vedi narratore_set_manual_initiative).
  if (isMaster) {
    buttons.push(`<button class="btn btn-ghost" data-combatmanualinitiative="${characterId}">✎ Iniziativa manuale</button>`);
    buttons.push(`<button class="btn btn-ghost" data-combatrenameparticipant="${characterId}">✎ Rinomina nel tabellone</button>`);
  }
...
/* "Modalità gioco" (S5, piano UX "confronto D&D Beyond"): preferenza
   persistente PER-DISPOSITIVO (mai legata al personaggio, mai sincronizzata
   col cloud) per nascondere le tab di edit/progressione (Identità,
   Background, Statistiche, Livelli) e lasciare solo ciò che serve in
   sessione (Fronte Scheda, Tratti, Retro Scheda, Equipaggiamento). Si
   somma — non sostituisce — le restrizioni automatiche già esistenti
   (combatSheetRestricted/narratorEditIsNpc, vedi showViewDom); va
   applicata SOLO ai propri personaggi, mai durante l'editing Narratore
   di un personaggio altrui/di un PNG (narratorEditMode/narratorEditIsNpc),
   dove Identità/Statistiche possono davvero servire indipendentemente
   dalla preferenza personale impostata sui propri. */
const SHEET_PLAYMODE_KEY = 'ms_sheet_playmode_v1';
const SHEET_PLAYMODE_HIDDEN_TABS = ['identita', 'note', 'statistiche', 'livelli'];
let sheetPlayModeOnly = (() => {
  try { return localStorage.getItem(SHEET_PLAYMODE_KEY) === '1'; } catch (e) { return false; }
})();
function updateSheetPlayModeButton() {
  const btn = $('#btn-sheet-playmode');
  if (!btn) return;
  // Nessun effetto visibile durante l'editing Narratore di un altrui/PNG
  // (vedi commento sopra): il pulsante resta nascosto, non solo disattivato,
  // per non far credere che l'interruttore agisca lì.
  btn.classList.toggle('hidden', narratorEditIsNpc || narratorEditMode);
  btn.classList.toggle('active', sheetPlayModeOnly);
  btn.setAttribute('aria-pressed', String(sheetPlayModeOnly));
  btn.title = sheetPlayModeOnly ? 'Modalità gioco attiva — mostra tutte le tab' : 'Attiva la modalità gioco (nasconde Identità, Background, Statistiche, Livelli)';
}
function toggleSheetPlayMode() {
  sheetPlayModeOnly = !sheetPlayModeOnly;
  try { localStorage.setItem(SHEET_PLAYMODE_KEY, sheetPlayModeOnly ? '1' : '0'); } catch (e) {}
  // Se la tab attiva sparisce, sposta su "Fronte Scheda" (sempre visibile).
  if (sheetPlayModeOnly && !narratorEditIsNpc && !narratorEditMode) {
    const activeBtn = $('#tabs .tab-btn.active');
    if (activeBtn && SHEET_PLAYMODE_HIDDEN_TABS.includes(activeBtn.dataset.tab)) showTab('gioco');
  }
  showViewDom('sheet');
  toast(sheetPlayModeOnly ? 'Modalità gioco attiva' : 'Modalità gioco disattivata');
}
...
function applyNarratorEditUiState() {
  ...
  if (body && active) {
    ...
    // Ogni controllo disabilitato QUI viene marcato (data-narrator-locked),
    // così "Modifica" lo riabilita anche se è fisso nella pagina e non
    // viene ridisegnato da renderSheet (es. i "✎ Modifica" dei contenitori
    // del Background): prima restavano disabilitati per sempre.
    restoreNarratorLockedControls(body);
    controls.forEach(el => { el.dataset.narratorAllowed = el.disabled ? '0' : '1'; });
    const lockControl = el => { if (!el.disabled) { el.disabled = true; el.dataset.narratorLocked = '1'; } };
    if (!editing) {
      controls.forEach(lockControl);
      body.querySelectorAll('[data-tecabedit]').forEach(el => { el.disabled = false; delete el.dataset.narratorLocked; });
      ...
      if (card) card.querySelectorAll('input, select, textarea, button').forEach(el => {
        if (el.dataset.narratorAllowed !== '0') { el.disabled = false; delete el.dataset.narratorLocked; }
      });
    }
  }
  const name = $('#f-nome');
...
/* Riabilita solo i controlli disabilitati dalla modalità Narratore (mai
   quelli disabilitati dalla scheda stessa per le sue regole). */
function restoreNarratorLockedControls(root) {
  if (!root) return;
  root.querySelectorAll('[data-narrator-locked]').forEach(el => {
    el.disabled = false;
    delete el.dataset.narratorLocked;
  });
}
...
function openCharacterForNarratorEdit(row) {
  ...
  renderSheet();
  showView('sheet');
  // Ritratto in Storage: URL firmato generato al bisogno (autorizzazione
  // lato server: get_character_portraits + policy del bucket), solo in
  // memoria — la copia "ospite" non lo invia mai (characterCloudPayload).
  if (!c.portrait && typeof signedPortraitUrls === 'function') {
    signedPortraitUrls([row.id]).then(urls => {
      if (!urls[row.id] || getActive() !== c) return;
      c.portrait = urls[row.id];
      if (narratorEditSnapshot) narratorEditSnapshot.portrait = urls[row.id];
      renderPortrait(c);
      renderHeader(c);
    }).catch(() => {});
  }
...
function exitNarratorEditMode() {
  ...
  activeId = null;
  saveAll();
  // Uscendo, nessun controllo fisso della scheda deve restare disabilitato
  // per le schede aperte dopo (anche quelle personali del Narratore).
  restoreNarratorLockedControls($('#view-sheet .sheet-body'));
  const banner = $('#narrator-edit-banner');
...
/* Registro attacchi (S16, piano UX "confronto D&D Beyond"): sola lettura,
   mostra solo cifre già viste dal vivo da chiunque ha assistito
   all'incontro (vedi commento su listCombatAttackLog, js/cloud-combat.js)
   — mai un dato nuovo, mai uno stat block. Filtro per personaggio
   puramente client-side sui dati già arrivati (stesso principio già usato
   per il filtro di ricerca S9/S12), nessuna nuova query al cambio filtro. */
/* Registro di gioco condiviso (FC5, FASE FINALE): prima ("Registro
   attacchi", S16) mostrava SOLO gli attacchi risolti — qui unificato con
   due fonti REALI già esistenti, mai simulate lato client:
   - "eventi" (avvio/fine incontro): combatState.encounter.created_at/
     ended_at, colonne già persistite da narratore_start_combat_encounter/
     narratore_end_combat_encounter (nessuna query in più, combatState è
     già in memoria).
   - "status" (applicazione di un effetto): combatState.activeEffects,
     già scaricato da get_combat_board per i badge nella barra ritratti —
     qui solo riletto per il registro. NOTA (lacuna nota, non bloccante):
     mostra solo l'APPLICAZIONE, mai la rimozione/scadenza, perché
     combat_active_effects conserva solo gli effetti ancora attivi (la
     riga sparisce alla rimozione, nessuno storico) — una cronologia
     completa richiederebbe una tabella di log dedicata, non ancora
     giustificata da alcuna UI reale.
   I "tiri" (combat_attacks, via listCombatAttackLog) restano l'unica
   fonte che richiede una query dedicata. */
let lastCombatAttackLog = [];
const COMBAT_LOG_PAGE_SIZE = 25;
let combatLogVisibleCount = COMBAT_LOG_PAGE_SIZE;
let combatLogExpandedAttackIds = new Set();
function combatLogDefenseLabel(atk) {
  if (atk.defense_type === 'dodge') return atk.defense_success ? 'schivata riuscita' : 'schivata fallita';
  if (atk.defense_type === 'block') return atk.defense_success ? 'blocco riuscito' : 'blocco fallito';
  return 'nessuna difesa';
}
// "Richiamare il dettaglio del tiro": ogni riga di tipo tiro/danno può
// espandersi mostrando i tiri grezzi già salvati in combat_attacks (mai
// ricalcolati qui) — per colpire, danno, difesa, salvezza di Robustezza.
function combatLogAttackDetailHtml(atk) {
  const lines = [
    atk.attack_roll_detail ? `Per colpire: ${escapeHtml(atk.attack_roll_detail)}` : null,
    atk.damage_roll_detail ? `Danno: ${escapeHtml(atk.damage_roll_detail)}` : null,
    atk.defense_roll_detail ? `Difesa (${combatLogDefenseLabel(atk)}): ${escapeHtml(atk.defense_roll_detail)}` : null,
    atk.robustezza_roll_detail ? `Robustezza: ${escapeHtml(atk.robustezza_roll_detail)}` : null
  ].filter(Boolean);
  if (!lines.length) return '';
  return `<div style="border-top:1px solid var(--bordo-scuro);margin-top:4px;padding-top:4px;display:flex;flex-direction:column;gap:2px;">
    ${lines.map(l => `<p class="helper-text" style="margin:0;">${l}</p>`).join('')}
  </div>`;
}
function combatLogRowHtml(entry) {
  if (entry.kind === 'evento') {
    return `<div class="box" style="margin-bottom:8px;"><div class="box-bar"></div><div class="box-pad" style="display:flex;flex-direction:column;gap:2px;">
      <p style="margin:0;">🚩 <b>${escapeHtml(entry.label)}</b></p>
      <p class="helper-text" style="margin:0;">${notificationRelativeTime(entry.ts)}</p>
    </div></div>`;
  }
  if (entry.kind === 'status') {
    const e = entry.data;
    const status = percentContestStatusInfo(e.status_key);
    const icon = status ? status.icon : combatEffectIcon(e.effect_kind);
    const label = status ? status.label : (e.source_label || 'Effetto');
    const targetName = combatCharNameById(e.target_character_id);
    return `<div class="box" style="margin-bottom:8px;"><div class="box-bar"></div><div class="box-pad" style="display:flex;flex-direction:column;gap:2px;">
      <p style="margin:0;">${icon} <b>${escapeHtml(label)}</b> applicato a <b>${escapeHtml(targetName)}</b></p>
      <p class="helper-text" style="margin:0;">${escapeHtml(e.source_label || '')}${e.source_label ? ' — ' : ''}${notificationRelativeTime(entry.ts)}</p>
    </div></div>`;
  }
  const atk = entry.data;
  const attackerName = atk.is_environmental ? 'Evento' : combatCharNameById(atk.attacker_character_id);
  const targetName = combatCharNameById(atk.target_character_id);
  const expanded = combatLogExpandedAttackIds.has(atk.id);
  const detailHtml = combatLogAttackDetailHtml(atk);
  return `<button type="button" class="box" data-combatlogattack="${atk.id}" style="margin-bottom:8px;width:100%;text-align:left;cursor:${detailHtml ? 'pointer' : 'default'};background:var(--antracite-2);border:none;">
    <div class="box-bar"></div><div class="box-pad" style="display:flex;flex-direction:column;gap:4px;">
    <p style="margin:0;"><b>${escapeHtml(attackerName)}</b> → <b>${escapeHtml(targetName)}</b> con <i>${escapeHtml(atk.source_label || '')}</i>${atk.is_surprise_attack ? ' <span class="chip">Sorpresa</span>' : ''}</p>
    <p class="helper-text" style="margin:0;">Round ${atk.round_number} — ${combatLogDefenseLabel(atk)} — Danno finale: ${atk.final_damage != null ? atk.final_damage : '—'}${detailHtml ? (expanded ? ' · nascondi dettaglio ▲' : ' · dettaglio tiro ▼') : ''}</p>
    ${expanded ? detailHtml : ''}
  </div></button>`;
}
// Unifica le tre fonti reali (vedi commento di testa) in un'unica lista
// ordinata dal più recente — "nessuna simulazione client": ogni voce
// arriva da una colonna/riga già persistita, mai fabbricata qui.
function buildUnifiedCombatLog() {
  const entries = lastCombatAttackLog.map(atk => ({ kind: 'tiro', ts: atk.created_at, data: atk }));
  const enc = combatState && combatState.encounter;
  if (enc) {
    if (enc.created_at) entries.push({ kind: 'evento', ts: enc.created_at, label: 'Incontro avviato' });
    if (enc.ended_at) entries.push({ kind: 'evento', ts: enc.ended_at, label: 'Incontro concluso' });
  }
  (combatState && combatState.activeEffects || []).forEach(e => entries.push({ kind: 'status', ts: e.created_at, data: e }));
  entries.sort((a, b) => new Date(b.ts) - new Date(a.ts));
  return entries;
}
function combatLogFilteredEntries() {
  const kindSel = $('#combat-log-filter-kind');
  const charSel = $('#combat-log-filter-char');
  const kind = kindSel ? kindSel.value : '';
  const charId = charSel ? charSel.value : '';
  let entries = buildUnifiedCombatLog();
  if (kind) entries = entries.filter(e => e.kind === kind);
  if (charId) {
    entries = entries.filter(e => {
      if (e.kind === 'tiro') return e.data.attacker_character_id === charId || e.data.target_character_id === charId;
      if (e.kind === 'status') return e.data.target_character_id === charId || e.data.source_character_id === charId;
      return false; // gli eventi di incontro non riguardano un personaggio specifico
    });
  }
  return entries;
}
function renderCombatLogList() {
  ...
  const loadMoreBtn = $('#combat-log-load-more');
  if (!list) return;
  const entries = combatLogFilteredEntries();
  const visible = entries.slice(0, combatLogVisibleCount);
  list.innerHTML = visible.length
    ? visible.map(combatLogRowHtml).join('')
    : `<p class="helper-text" style="margin:0;">${entries.length ? 'Nessuna voce per questo filtro.' : 'Nessun evento ancora in questo incontro.'}</p>`;
  // Caricamento progressivo (FC5): non tutta la lista in una volta su
  // incontri lunghi — altre COMBAT_LOG_PAGE_SIZE voci per click, mai un
  // secondo fetch di rete (i dati sono già tutti in memoria).
  if (loadMoreBtn) loadMoreBtn.classList.toggle('hidden', entries.length <= visible.length);
}
async function openCombatLogModal() {
  const modal = $('#combat-log-modal');
  if (!modal || !combatViewEncounterId) return;
  modal.classList.remove('hidden');
  combatLogVisibleCount = COMBAT_LOG_PAGE_SIZE;
  combatLogExpandedAttackIds = new Set();
  const sel = $('#combat-log-filter-char');
  if (sel) {
    sel.innerHTML = '<option value="">Tutti</option>' + (combatState.participants || [])
      .map(p => `<option value="${p.characterId}">${escapeHtml(combatParticipantName(p))}</option>`).join('');
  }
  $('#combat-log-list').innerHTML = '<p class="helper-text" style="margin:0;">Verifica in corso…</p>';
  try {
    lastCombatAttackLog = await listCombatAttackLog(combatViewEncounterId);
  } catch (e) {
    lastCombatAttackLog = [];
    $('#combat-log-list').innerHTML = `<p class="helper-text" style="margin:0;">Errore: ${escapeHtml(describeError(e))}</p>`;
    return;
  }
  renderCombatLogList();
}
// Condivisione Realtime (FC5): richiamata da onCombatRealtimeChange (mai
// un canale a parte) solo se il registro è aperto in quel momento — evita
// un fetch inutile di combat_attacks quando nessuno lo sta guardando.
// eventi/status si aggiornano già da soli (combatState viene riassegnato
// prima che questa funzione sia richiamata).
async function refreshCombatLogIfOpen() {
  const modal = $('#combat-log-modal');
  if (!modal || modal.classList.contains('hidden') || !combatViewEncounterId) return;
  try { lastCombatAttackLog = await listCombatAttackLog(combatViewEncounterId); }
  catch (e) { return; }
  renderCombatLogList();
}
...
function combatStatusOutcomeLineHtml(status, attackerRoll, defenderRoll, landed) {
  ...
  const rollLine = `${escapeHtml(status.label)} — attacco ${attackerRoll} vs resistenza ${defenderRoll}`;
  if (!landed) {
    return `<p class="helper-text" style="margin:0;">${status.icon} ${rollLine}: stato non entrato.</p>`;
  }
  const wired = ['bruciare', 'avvelenare', 'tramortire', 'elettrificare', 'stordire', 'immobilizzare', 'rallentare', 'confondere', 'corrodere', 'silenziare', 'congelare', 'accecare'].includes(status.key);
  const durataInfo = status.key === 'tramortire'
    ...
    : status.key === 'accecare'
    ? ' (2 turni: Mira ridotta del 70% sui tiri "Per colpire")'
...
function combatStatusOutcomeHtml(atk) {
  ...
  if (!status) return '';
  return combatStatusOutcomeLineHtml(status, atk.attacker_status_roll_total, atk.defender_status_roll_total, !!atk.status_applied);
}
/* Stessa narrazione di combatStatusOutcomeHtml, ma per ciascun effetto
   AGGIUNTIVO configurato sulla riga (atk.extra_effects) — checkpoint
   "creazione abilità": una Tecnica/Abilità può avere più di un effetto di
   stato, quindi anche l'esito va raccontato per ognuno, non solo per il
   principale. Un elemento senza nome, o fuori dal catalogo stati, non
   produce alcuna riga (stesso trattamento del principale/arma). */
function combatExtraStatusOutcomeHtml(atk) {
  return (Array.isArray(atk.extra_effects) ? atk.extra_effects : [])
    .map(ee => {
      const status = percentContestStatusInfo(ee && ee.nome);
      if (!status) return '';
      return combatStatusOutcomeLineHtml(status, ee.attackerRollTotal, ee.defenderRollTotal, !!ee.landed);
    })
    .join('');
}
...
function renderCombatAttackPanel() {
  ...
  if (atk.is_danno_fisso) {
    ...
    body += combatExtraStatusOutcomeHtml(atk);
  ...
  if (atk.status === 'save_rolled') {
    ...
    if (atk.is_critical_hit) {
      body += '<p class="helper-text combat-crit-highlight" style="margin:0;color:var(--fisico-forte);">🎯 Colpo critico! Danno raddoppiato.</p>';
    }
    if (atk.is_frozen_crit) {
      body += `<p class="helper-text combat-crit-highlight" style="margin:0;color:var(--fisico-forte);">❄️ ${escapeHtml(atk.targeted_body_part || '')}: colpo alla parte congelata — critico confermato, danno raddoppiato!</p>`;
    }
    body += `<p style="margin:0;"><b>Danno finale: ${atk.final_damage}</b></p>`;
...
function combatComputeAttackRoll(atk, attackerData) {
  ...
  // Effetti di stato AGGIUNTIVI (atk.extra_effects, vedi
  // combat_resolve_extra_status_entry/effettoBlockHtml): stesso identico
  // principio del binario Tecnica/arma sopra, un tiro d100 indipendente per
  // ciascun effetto configurato oltre al principale — mai lo stesso dado,
  // ognuno può landare, fallire o essere immune per conto proprio. Un
  // elemento con un nome fuori dal catalogo (percentContestStatusInfo
  // nullo) non tira nulla, stesso trattamento del principale/arma.
  const extraStatusRolls = (Array.isArray(atk.extra_effects) ? atk.extra_effects : []).map(ee => {
    const match = percentContestStatusInfo(ee && ee.nome);
    if (!match) return null;
    const equipBonus = equipBonusTotal(attackerData, 'status', match.key);
    const drugBonus = statusRollBonus(atk.attacker_character_id, match.key);
    const abilityBonus = Number(ee.bonusPct) || 0;
    const totalBonus = equipBonus + drugBonus + abilityBonus;
    const d100 = rollDie(100, `Stato extra: ${match.label} (d100)`);
    const total = d100 + totalBonus;
    return { total, detail: totalBonus ? `d100:${d100} +${totalBonus} (${match.label})` : `d100: ${d100}` };
  });
  ...
  return { attackRollTotal, attackRollDetail, damageRollTotal, damageRollDetail, damageRollTotal2, damageRollDetail2, statusRollTotal, statusRollDetail, perforareTaglioRollTotal, perforareTaglioRollDetail, weaponStatusRollTotal, weaponStatusRollDetail, counterRollTotal, counterRollDetail, extraStatusRolls };
}
...
async function combatRollDefense(atk, type) {
  ...
  // Tiro di resistenza del bersaglio per ciascun effetto AGGIUNTIVO
  // (atk.extra_effects), stesso principio del principale/arma sopra —
  // stesso ordine, mai lo stesso dado, nessun tiro per un elemento fuori
  // dal catalogo stati (vedi combatComputeAttackRoll/effettoBlockHtml).
  const extraStatusRolls = (Array.isArray(atk.extra_effects) ? atk.extra_effects : []).map(ee => {
    const match = percentContestStatusInfo(ee && ee.nome);
    if (!match) return null;
    const r = combatStatusResistRoll(targetData, targetId, (ee && ee.tratto) || 'Resistenza', match.key, `Stato extra: ${match.label}`, atk.targeted_body_part);
    return { total: r.total, detail: r.detail };
  });

  if (type === 'none') {
    try {
      await submitCombatDefenseRoll(atk.id, 'none', null, null, auto1.total, auto1.detail, auto2 ? auto2.total : null, auto2 ? auto2.detail : null, statusRollTotal, statusRollDetail, weaponStatusRollTotal, weaponStatusRollDetail, extraStatusRolls);
      await refreshCombatBoard();
    } catch (err) { toast(describeError(err)); }
    return;
  }
  ...
  try {
    await submitCombatDefenseRoll(atk.id, type, total, detail, auto1.total, auto1.detail, auto2 ? auto2.total : null, auto2 ? auto2.detail : null, statusRollTotal, statusRollDetail, weaponStatusRollTotal, weaponStatusRollDetail, extraStatusRolls);
...
/* Effetti di stato AGGIUNTIVI di una riga (r.effettiExtra, vedi
   effettoBlockHtml/makeTecnicaRow/makeAbilitaRow), nella forma che
   declare_combat_attack/apply_danno_fisso si aspettano in p_source —
   bonusPct sempre 0 per le Tecniche, stesso trattamento del principale. */
function combatEffettiExtraFor(r, rowKind) {
  return (Array.isArray(r.effettiExtra) ? r.effettiExtra : [])
    .filter(ee => ee && ee.nome)
    .map(ee => ({ nome: ee.nome, tratto: ee.tratto || '', bonusPct: rowKind === 'abilita' ? (Number(ee.bonusPct) || 0) : 0 }));
}
  ...
  (data[field] || []).forEach((r, i) => {
    ...
    if ((isMisto || !r.durata || r.durata === 'gratuita') && (r.tipo === 'danno' || isMisto) && Number(r.dannoBase) > 0) {
      sources.push({
        ...
        // Effetti di stato aggiuntivi oltre al principale sopra (checkpoint
        // "creazione abilità": una Tecnica/Abilità può avere più di un
        // effetto di stato) — vedi combatEffettiExtraFor/declare_combat_attack.
        effettiExtra: combatEffettiExtraFor(r, rowKind),
...
async function resolveDannoFisso(encounterId, attackerCharacterId, targetCharacterId, source) {
  ...
  // Effetti di stato AGGIUNTIVI (source.effettiExtra): il "Danno fisso" non
  // ha una fase separata di dichiarazione/tiro/difesa come un attacco
  // normale, quindi entrambi i tiri (attaccante e resistenza del
  // bersaglio) vanno calcolati qui in un colpo solo, per OGNI effetto
  // configurato — stessa formula di sopra, stesso principio del binario
  // principale/extra già in uso per gli attacchi normali (vedi
  // combatComputeAttackRoll/combatRollDefense/apply_danno_fisso).
  const extraEffects = (Array.isArray(source.effettiExtra) ? source.effettiExtra : []).map(ee => {
    const entry = { nome: ee.nome || '', tratto: ee.tratto || '', bonusPct: Number(ee.bonusPct) || 0 };
    const match = percentContestStatusInfo(ee.nome);
    if (!match) return entry;
    const equipBonus = equipBonusTotal(attackerData, 'status', match.key);
    const drugBonus = statusRollBonus(attackerCharacterId, match.key);
    const totalBonus = equipBonus + drugBonus + entry.bonusPct;
    const d100 = rollDie(100, `Stato extra: ${match.label} (d100)`);
    entry.attackerRollTotal = d100 + totalBonus;
    entry.attackerRollDetail = totalBonus ? `d100:${d100} +${totalBonus} (${match.label})` : `d100: ${d100}`;
    const rDef = combatStatusResistRoll(targetData, targetCharacterId, entry.tratto || 'Resistenza', match.key, `Stato extra: ${match.label}`, null);
    entry.defenderRollTotal = rDef.total; entry.defenderRollDetail = rDef.detail;
    return entry;
  });
  try {
    await applyDannoFisso(encounterId, attackerCharacterId, targetCharacterId, source, statusAttackerRoll, statusAttackerDetail, statusDefenderRoll, statusDefenderDetail, extraEffects);
    await refreshCombatBoard();
...
function wireCombatView() {
  // Zoom (FC4): aggiorna solo il fattore locale + ridisegna la mappa,
  // nessuna scrittura di rete (non è uno stato di gioco condiviso).
  function applyCombatMapZoom(delta) {
    combatMapZoom = Math.max(COMBAT_MAP_ZOOM_MIN, Math.min(COMBAT_MAP_ZOOM_MAX, Math.round((combatMapZoom + delta) * 100) / 100));
    const label = $('#combat-zoom-level');
    if (label) label.textContent = Math.round(combatMapZoom * 100) + '%';
    renderCombatMap();
  }
  $('#combat-zoom-in').addEventListener('click', () => applyCombatMapZoom(COMBAT_MAP_ZOOM_STEP));
  $('#combat-zoom-out').addEventListener('click', () => applyCombatMapZoom(-COMBAT_MAP_ZOOM_STEP));
  ...
  $('#combat-map').addEventListener('click', e => {
    ...
    const menuBtn = e.target.closest('[data-tokenmenu]');
    if (menuBtn) {
      openCombatOwnActionsMenu(menuBtn.dataset.tokenmenu);
      return;
    }
  ...
  $('#combat-own-actions-body').addEventListener('click', e => {
    ...
    if (e.target.closest('#combat-own-previous-turn')) {
      $('#combat-own-actions-menu').classList.add('hidden');
      combatPreviousTurn(combatViewEncounterId, combatOwnActionsCharId)
        .then(async () => { await refreshCombatBoard(); toast('Turno tornato indietro'); })
        .catch(err => toast(describeError(err)));
      return;
    }
    ...
    const manualInitBtn = e.target.closest('[data-combatmanualinitiative]');
    if (manualInitBtn) {
      $('#combat-own-actions-menu').classList.add('hidden');
      const raw = prompt('Valore di iniziativa (numero):');
      if (raw === null) return;
      const value = parseInt(raw, 10);
      if (!Number.isFinite(value)) { toast('Inserisci un numero valido'); return; }
      setManualInitiative(combatViewEncounterId, manualInitBtn.dataset.combatmanualinitiative, value, 'manuale')
        .then(async () => { await refreshCombatBoard(); toast('Iniziativa aggiornata'); })
        .catch(err => toast(describeError(err)));
      return;
    }
    const renameParticipantBtn = e.target.closest('[data-combatrenameparticipant]');
    if (renameParticipantBtn) {
      $('#combat-own-actions-menu').classList.add('hidden');
      const charId = renameParticipantBtn.dataset.combatrenameparticipant;
      const pCur = (combatState.participants || []).find(pp => pp.characterId === charId);
      const name = prompt('Etichetta nel tabellone (vuoto per tornare al nome del personaggio):', (pCur && pCur.displayName) || '');
      if (name === null) return;
      renameCombatParticipant(combatViewEncounterId, charId, name.trim())
        .then(async () => { await refreshCombatBoard(); toast('Etichetta aggiornata'); })
        .catch(err => toast(describeError(err)));
      return;
    }
  ...
  // ---- registro attacchi (S16): visibile a tutti, non solo al Narratore ----
  $('#btn-open-combat-log').addEventListener('click', () => openCombatLogModal());
  $('#btn-open-quickref').addEventListener('click', () => {
    const idx = RULES_CHAPTERS.findIndex(c => c.t === 'Riferimento rapido');
    if (idx >= 0) openRulesChapter(idx);
  });
  // ---- chat di campagna (FC6): "apertura dal gioco", stesso modale della scheda storia ----
  $('#btn-open-campaign-chat-combat').addEventListener('click', () => openCampaignChatModal(combatViewCampaignId));
  $('#combat-log-close').addEventListener('click', () => $('#combat-log-modal').classList.add('hidden'));
  $('#combat-log-filter-char').addEventListener('change', () => { combatLogVisibleCount = COMBAT_LOG_PAGE_SIZE; renderCombatLogList(); });
  $('#combat-log-filter-kind').addEventListener('change', () => { combatLogVisibleCount = COMBAT_LOG_PAGE_SIZE; renderCombatLogList(); });
  $('#combat-log-load-more').addEventListener('click', () => { combatLogVisibleCount += COMBAT_LOG_PAGE_SIZE; renderCombatLogList(); });
  // Espandi/richiudi il dettaglio di un tiro (FC5): un solo listener
  // delegato, la lista viene ridisegnata a ogni filtro/pagina quindi i
  // bottoni cambiano identità DOM di continuo.
  $('#combat-log-list').addEventListener('click', e => {
    const row = e.target.closest('[data-combatlogattack]');
    if (!row) return;
    const id = row.dataset.combatlogattack;
    if (combatLogExpandedAttackIds.has(id)) combatLogExpandedAttackIds.delete(id);
    else combatLogExpandedAttackIds.add(id);
    renderCombatLogList();
  });
  ...
  $('#combat-attack-panel').addEventListener('click', async e => {
    ...
    if (e.target.closest('#btn-combat-apply-damage')) {
      try {
        // Durabilità di armatura/scudo/arma coinvolti: calcolata dal SERVER
        // (20261001020000_durability_server_authoritative.sql) dai dati
        // reali, con dadi tirati lato server. Il client invia solo l'azione.
        await applyCombatAttackDamage(atk.id);
        await refreshCombatBoard();
  ...
  $('#combat-wakelock-toggle').addEventListener('change', e => {
    combatWakeLockSetPreferred(e.target.checked);
    if (e.target.checked) combatWakeLockAcquire(); else combatWakeLockRelease();
  });
}
...
function renderCharList() {
  ...
  if (!visible.length) {
    ...
    wrap.innerHTML = sorted.map(c => {
      ...
      const portraitStyle = c.portrait ? ` style="background-image:url(${escapeHtml(c.portrait)});${portraitPosCss(c.portraitPos)}"` : '';
...
/* =============================================== ONBOARDING PER RUOLO (S2)
   Piano UX "confronto D&D Beyond": tre percorsi verso un'azione reale già
   esistente nell'app — nessuna nuova schermata intermedia inventata.
   "Voglio narrare" non porta a una "storia dimostrativa" (non esiste
   ancora un contenuto demo, fa parte della Biblioteca editoriale non
   ancora popolata, S13/UX2): porta onestamente al punto reale dove si
   diventa Narratore e si crea la prima storia (Account, tab Narratore,
   già attiva di default — mostra da sola il passo mancante: accesso con
   account permanente, poi "Diventa Narratore", poi creazione). Nessuna
   azione automatica in background (mai un acquisto di ruolo silenzioso). */
function markOnboardingSeen() {
  try { localStorage.setItem(ONBOARDING_SEEN_KEY, '1'); } catch (e) {}
}
function renderOnboardingView() {
  const playerBtn = $('#onb-role-player');
  const narratorBtn = $('#onb-role-narrator');
  const rulesBtn = $('#onb-role-rules');
  const skipBtn = $('#onb-skip');
  if (playerBtn) playerBtn.onclick = () => { markOnboardingSeen(); createCharacterFlow(); };
  if (narratorBtn) narratorBtn.onclick = () => { markOnboardingSeen(); renderAccountArea(); showView('account'); };
  if (rulesBtn) rulesBtn.onclick = () => { markOnboardingSeen(); renderRules(); showView('rules'); };
  if (skipBtn) skipBtn.onclick = () => { markOnboardingSeen(); showView('cover'); };
}

/* =============================================== HOME "IL TUO TAVOLO" (S1)
   Piano UX "confronto D&D Beyond": sostituisce la copertina come punto di
   arrivo dopo l'accesso — la copertina resta intro/brand. Nessun dato
   inventato: ogni blocco legge una fonte già esistente altrove nell'app
   (personaggi locali, storie reali via listMyCampaigns/renderPlayerStories,
   sessione in corso già supportata dal Narratore tramite
   narratore_set_session_active). Niente feed promozionale, niente
   "prossima sessione" finta: se un dato non esiste ancora per davvero,
   qui compare uno stato vuoto onesto con un collegamento utile, mai un
   segnaposto che finge di funzionare. */
async function renderHomeView() {
  let session = null;
  try { session = await currentCloudSession(); } catch (e) {}
  const isRealSession = session && !isGuestUser(session);
  let profile = null;
  let accountRole = 'player';
  if (isRealSession) {
    try { profile = await getMyProfile(session.user.id); accountRole = (profile && profile.account_role) || 'player'; } catch (e) {}
  }
  const nickname = (profile && profile.display_name) || (session && session.user && session.user.email) || '';
  $('#home-greeting-title').textContent = nickname ? `Ciao, ${nickname}` : 'Il tuo tavolo';
  $('#home-greeting-sub').textContent = accountRole === 'narrator' || accountRole === 'admin'
    ? 'Riprendi da dove hai lasciato la tua storia.'
    : 'Riprendi da dove hai lasciato il tuo personaggio.';

  renderHomeResumeCard();
  renderHomeActions(accountRole, isRealSession);
  renderHomeCharactersRow();
  await renderHomeStoriesRow(isRealSession, accountRole);
  await renderHomeNotifSummary(isRealSession);
}

/* Notifiche non lette + ultima attività (FC2, macrofase completamento
   funzionale): stessa fonte dati del centro notifiche (S19,
   listMyNotifications) — nessuna seconda query o duplicazione di stato,
   solo un riepilogo in Home che rimanda a #view-notifications per il
   dettaglio. */
async function renderHomeNotifSummary(isRealSession) {
  const wrap = $('#home-notif-wrap');
  if (!wrap) return;
  if (!isRealSession || typeof listMyNotifications !== 'function') { wrap.innerHTML = ''; return; }
  try {
    const list = await listMyNotifications();
    if (!list.length) { wrap.innerHTML = ''; return; }
    const unread = list.filter(n => !n.read_at).length;
    const latest = list[0];
    // Stessa cache globale usata da renderNotificationsView: se l'utente
    // apre poi il centro notifiche completo, il nome è già risolto (nessun
    // doppio fetch), e qui evita di mostrare una storia "" per un tipo di
    // notifica che cita il nome della campagna (es. session_started).
    if (latest.campaign_id && typeof notificationCampaignNameById === 'object' && !notificationCampaignNameById[latest.campaign_id] && typeof fetchCampaignNamesById === 'function') {
      try {
        const names = await fetchCampaignNamesById([latest.campaign_id]);
        notificationCampaignNameById = Object.assign({}, notificationCampaignNameById, names);
      } catch (e) {}
    }
    const info = (typeof NOTIFICATION_TYPE_INFO === 'object' && NOTIFICATION_TYPE_INFO[latest.type]) || {};
    const latestText = (typeof notificationDetailText === 'function' ? notificationDetailText(latest) : (info.label || 'Attività recente'));
    const when = typeof notificationRelativeTime === 'function' ? notificationRelativeTime(latest.created_at) : '';
    wrap.innerHTML = `<button type="button" class="home-resume-card" id="home-notif-summary-btn">
      <div class="hr-icon"><span aria-hidden="true" style="font-size:20px;">${info.icon || '🔔'}</span></div>
      <div class="hr-body">
        <div class="hr-label">${unread > 0 ? `${unread} notifica${unread === 1 ? '' : 'e'} non lett${unread === 1 ? 'a' : 'e'}` : 'Nessuna notifica non letta'}</div>
        <div class="hr-title" style="font-size:14px;">${escapeHtml(latestText)}${when ? ' · ' + escapeHtml(when) : ''}</div>
      </div>
      ${unread > 0 ? `<span class="notif-badge">${unread}</span>` : '<div class="hr-arrow">→</div>'}
    </button>`;
    $('#home-notif-summary-btn').addEventListener('click', () => goToMenuTarget('notifications'));
  } catch (e) {
    wrap.innerHTML = '';
  }
}

function renderHomeResumeCard() {
  const wrap = $('#home-resume-wrap');
  const active = getActive();
  if (active) {
    // Indicatore di sincronizzazione (FC2): visibile solo per un
    // personaggio davvero collegato al cloud (cloudCharacterId) — per uno
    // solo locale non esiste alcuna sincronizzazione da segnalare, mai un
    // indicatore finto. cloudDirty è lo stesso flag già scritto da
    // touchActive()/scheduleCloudAutoPush (js/app.js), nessuno stato nuovo.
    let syncLabel = '';
    if (active.cloudCharacterId) {
      syncLabel = active.cloudDirty ? ' · ☁ sincronizzazione…' : ' · ☁ sincronizzato';
    }
    wrap.innerHTML = `<button class="home-resume-card" id="home-resume-btn">
      <div class="hr-icon"><svg viewBox="0 0 24 24" aria-hidden="true" stroke="#fff"><path d="M12 3v18M3 12h18"/></svg></div>
      <div class="hr-body">
        <div class="hr-label">Riprendi</div>
        <div class="hr-title">${escapeHtml(active.nome || 'Personaggio senza nome')}${escapeHtml(syncLabel)}</div>
      </div>
      <div class="hr-arrow">→</div>
    </button>`;
    $('#home-resume-btn').addEventListener('click', () => openCharacter(active.id));
  } else {
    wrap.innerHTML = '';
  }
}

function renderHomeActions(accountRole, isRealSession) {
  const wrap = $('#home-actions-wrap');
  if (!isRealSession) {
    wrap.innerHTML = `<div class="home-quick-actions">
      <button class="btn btn-primary" id="home-act-newchar">Crea un personaggio</button>
      <button class="btn btn-ghost" id="home-act-rules">Sfoglia le regole</button>
    </div>`;
    $('#home-act-newchar').addEventListener('click', () => createCharacterFlow());
    $('#home-act-rules').addEventListener('click', () => goToMenuTarget('rules'));
    return;
  }
  if (accountRole === 'narrator' || accountRole === 'admin') {
    wrap.innerHTML = `<div class="home-quick-actions">
      <button class="btn btn-primary" id="home-act-campaigns">Le tue storie</button>
      <button class="btn btn-ghost" id="home-act-rules">Sfoglia le regole</button>
    </div>`;
    // "Crea PNG" (openNpcRandomizerModal) resta raggiungibile solo da
    // dentro una storia aperta (#view-campaignsheet, pulsante
    // #btn-open-npc-randomizer): il PNG viene attribuito alla storia
    // letta da activeCampaignSheetId, uno stato che solo openCampaignSheet
    // imposta. Aprirlo da qui, senza prima scegliere la storia, rischierebbe
    // di attribuire il PNG a quella sbagliata (o a nessuna) — l'azione
    // rapida onesta è quindi "vai alle tue storie", non un accesso diretto.
    $('#home-act-campaigns').addEventListener('click', () => goToMenuTarget('campaigns'));
    $('#home-act-rules').addEventListener('click', () => goToMenuTarget('rules'));
  } else {
    wrap.innerHTML = `<div class="home-quick-actions">
      <button class="btn btn-primary" id="home-act-newchar">Crea un personaggio</button>
      <button class="btn btn-ghost" id="home-act-previously">Leggi il recap</button>
    </div>`;
    $('#home-act-newchar').addEventListener('click', () => createCharacterFlow());
    $('#home-act-previously').addEventListener('click', () => goToMenuTarget('previously'));
  }
}

function renderHomeCharactersRow() {
  const wrap = $('#home-characters-row');
  const visible = typeof visibleCharacters === 'function' ? visibleCharacters() : characters;
  if (!visible.length) {
    wrap.innerHTML = `<div class="home-empty-inline">Non hai ancora nessun personaggio. <button class="link-btn" id="home-empty-newchar" style="background:none;border:none;padding:0;text-decoration:underline;cursor:pointer;">Creane uno</button>.</div>`;
    const btn = $('#home-empty-newchar');
    if (btn) btn.addEventListener('click', () => createCharacterFlow());
    return;
  }
  const recent = [...visible].sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0)).slice(0, 6);
  wrap.innerHTML = '<div class="home-card-row">' + recent.map(c => {
    const b = BUILDS[c.build] || { label: '' };
    return `<button class="home-mini-card" data-homechar="${c.id}">
      <div class="hmc-title">${escapeHtml(c.nome || 'Senza nome')}</div>
      <div class="hmc-meta">${b.label} · Lv ${c.livello || 1}</div>
    </button>`;
  }).join('') + '</div>';
  wrap.querySelectorAll('[data-homechar]').forEach(btn => {
    btn.addEventListener('click', () => openCharacter(btn.dataset.homechar));
  });
}

async function renderHomeStoriesRow(isRealSession, accountRole) {
  const wrap = $('#home-stories-row');
  const sessionWrap = $('#home-session-wrap');
  const combatWrap = $('#home-combat-resume-wrap');
  if (!isRealSession) {
    wrap.innerHTML = '<div class="home-empty-inline">Accedi per vedere le tue storie.</div>';
    sessionWrap.innerHTML = '';
    if (combatWrap) combatWrap.innerHTML = '';
    return;
  }
  try {
    let stories = [];
    if (accountRole === 'narrator' || accountRole === 'admin') {
      stories = (await listMyCampaigns()).map(c => ({ id: c.id, name: c.name, sessionActive: c.session_active, sessionLabel: c.session_label }));
    } else {
      const visible = typeof visibleCharacters === 'function' ? visibleCharacters() : characters;
      const seen = new Set();
      stories = visible.filter(c => c.cloudCampaignId && !c.cloudCampaignTrashedAt).filter(c => {
        if (seen.has(c.cloudCampaignId)) return false;
        seen.add(c.cloudCampaignId);
        return true;
      }).map(c => ({ id: c.cloudCampaignId, name: c.cloudCampaignName || 'Storia' }));
    }
    if (!stories.length) {
      wrap.innerHTML = `<div class="home-empty-inline">${accountRole === 'narrator' || accountRole === 'admin' ? 'Non hai ancora creato una storia.' : 'Non fai ancora parte di nessuna storia: apri un personaggio, tab Identità, per chiedere di entrare in una.'}</div>`;
      sessionWrap.innerHTML = '';
      if (combatWrap) combatWrap.innerHTML = '';
      return;
    }
    wrap.innerHTML = '<div class="home-card-row">' + stories.map(s =>
      `<button class="home-mini-card" data-homestory="${s.id}">
        <div class="hmc-title">${escapeHtml(s.name)}</div>
        <div class="hmc-meta">${s.sessionActive ? '🟢 Sessione in corso' : 'Storia'}</div>
      </button>`
    ).join('') + '</div>';
    wrap.querySelectorAll('[data-homestory]').forEach(btn => {
      btn.addEventListener('click', () => openCampaignSheet(btn.dataset.homestory));
    });
    const live = stories.find(s => s.sessionActive);
    if (live) {
      sessionWrap.innerHTML = `<button class="home-resume-card" id="home-session-btn">
        <div class="hr-icon"><svg viewBox="0 0 24 24" aria-hidden="true" stroke="#fff"><circle cx="12" cy="12" r="4"/></svg></div>
        <div class="hr-body">
          <div class="hr-label">Sessione in corso</div>
          <div class="hr-title">${escapeHtml(live.sessionLabel || live.name)}</div>
        </div>
        <div class="hr-arrow">→</div>
      </button>`;
      $('#home-session-btn').addEventListener('click', () => openCampaignSheet(live.id));
    } else {
      sessionWrap.innerHTML = '';
    }
    if (combatWrap) {
      try {
        const encounters = await listMyResumableEncounters(stories.map(s => s.id));
        const enc = encounters[0];
        if (enc) {
          const story = stories.find(s => s.id === enc.campaign_id);
          combatWrap.innerHTML = `<button class="home-resume-card" id="home-combat-resume-btn">
            <div class="hr-icon"><svg viewBox="0 0 24 24" aria-hidden="true" stroke="#fff"><path d="M6 6l12 12M18 6L6 18"/></svg></div>
            <div class="hr-body">
              <div class="hr-label">${enc.status === 'active' ? 'Combattimento in corso' : 'Combattimento in preparazione'}</div>
              <div class="hr-title">${escapeHtml(enc.label || (story ? story.name : 'Combattimento'))}</div>
            </div>
            <div class="hr-arrow">→</div>
          </button>`;
          $('#home-combat-resume-btn').addEventListener('click', () => openCampaignSheet(enc.campaign_id));
        } else {
          combatWrap.innerHTML = '';
        }
      } catch (e) {
        combatWrap.innerHTML = '';
      }
    ...
    wrap.innerHTML = `<div class="home-empty-inline">Impossibile caricare le storie: ${escapeHtml(describeError(e))}</div>`;
    sessionWrap.innerHTML = '';
    if (combatWrap) combatWrap.innerHTML = '';
  }
}
...
function renderBuildGrid(c) {
  ...
  grid.innerHTML = BUILD_KEYS.map(key => {
    ...
    return `<div class="build-card ${selected ? 'selected ' + selClass : ''} ${locked ? 'locked' : ''}" data-buildcard="${key}" role="button" tabindex="${locked ? -1 : 0}" aria-pressed="${selected}" aria-label="Classe ${escapeHtml(b.label)}">
...
// Campi raccolti in "Altri dettagli" (sempre visibili, mai dietro un
// <details> da aprire: timing/conteggi sono informazioni primarie per il
// funzionamento in combattimento, non secondarie — mai nascoste, come le
// altre, nome/tipo/effetto/bonus/malus/costo).
...
function editTecAbCardRows(id, rows, dataAttr, fields, cellRenderers, c) {
  ...
  container.innerHTML = pendingHtml + rows.map((r, i) => {
    const mainFields = fields.filter(f => f !== 'nome');
...
function tecabEditorSectionedHtml(r, i, dataAttr, fields, cellRenderers, c) {
  ...
  return `<div class="tecab-card tecab-editor-card" data-tecab-card="${dataAttr}::${i}">
    ...
    ${section('Altri dettagli', detailFields)}
  </div>`;
}
...
/* Ridimensiona (max 512px) e comprime (WebP) l'immagine scelta
   (compressPortraitImage, js/character-portraits.js). Per un personaggio
   solo locale il data-URL è l'unica copia, come prima; per uno nel cloud è
   la cache locale e portraitDirty fa sì che al prossimo salvataggio venga
   caricata nel bucket privato, sostituendo lo STESSO oggetto. */
function loadPortraitFile(file) {
  ...
  reader.onload = () => {
    const img = new Image();
    img.onload = () => {
      c.portrait = compressPortraitImage(img).dataUrl;
      c.portraitDirty = true;
...
const ONBOARDING_SEEN_KEY = 'ms_onboarding_seen_v1';
/* Accessibilità (S20, piano UX "confronto D&D Beyond"): "riduci le
   animazioni" forza le stesse regole già presenti sotto
   prefers-reduced-motion (css/style.css), indipendentemente
   dall'impostazione del sistema operativo — reale, non un interruttore
   senza effetto. Applicata prima di ogni render per evitare un lampo di
   animazione al primo frame. Locale per-dispositivo, stesso pattern di
   ms_sheet_playmode_v1. */
const A11Y_REDUCE_MOTION_KEY = 'ms_a11y_reduce_motion_v1';
function getReduceMotionPref() {
  try { return localStorage.getItem(A11Y_REDUCE_MOTION_KEY) === '1'; } catch (e) { return false; }
}
function applyAccessibilityPreferences() {
  document.documentElement.classList.toggle('a11y-reduce-motion', getReduceMotionPref());
}
function setReduceMotionPref(on) {
  try { localStorage.setItem(A11Y_REDUCE_MOTION_KEY, on ? '1' : '0'); } catch (e) {}
...
function init() {
  ...
  // Onboarding (S2, piano UX "confronto D&D Beyond"): mostrato una sola
  // volta per dispositivo, solo al primo avvio davvero "vergine" (nessun
  // personaggio mai creato qui, mai visto prima) — segnale locale, niente
  // rete: un utente che ha già almeno un personaggio non è nuovo, quale
  // che sia lo stato della sua sessione cloud (ancora sconosciuto a questo
  // punto, la verifica è asincrona). Mai più dopo la prima volta, anche
  // cancellando tutti i personaggi.
  let seenOnboarding = true;
  try { seenOnboarding = !!localStorage.getItem(ONBOARDING_SEEN_KEY); } catch (e) {}
  if (!seenOnboarding && characters.length === 0 && typeof renderOnboardingView === 'function') {
    viewStack = ['onboarding'];
    renderOnboardingView();
    showViewDom('onboarding');
  } else {
    showView('cover');
  }
  renderAppSignature();
  updateSignatureReservedSpace();
  wireStaticEvents();
  ...
  // Il controllo iniziale può essere già terminato (offline, nessuna
  // release o timeout): non ripeterlo solo perché pending è ora false.
  if (!window.RMStartup || !window.RMStartup.handlesInitialUpdate) checkForUpdate();
...
function wireStaticEvents() {
  ...
  // FC2 (macrofase completamento funzionale): "Compendio" in Home non era
  // mai stato collegato a un click handler (bottone statico, morto dalla
  // sua introduzione) — corretto qui insieme al nuovo collegamento
  // "Biblioteca", stesso pattern data-menu-nav già in uso altrove.
  // Repertorio: tre porte verso la stessa vista, ciascuna sulla propria
  // scheda. Delegato dalla riga, che e' statica in index.html.
  $('#home-repertoire-row').addEventListener('click', e => {
    const card = e.target.closest && e.target.closest('[data-repertoire]');
    if (!card) return;
    if (typeof renderRepertoireView === 'function') renderRepertoireView(card.dataset.repertoire);
    showView('repertoire');
  });

  $('#repertoire-tabs').addEventListener('click', e => {
    const btn = e.target.closest && e.target.closest('.tab-btn');
    if (!btn) return;
    if (typeof renderRepertoireView === 'function') renderRepertoireView(btn.dataset.reptab);
  });

  $('#repertoire-search').addEventListener('input', () => {
    if (typeof renderRepertoireItems === 'function') renderRepertoireItems();
  });

  // Una card sola: il Compendio porta alla vista a due schede
  // (Biblioteca/Avventure). Il lettore delle regole si apre da li',
  // aprendo il manuale — nessuna seconda porta in Home.
  $('#home-rules-cta').addEventListener('click', () => goToMenuTarget('library'));
  $$('[data-nav]').forEach(b => b.addEventListener('click', () => history.back()));
  $('#btn-char-menu').addEventListener('click', charMenu);
  $('#btn-sheet-playmode').addEventListener('click', toggleSheetPlayMode);
  wireCloudAccountEvents();
  wireCloudCharacterEvents();
  wireCampaignChatModal();
  if (typeof wireChatPushBanner === 'function') wireChatPushBanner();
  if (typeof wirePushPermissionBanner === 'function') wirePushPermissionBanner();
  if (typeof wireSettingsEvents === 'function') wireSettingsEvents();
  wireRulesEvents();
  ...
  $('#update-banner-btn').addEventListener('click', () => {
    if (!updateUrl) return;
    // In-app quando i plugin nativi necessari sono presenti (APK a
    // partire dalla release con @capacitor/filesystem + file-opener +
    // il permesso REQUEST_INSTALL_PACKAGES): downloadAndInstallNativeUpdate
    // ripiega DA SOLA sul browser per qualunque motivo non riesca — su un
    // APK installato prima di questa funzione, il ripiego è immediato.
    if (isNativeAndroidPlatform() && nativeUpdatePluginsAvailable()) {
      downloadAndInstallNativeUpdate(updateUrl, updateApkVersion, updateApkSize);
      return;
    }
    ...
    redirectToApkDownload(updateUrl);
  });
  ...
  coverMenuBtn.addEventListener('click', e => {
    ...
    if (!open && typeof updateNotificationsBadge === 'function') updateNotificationsBadge();
  });
  ...
  coverMenu.addEventListener('click', e => {
    ...
    if (!item) return;
    closeCoverMenu();
    goToMenuTarget(item.dataset.menuNav);
  });

  // ---- Navigazione persistente (Home/Personaggi/Storie/Compendio/Cerca) ----
  // Piano UX "confronto D&D Beyond": le cinque destinazioni principali non
  // devono più vivere solo nell'hamburger — stessa tabella di instradamento
  // di sopra (goToMenuTarget), riusata qui invece di duplicarla.
  const rmNav = $('#rm-nav');
  if (rmNav) {
    rmNav.addEventListener('click', e => {
      const item = e.target.closest('.rm-nav-item');
      if (!item) return;
      goToMenuTarget(item.dataset.rmNav);
    });
  }

  // ---- accesso al profilo dall'avatar (Fase 1 — piano D&D-Beyond-like) ----
  // Punta alla vista Account esistente (stesso ingresso della voce di menu
  // "Account"), estesa in Fase 3 col profilo completo (avatar/piano/
  // changelog/preferenze/assistenza/privacy/termini/versione).
  // Il tondo "?" in copertina: apre l'Assistenza, non l'Account. Mostrava un
  // punto interrogativo fisso (nessuno ha mai valorizzato l'iniziale del
  // profilo) e portava al profilo: in qualunque app quel simbolo significa
  // "aiuto". L'Account resta la prima voce dell'indice.
  $('#btn-help').addEventListener('click', () => {
    if (typeof renderSupportView === 'function') renderSupportView();
    showView('support');
  });

  // ---- fascia firma/release: il link versione apre il changelog ----
  $('#app-signature-version').addEventListener('click', () => { renderChangelogView(); showView('changelog'); });

  // ---- ricerca globale (Fase 4, estesa in S12: predittiva/categorie/cronologia) ----
  $('#search-global-form').addEventListener('submit', e => {
    e.preventDefault();
    clearTimeout(globalSearchDebounceTimer);
    performGlobalSearch($('#search-global-input').value);
  });
  // "Suggerimenti durante la digitazione" (S12): debounce breve, non una
  // ricerca ad ogni tasto — evita una raffica di query di rete (campagne/
  // contenuti) mentre si sta ancora scrivendo.
  $('#search-global-input').addEventListener('input', e => {
    clearTimeout(globalSearchDebounceTimer);
    const value = e.target.value;
    globalSearchDebounceTimer = setTimeout(() => performGlobalSearch(value), 300);
  });
  $('#search-global-input').addEventListener('focus', e => {
    if (!e.target.value.trim()) renderRecentSearchesChips();
  });
  $('#search-recent').addEventListener('click', e => {
    const recentBtn = e.target.closest('[data-recentsearch]');
    if (recentBtn) {
      $('#search-global-input').value = recentBtn.dataset.recentsearch;
      clearTimeout(globalSearchDebounceTimer);
      performGlobalSearch(recentBtn.dataset.recentsearch);
      return;
    }
    if (e.target.closest('[data-clearrecentsearches]')) {
      clearRecentSearches();
      renderRecentSearchesChips();
      return;
    }
  });
  $('#search-global-categories').addEventListener('click', e => {
    const btn = e.target.closest('[data-searchcategory]');
    if (!btn) return;
    globalSearchActiveCategory = btn.dataset.searchcategory;
    renderGlobalSearchCategories();
    renderGlobalSearchResultsList($('#search-global-input').value.trim());
  });
  $('#search-global-results').addEventListener('click', e => {
    const row = e.target.closest('[data-searchresult]');
    if (!row) return;
    const kind = row.dataset.searchresult;
    if (kind === 'rule') {
      renderRules();
      showView('rules');
      openRulesChapter(Number(row.dataset.chapterindex));
      return;
    }
    if (kind === 'character') {
      const c = visibleCharacters().find(x => x.id === row.dataset.charid);
    ...
    if (kind === 'campaign') {
      if (typeof openCampaignSheet === 'function') openCampaignSheet(row.dataset.campaignid);
      return;
    }
    if (kind === 'content') {
      const r = lastGlobalSearchResults.find(x => x.type === 'content' && x.id === row.dataset.contentid);
      if (!r) { toast('Contenuto non trovato'); return; }
      if (typeof renderCustomContentsView === 'function') renderCustomContentsView();
      showView('customcontents');
      if (typeof openCustomContentForm === 'function') openCustomContentForm(r.item);
      return;
    }
    if (kind === 'setting') {
      if (typeof openLibrarySettingPage === 'function') openLibrarySettingPage(row.dataset.settingkey);
      return;
    }
  });
  ...
  $('#char-list').addEventListener('click', e => {
    ...
    if (cc && !cc.creationCompleted) openCreationWizard(cc.id, true);
  ...
  // Le card classe (.build-card) sono div con role="button"/tabindex, non
  // <button> reali (i pulsanti di swap Eclettico dentro, sì, e funzionano
  // già da tastiera come qualunque <button>): senza questo handler la
  // scelta della classe — passo obbligatorio della creazione — restava
  // raggiungibile solo col mouse/touch (checkpoint accessibilità, Fase 2).
  // click() sintetico: riusa esattamente la stessa logica del listener
  // 'click' qui sotto, nessuna duplicazione.
  ...
  $('#trait-roll-btn').addEventListener('click', () => {
    ...
    if (raw === '__unknown__') {
      ...
      const extreme100 = rollExtremeClass(d100, 100);
      if (extreme100) resultEl.classList.add(extreme100);
    ...
    const extreme20 = rollExtremeClass(d20, 20);
    if (extreme20) resultEl.classList.add(extreme20);
  });
  ...
  $('#stat-roll-btn').addEventListener('click', () => {
    ...
    resultEl.className = 'roll-result';
    if (label === 'd12+d8') {
      // Somma di due dadi: "massimo/minimo" non ha un significato singolo
      // e univoco, nessuna evidenziazione (non si inventa un criterio).
      ...
      const extremeStat = rollExtremeClass(roll, sides);
      if (extremeStat) resultEl.classList.add(extremeStat);
    }
  });
  ...
  $('#portrait-remove').addEventListener('click', () => {
    const c = getActive(); if (!c) return;
    c.portrait = null;
    c.portraitDirty = true; // rimozione da portare nel cloud (clear_character_portrait)
    c.portraitPos = null;
    renderPortrait(c);
    renderHeader(c);
    touchActive();
...
function wireTecAbExtra(sel, field) {
  $(sel).addEventListener('change', e => {
    ...
    // Effetti di stato AGGIUNTIVI (r.effettiExtra[j]): stessi 4 casi sopra,
    // stessa identica logica, solo indirizzati a un elemento dell'array
    // invece che ai campi effettoNome/effettoTratto della riga — vedi
    // effettoBlockHtml/ensureDannoAttivaFields.
    const effettoExtraPreset = e.target.closest('[data-effettoextrapreset]');
    if (effettoExtraPreset) {
      const [, iStr, jStr] = effettoExtraPreset.dataset.effettoextrapreset.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      const ee = row.effettiExtra && row.effettiExtra[j]; if (!ee) return;
      if (effettoExtraPreset.value === '__custom__') {
        const customInput = effettoExtraPreset.parentElement.querySelector('[data-effettoextranome]');
        if (customInput) { customInput.classList.remove('hidden'); customInput.focus(); }
        return;
      }
      ee.nome = effettoExtraPreset.value;
      if (!ee.nome) ee.tratto = '';
      (field === 'tecniche' ? renderTecniche : renderAbilita)(c);
      touchActive();
      return;
    }
    const effettoExtraNome = e.target.closest('[data-effettoextranome]');
    if (effettoExtraNome) {
      const [, iStr, jStr] = effettoExtraNome.dataset.effettoextranome.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      const ee = row.effettiExtra && row.effettiExtra[j]; if (!ee) return;
      ee.nome = effettoExtraNome.value.trim();
      if (!ee.nome) ee.tratto = '';
      (field === 'tecniche' ? renderTecniche : renderAbilita)(c);
      touchActive();
      return;
    }
    const effettoExtraTrattoSel = e.target.closest('[data-effettoextratrattosel]');
    if (effettoExtraTrattoSel) {
      const [, iStr, jStr] = effettoExtraTrattoSel.dataset.effettoextratrattosel.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      const ee = row.effettiExtra && row.effettiExtra[j]; if (!ee) return;
      if (effettoExtraTrattoSel.value === '__custom__') {
        const customInput = effettoExtraTrattoSel.parentElement.querySelector('[data-effettoextratrattocustom]');
        if (customInput) { customInput.classList.remove('hidden'); customInput.focus(); }
        return;
      }
      ee.tratto = effettoExtraTrattoSel.value;
      (field === 'tecniche' ? renderTecniche : renderAbilita)(c);
      touchActive();
      return;
    }
    const effettoExtraTrattoCustom = e.target.closest('[data-effettoextratrattocustom]');
    if (effettoExtraTrattoCustom) {
      const [, iStr, jStr] = effettoExtraTrattoCustom.dataset.effettoextratrattocustom.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      const ee = row.effettiExtra && row.effettiExtra[j]; if (!ee) return;
      ee.tratto = effettoExtraTrattoCustom.value.trim();
      touchActive();
    }
  });
  $(sel).addEventListener('click', e => {
    const c = getActive(); if (!c) return;
    const add = e.target.closest('[data-addeffettoextra]');
    if (add) {
      const i = Number(add.dataset.addeffettoextra.split('::')[1]);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      if (!Array.isArray(row.effettiExtra)) row.effettiExtra = [];
      if (row.effettiExtra.length >= 4) { toast('Massimo 4 effetti aggiuntivi per riga'); return; }
      row.effettiExtra.push({ nome: '', tratto: '', bonusPct: 0 });
      (field === 'tecniche' ? renderTecniche : renderAbilita)(c);
      touchActive();
      return;
    }
    const del = e.target.closest('[data-deleffettoextra]');
    if (del) {
      const [, iStr, jStr] = del.dataset.deleffettoextra.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      if (row.effettiExtra) row.effettiExtra.splice(j, 1);
      (field === 'tecniche' ? renderTecniche : renderAbilita)(c);
      touchActive();
    }
  });
  $(sel).addEventListener('input', e => {
    ...
    // Bonus % al tiro di stato, per un effetto AGGIUNTIVO: stesso campo di
    // data-effettobonuspct sopra, indirizzato a row.effettiExtra[j].
    const extraBonusPct = e.target.closest('[data-effettoextrabonuspct]');
    if (extraBonusPct && field === 'abilita') {
      const c = getActive(); if (!c) return;
      const [, iStr, jStr] = extraBonusPct.dataset.effettoextrabonuspct.split('::');
      const i = Number(iStr), j = Number(jStr);
      const row = c[field][i];
      if (tecAbFieldRowLocked(field, row, i)) return;
      const ee = row.effettiExtra && row.effettiExtra[j]; if (!ee) return;
      ee.bonusPct = Math.floor(Number(extraBonusPct.value)) || 0;
      touchActive();
      return;
    }
...
let updateUrl = null;
let updateApkSize = null;
let updateApkVersion = null;
let updateOtaErrorDetail = null;
...
/* Fase 5 del piano di pubblicazione store: nelle build Play/App Store
   l'updater OTA è vietato dalle policy (self-update fuori dal meccanismo
   ufficiale dello store) — restituendo null qui, ogni chiamante che già
   controlla "if (up && ...)" (notifyAppReady sotto, checkForUpdate più in
   basso) si disattiva da solo, senza bisogno di toccare quei punti uno per
   uno. Resta attivo SOLO nel canale sideload/PWA (js/build-channel.js). */
function otaPlugin() {
  if (typeof isStoreChannel === 'function' && isStoreChannel()) return null;
...
/* Rete degradata/assente verso l'API GitHub: senza un tetto massimo,
   fetch() può restare in sospeso per minuti su una connessione che non
   risponde né fallisce subito, lasciando la UI in attesa senza alcun
   segnale — getStoriesIndex() ha già un fallback sulla cache locale
   (vedi sotto), ma solo se questa funzione torna a restituire il
   controllo entro un tempo ragionevole. La soglia è quella unica di
   withTimeout (CLOUD_TIMEOUT_MS in js/cloud-account.js): una seconda
   costante qui sarebbe solo un numero che sembra governare il timeout
   senza governarlo. */
async function fetchStoriesIndexRemote() {
  try {
    const res = await withTimeout(
      fetch(`${GH_API}/contents/stories/index.json?ref=${GH_BRANCH}`, {
...
/* Prima di questo fix: un fetch che LANCIA (rete assente, non solo un
   4xx/5xx — es. offline reale) non veniva mai intercettato qui, e il
   chiamante (wireStaticEvents, #prem-popup-open-online) non ha un
   try/catch attorno all'await: il pulsante restava bloccato su
   "Scaricamento…" per sempre, nessun errore mostrato. Stesso pattern di
   fetchStoriesIndexRemote sopra: try/catch + timeout, null in ogni caso di
   fallimento — il chiamante già gestisce bytes===null con un toast. */
async function fetchStoryPdfBytes(id) {
  try {
    const res = await withTimeout(
      fetch(`${GH_API}/contents/stories/${id}.pdf?ref=${GH_BRANCH}`, {
...
/* Pura: dato l'elenco delle release GitHub e la versione installata,
   sceglie le DUE migliori release rilevanti — mai una sola "release
   assoluta migliore", per costruzione dello stesso bug reale che questa
   funzione ripara (2026-09-13): la release "solo APK" v1.0.288 è stata
   pubblicata mentre il build automatico OTA di v1.0.287 (stesso codice
   web, nessuna modifica nativa) veniva annullato dal gruppo di
   concorrenza dello stesso branch (due run sullo stesso ref, il più
   recente cancella quello in corso) — quella correzione web è rimasta
   raggiungibile solo installando 288 a mano, mai via OTA, perché la
   versione precedente di questa funzione considerava solo l'ultima
   release in assoluto e tentava l'OTA (o il banner) solo su quella: se
   la più recente non ha bundle.zip, un aggiornamento OTA più vecchio ma
   ancora valido restava invisibile. Le due classifiche restano
   indipendenti apposta: bestOta è la release-con-zip più recente più
   nuova dell'installata (comunque raggiungibile anche se un'altra
   release nativa, senza zip, è più recente ancora); bestApk è la
   release-con-apk più recente più nuova dell'installata, a prescindere
   da bestOta — dopo un'eventuale applicazione OTA l'app si ricarica e
   questa stessa funzione rivaluta tutto da capo sulla nuova versione. */
function pickBestUpdateReleases(rels, installedVersion) {
  let bestOta = null, bestOtaVer = null;
  let bestApk = null, bestApkVer = null;
  (Array.isArray(rels) ? rels : []).forEach(rel => {
    if (rel.draft || rel.prerelease) return;
    const m = /^apk-v(\d+(?:\.\d+)*)$/.exec(rel.tag_name || '');
    if (!m) return;
    const version = m[1];
    if (cmpVersions(version, installedVersion) <= 0) return;
    const assets = rel.assets || [];
    if (assets.some(a => a.name === 'bundle.zip') && (bestOtaVer === null || cmpVersions(version, bestOtaVer) > 0)) {
      bestOta = rel; bestOtaVer = version;
    }
    if (bestApkVer === null || cmpVersions(version, bestApkVer) > 0) {
      bestApk = rel; bestApkVer = version;
    }
  });
  return { bestOta, bestOtaVer, bestApk, bestApkVer };
}

/* Solo nell'app nativa: confronta la versione installata (APP_VERSION,
   scritta dalla build) con le release Android su GitHub (vedi
   pickBestUpdateReleases sopra per come vengono scelte).
   Se esiste un aggiornamento OTA applicabile lo tenta in background
   (scarica bundle.zip nella memoria interna e lo applica subito, senza
   download visibili né installazioni); se non ce n'è uno applicabile (o
   il tentativo fallisce) e la release più recente in assoluto richiede
   comunque l'APK, mostra il banner con il download manuale/in-app.

   Nelle build destinate a uno store questa funzione non deve fare NULLA:
   non solo l'updater OTA (già spento da otaPlugin()), ma anche il ripiego
   "banner con download manuale dell'APK" e la richiesta stessa all'API
   Releases di GitHub. Le policy degli store vietano qualunque canale di
   aggiornamento alternativo, e in quel canale sarebbe comunque inutile. */
async function checkForUpdate() {
  // La schermata iniziale possiede il controllo: nessun download automatico
  // concorrente che aggiri il pulsante o si applichi dopo il video.
  if (window.RMStartup && window.RMStartup.isChecking()) return;
  if (navigator.onLine === false) return;
  if (!isNativeApp() || typeof APP_VERSION === 'undefined' || !APP_VERSION) return;
  if (typeof isStoreChannel === 'function' && isStoreChannel()) return;
  const repo = await resolveReleasesRepo();
  withTimeout(fetch(`https://api.github.com/repos/${repo}/releases?per_page=20`), 'Controllo aggiornamenti')
    .then(r => r.json())
    .then(async rels => {
      const { bestOta, bestOtaVer, bestApk, bestApkVer } = pickBestUpdateReleases(rels, APP_VERSION);

      // Motivo del fallimento OTA (se capita), mostrato più sotto nel
      // banner invece di restare solo in console: un errore muto qui
      // aveva già nascosto un problema reale di aggiornamento (2026-09-20,
      // segnalato dall'utente — il fallback al banner/browser avveniva
      // sempre, senza mai un indizio del perché l'OTA silenziosa fallisse).
      let otaErrorDetail = null;

      // 1) tentativo OTA silenzioso, sulla release OTA più recente
      // applicabile — mai scavalcata da una release nativa più recente
      // ancora sconosciuta a questo ramo (vedi commento sopra).
      if (bestOta) {
        const up = otaPlugin();
        const zip = bestOta.assets.find(a => a.name === 'bundle.zip');
        if (up && zip) {
          try {
            toast(`Aggiornamento alla v${bestOtaVer} in corso…`);
            const bundle = await up.download({ url: zip.browser_download_url, version: bestOtaVer });
            ...
            otaErrorDetail = (e && (e.message || e.errorMessage)) || String(e);
          }
        }
      }

      // 2) ripiego/nativo: banner solo se esiste davvero una release più
      // recente dell'installata che richiede l'APK (indipendente dal
      // risultato del tentativo OTA sopra, che se riuscito ha già fatto
      // return: qui APP_VERSION è ancora quella realmente in esecuzione).
      if (!bestApk) return;
      const apk = bestApk.assets.find(a => a.name && a.name.endsWith('.apk'));
      updateUrl = apk ? apk.browser_download_url : bestApk.html_url;
      updateOtaErrorDetail = otaErrorDetail;
      updateApkSize = apk ? apk.size : null;
      updateApkVersion = bestApkVer;
      $('#update-banner-text').textContent = otaErrorDetail
        ? `Nuova versione disponibile (v${bestApkVer}) — aggiornamento automatico non riuscito: ${otaErrorDetail}`
        : `Nuova versione disponibile (v${bestApkVer})`;
...
/* Avvio nativo: la schermata iniziale resta aperta fino alla scelta e
   all'applicazione dell'update. Gli aggiornamenti in background, quando
   l'app è già aperta, conservano checkForUpdate()/afterIntroFinished(). */
async function prepareStartupUpdate(ui) {
  if (typeof APP_VERSION === 'undefined' || !APP_VERSION) return;
  if (typeof isStoreChannel === 'function' && isStoreChannel()) return;
  if (navigator.onLine === false) return;
  ui.status('Controllo aggiornamenti…');
  const controller = new AbortController();
  let timer;
  let releases;
  try {
    // Limite dell'intero controllo (configurazione + API + JSON), mai
    // usato per decidere se un fotogramma del video sia pronto.
    releases = await Promise.race([
      (async () => {
        const repo = await resolveReleasesRepo();
        if (controller.signal.aborted) throw new Error('Controllo scaduto');
        const res = await fetch(`https://api.github.com/repos/${repo}/releases?per_page=20`, { signal: controller.signal });
        if (!res.ok) throw new Error('Controllo aggiornamenti HTTP ' + res.status);
        const data = await res.json();
        if (!Array.isArray(data)) throw new Error('Elenco aggiornamenti non valido');
        return data;
      })(),
      new Promise((resolve, reject) => {
        timer = setTimeout(() => { controller.abort(); reject(new Error('Controllo aggiornamenti scaduto')); }, 12000);
      })
    ]);
  } catch (e) {
    console.info('[startup] utilizzo la versione installata:', e.message);
    return;
  } finally { clearTimeout(timer); }
  const { bestOta, bestOtaVer, bestApk, bestApkVer } = pickBestUpdateReleases(releases, APP_VERSION);
  const updater = otaPlugin();
  const zip = bestOta && bestOta.assets.find(a => a.name === 'bundle.zip');
  if (updater && zip) {
    ui.status(`Nuova versione disponibile: ${bestOtaVer}`);
    let buttonText = 'Scarica l’aggiornamento';
    for (;;) {
      await ui.action(buttonText);
      ui.status('Download aggiornamento…');
      let listener;
      try {
        // Evento documentato da capacitor-updater 8.51.16: percent 0..100.
        // Se il vecchio plugin non lo espone, resta l'indicatore di attività.
        // addListener() può non restituire una vera Promise (handle
        // sincrono senza .then/.catch, a seconda di piattaforma/versione
        // del plugin) o lanciare un'eccezione sincrona: un fallimento qui
        // deve compromettere SOLO la percentuale di avanzamento, mai
        // l'intero download (bug reale 2026-09-22: ".catch is not a
        // function" bloccava il download stesso, non solo la barra —
        // schermata di avvio bloccata su "Riprova il download" senza
        // alcun percorso di uscita, perché checkForUpdate() resta escluso
        // finché questa funzione non ritorna, vedi RMStartup.isChecking()).
        // Promise.resolve(...) normalizza entrambi i casi: se addListener
        // restituisce già una Promise la incatena, se restituisce un
        // valore sincrono lo avvolge senza bisogno che quel valore abbia
        // un proprio .catch.
        if (typeof updater.addListener === 'function') {
          try {
            listener = await Promise.resolve(updater.addListener('download', event => {
              if (event && Number.isFinite(event.percent)) ui.progress(event.percent / 100);
            }));
          } catch (e) { listener = null; }
        }
        const bundle = await updater.download({ url: zip.browser_download_url, version: bestOtaVer });
        if (listener && listener.remove) await listener.remove().catch(() => {});
        listener = null;
        ui.status('Aggiornamento scaricato. Riavvio…');
        // L'intro non è ancora partita: il reload può avvenire ora.
        // I flag di sessione dell'intro non sono stati scritti; dopo il
        // reload la nuova versione esegue l'intro una sola volta.
        await updater.set(bundle);
        // set() ricarica la WebView: non avviare il vecchio video nel
        // breve intervallo tra la risoluzione del bridge e il reload.
        await new Promise(() => {});
      } catch (e) {
        console.error('[startup] aggiornamento non riuscito', e);
        const detail = (e && (e.message || e.errorMessage)) || String(e);
        ui.status(`Aggiornamento non riuscito: ${detail}. Controlla la connessione e riprova.`);
        buttonText = 'Riprova il download';
      } finally {
        if (listener && listener.remove) await listener.remove().catch(() => {});
      }
    }
  ...
  if (!bestApk) return;
  const apk = bestApk.assets.find(a => /\.apk$/i.test(a.name));
  if (!apk) return;
  ui.status(`Nuova versione disponibile: ${bestApkVer}`);
  let buttonText = 'Scarica l’aggiornamento';
  for (;;) {
    await ui.action(buttonText);
    ui.status('Download aggiornamento…');
    ...
    const opener = fileOpenerPlugin();
    try {
      if (!isNativeAndroidPlatform() || !fs || !opener) {
        redirectToApkDownload(apk.browser_download_url);
        ui.status('Completa il download e installa l’aggiornamento, poi riapri l’app.');
        buttonText = 'Scarica nuovamente';
        continue;
      }
      const fileName = `role-makers-update-${bestApkVer}.apk`;
      listener = await fs.addListener('progress', event => {
        if (event && event.contentLength > 0) ui.progress(event.bytes / event.contentLength);
      });
      const result = await fs.downloadFile({ url: apk.browser_download_url, path: fileName, directory: 'CACHE', progress: true });
      const filePath = (result && result.path) || (await fs.getUri({ path: fileName, directory: 'CACHE' })).uri;
      ui.status('Download completato. Conferma l’installazione di Android.');
      await opener.open({ filePath, contentType: 'application/vnd.android.package-archive' });
      // L'installer può essere annullato: l'utente può riaprirlo senza
      // scaricare nuovamente decine di MB né avviare l'intro vecchia.
      if (listener && listener.remove) await listener.remove();
      listener = null;
      for (;;) {
        await ui.action('Apri l’installazione');
        await opener.open({ filePath, contentType: 'application/vnd.android.package-archive' });
      }
    } catch (e) {
      console.error('[startup] download APK non riuscito', e);
      ui.status('Download non riuscito. Controlla la connessione e riprova.');
      buttonText = 'Riprova il download';
    } finally {
      if (listener && listener.remove) await listener.remove().catch(() => {});
    }
...
/* -------------------------------------- aggiornamento nativo in-app */
/* Richiesta esplicita dell'utente (2026-09-13): mai far uscire l'app per
   scaricare l'aggiornamento, mostrare il caricamento e il completamento
   restando sulla stessa schermata. Tre plugin nativi nuovi, presenti
   solo a partire dall'APK di questa funzione:
   - @capacitor/filesystem: downloadFile() nativo con evento 'progress'
     (bytes/contentLength) e scrittura diretta su disco — mai un fetch()
     della WebView: verificato che l'asset delle release NON invia alcuna
     intestazione Access-Control-Allow-Origin (a differenza della sola
     API JSON usata da checkForUpdate sopra, che la invia), quindi un
     fetch() normale non potrebbe nemmeno leggerne il corpo. downloadFile
     è documentata come deprecata a favore di @capacitor/file-transfer,
     ma resta pienamente funzionante: introdurre un quarto plugin per
     una funzione già coperta da uno dei tre necessari non sembrava
     giustificato.
   - @capacitor-community/file-opener: apre il file scaricato con
     contentType "application/vnd.android.package-archive" — su Android
     questo avvia da solo l'installer di sistema (il tocco finale
     "Installa" resta comunque suo, nessuna app di terze parti può
     evitarlo: è una richiesta di sicurezza del sistema operativo).
   - il permesso REQUEST_INSTALL_PACKAGES (patch_android_install_
     permission.py): senza, l'installer di sistema rifiuta il pacchetto
     anche se il file/intent sono corretti.
   Qualunque fallimento in qualunque punto (plugin assente su un APK più
   vecchio di questa funzione, rete, scrittura, permesso "installa app
   sconosciute" non ancora concesso dall'utente) ripiega SEMPRE sul
   download nel browser di sistema — mai un banner che resta bloccato a
   metà, mai un percorso nuovo senza quello già affidabile di sempre
   dietro come rete di sicurezza. */
...
function nativeUpdatePluginsAvailable() {
  return !!(nativeFilesystemPlugin() && fileOpenerPlugin());
}

// Isolata per i test (stesso motivo di ogni altro punto del codice in cui
// serve stubbare un side-effect non simulabile in un browser di test: qui
// una navigazione reale interromperebbe il test stesso).
function redirectToApkDownload(url) { location.href = url; }

const UPD_ARC_CIRCUMFERENCE = 2 * Math.PI * 40; // stesso raggio (40) del <circle r="40"> in index.html

/* Pura: geometria dell'animazione per una frazione 0..1 — separata dal
   DOM e dai plugin nativi per essere verificabile da sola. I due punti
   (gli stessi due dadi acqua/fuoco del marchio) partono ai lati opposti
   del cerchio e convergono in alto, sovrapponendosi a fraction=1; l'arco
   si riempie in parallelo. La transizione CSS (.upd-arc/.upd-dot-group,
   css/style.css) fa il resto: ad ogni chiamata si imposta solo il
   bersaglio, mai un ciclo ad ogni frame lato JS. */
function computeUpdateProgressVisual(fraction) {
  const f = Math.max(0, Math.min(1, fraction));
  return {
    dotADeg: -180 + 180 * f,
    dotBDeg: 180 - 180 * f,
    dashOffset: UPD_ARC_CIRCUMFERENCE * (1 - f),
    percent: Math.round(f * 100)
  };
}

function renderUpdateProgress(fraction, label) {
  const v = computeUpdateProgressVisual(fraction);
  const dotA = $('#upd-dot-a'), dotB = $('#upd-dot-b'), arc = $('#upd-arc'), text = $('#update-progress-text');
  if (dotA) dotA.style.transform = `rotate(${v.dotADeg}deg)`;
  if (dotB) dotB.style.transform = `rotate(${v.dotBDeg}deg)`;
  if (arc) arc.style.strokeDashoffset = String(v.dashOffset);
  if (text) text.textContent = label || `Download in corso… ${v.percent}%`;
}

function showUpdateProgressUi() {
  const btn = $('#update-banner-btn'), text = $('#update-banner-text'), progress = $('#update-progress');
  if (btn) btn.classList.add('hidden');
  if (text) text.classList.add('hidden');
  if (progress) progress.classList.remove('hidden');
  renderUpdateProgress(0);
}
function hideUpdateProgressUi() {
  const btn = $('#update-banner-btn'), text = $('#update-banner-text'), progress = $('#update-progress');
...
/* Richiesta esplicita dell'utente (2026-09-13): quando un aggiornamento
   c'è, il banner prende il posto del modulo accesso/registrazione
   ("il menù"), non gli si affianca — il modulo di accesso non serve a
   chi deve comunque aggiornare prima di continuare, e uno schermo
   dedicato (marchio sopra, sempre presente in .cover-header, più questo
   banner ingrandito da solo qui sotto) rende l'aggiornamento la sola
   cosa da fare, come nelle app che citava come esempio. Nessun modo di
   saltarlo mostrato qui: non richiesto, e un aggiornamento reale (nuove
   colonne/RPC lato server, come da CLAUDE.md) può rendere l'app vecchia
   davvero non funzionante, non solo "in ritardo". */
function showNativeUpdateBanner() {
  const status = $('#home-status-box');
  if (status) status.classList.add('hidden');
  const banner = $('#update-banner');
  if (banner) { banner.classList.remove('hidden'); banner.classList.add('update-banner-takeover'); }
}

/* Scarica l'APK restando sulla copertina (mai un cambio di app), mostra
   l'avanzamento reale con l'animazione dei due punti, poi consegna il
   file all'installer di sistema. version è usata solo per il nome del
   file su disco (mai due download che si sovrascrivono a metà se per
   assurdo ne partissero due); sizeBytes è opzionale (solo per un'eventuale
   nota informativa, il calcolo dell'avanzamento usa sempre il valore
   reale restituito dall'evento 'progress', mai quello dichiarato qui). */
async function downloadAndInstallNativeUpdate(apkUrl, version, sizeBytes) {
  const fs = nativeFilesystemPlugin();
  const opener = fileOpenerPlugin();
  if (!isNativeAndroidPlatform() || !fs || !opener) {
    redirectToApkDownload(apkUrl);
    return;
  }
  const fileName = `role-makers-update-${version || 'latest'}.apk`;
  let listenerHandle = null;
  showUpdateProgressUi();
  try {
    listenerHandle = await fs.addListener('progress', status => {
      if (status && status.contentLength > 0) renderUpdateProgress(status.bytes / status.contentLength);
    });
    const result = await fs.downloadFile({ url: apkUrl, path: fileName, directory: 'CACHE', progress: true });
    const filePath = (result && result.path) || (await fs.getUri({ path: fileName, directory: 'CACHE' })).uri;
    renderUpdateProgress(1, 'Completato — apro l\'installazione…');
    await opener.open({ filePath, contentType: 'application/vnd.android.package-archive' });
    hideUpdateProgressUi();
  } catch (e) {
    console.error('Aggiornamento in-app non riuscito, ripiego sul browser', e);
    toast('Download nell\'app non riuscito: continuo nel browser di sistema');
    hideUpdateProgressUi();
    $('#update-banner-chrome-help').classList.remove('hidden');
    redirectToApkDownload(apkUrl);
  } finally {
    if (listenerHandle && listenerHandle.remove) { try { await listenerHandle.remove(); } catch (e) {} }
  }
}
