...
function goToMenuTarget(target) {
  ...
  if (target === 'previously') { renderPreviouslyOnView(); showView('previously'); return; }
  ...
  if (target === 'charlist') { renderCharList(); showView('list'); return; }
...
async function checkForUpdate() {
  ...
  withTimeout(fetch(`https://api.github.com/repos/${repo}/releases?per_page=20`), 'Controllo aggiornamenti')
    ...
    .then(async rels => {
      ...
      $('#update-banner-text').textContent = `Nuova versione disponibile (v${bestApkVer})`;
