...
let viewStack = ['cover'];
// Vero una sola volta per avvio: la prima volta che si scopre una sessione
// reale (non ospite) ancora ferma sulla copertina iniziale, si passa alla
// Home (S1) — mai più dopo, per non strappare l'utente da dove si trova
// per un evento di sessione successivo (refresh token, ecc. — vedi
// renderHomeIdentityBox in js/cloud-account.js).
let homeAutoRedirectDone = false;
...
/* Viste dove la navigazione persistente (#rm-nav, piano UX "confronto D&D
   Beyond") NON compare: copertina (resta intro/brand, non dashboard),
   scheda e combattimento (schermate a piena immersione, dove sottrarre
   spazio verticale fisso costerebbe più di quanto una barra sempre uguale
   offrirebbe) e il wizard di creazione (ha già i propri passi/pulsanti). */
const RM_NAV_HIDDEN_VIEWS = new Set(['cover', 'sheet', 'combat', 'create', 'charview', 'onboarding']);
function updateRmNav(name) {
  ...
  const activeMap = { home: 'home', list: 'charlist', campaigns: 'storie', account: 'storie', rules: 'rules', search: 'search' };
