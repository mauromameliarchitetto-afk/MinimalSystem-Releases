...
/* Un messaggio grezzo può venire mostrato all'utente COSÌ COM'È solo se la
   ...
   3) un Error costruito da questo stesso progetto con userMessage(): il
      marcatore non nasce mai da un errore intercettato (una libreria non
      lo imposta), va scritto a mano nel punto in cui il messaggio viene
      formulato per il giocatore. Serviva davvero: senza, ogni `throw new
      Error('testo italiano per l'utente')` del client finiva nel
      fallback generico "errore imprevisto" — verificato su dispositivo
      (2026-09-12) con "Attiva ora" delle notifiche, dove i tre motivi
      distinti di fallimento di enablePushNotifications() arrivavano
      all'utente tutti come lo stesso messaggio senza contenuto. */
function isSafeApplicationMessage(err, raw) {
  if (err && err.code === 'P0001') return true;
  if (err && err.isUserMessage === true) return true;
  if (/: nessuna risposta dal server, riprova$/.test(raw)) return true;
  return false;
}
/* Error il cui messaggio è scritto per essere letto dal giocatore così
   com'è (vedi punto 3 sopra), mai un dettaglio tecnico riformulato. */
...
/* Lato lettura della stessa Credential Management API (sopra è il lato
   scrittura): all'apertura del modulo di accesso prova a recuperare una
   credenziale già salvata da un accesso precedente — legata all'account
   Google/Chrome del dispositivo, non ai dati locali dell'app, quindi
   sopravvive a una disinstallazione/reinstallazione. Se il sistema la
   restituisce (in silenzio, o dopo che l'utente ha scelto dal proprio
   selettore nativo — mai un'azione che l'utente non abbia già compiuto)
   i campi si riempiono da soli e si prova subito l'accesso: se riesce
   l'utente non deve più nemmeno premere "Accedi", se fallisce (password
   cambiata altrove) i campi restano comunque pre-compilati. "Best
   effort" come maybeStoreCredential: mai un blocco se l'API manca o
   l'utente non ha nulla di salvato.

   La richiesta al sistema (get(), può mostrare un selettore nativo) parte
   una sola volta per apertura dell'app — fetchSavedCredentialOnce()
   memorizza la stessa Promise per ogni chiamata successiva, mai una
   seconda finestra di selezione. Il RIEMPIMENTO dei campi invece va
   riapplicato a ogni ridisegno del modulo, non solo al primo: sb.auth.
   onAuthStateChange (più sotto) richiama renderHomeIdentityBox() anche
   per il suo evento iniziale (nessuna sessione), che ridisegna il modulo
   con campi vuoti subito DOPO il primo riempimento — un guard "un solo
   tentativo in assoluto" (bug reale trovato scrivendo il test:
   tests/e2e/credential-autofill.spec.js) lo cancellava senza che nulla
   lo riproponesse. Il TENTATIVO DI ACCESSO automatico invece resta unico
   (autoSignInAttempted): niente invii ripetuti ad ogni ridisegno. */
let savedCredentialPromise = null;
let autoSignInAttempted = false;
function fetchSavedCredentialOnce() {
  if (!savedCredentialPromise) {
    savedCredentialPromise = (async () => {
      if (typeof PasswordCredential === 'undefined' || !navigator.credentials || !navigator.credentials.get) return null;
      try {
        const cred = await navigator.credentials.get({ password: true, mediation: 'optional' });
        if (cred && cred.type === 'password' && cred.id && cred.password) return cred;
      } catch (e) { /* nessun problema: nessuna credenziale da riproporre */ }
      return null;
    })();
  }
  return savedCredentialPromise;
}
async function tryAutofillSavedCredential() {
  const cred = await fetchSavedCredentialOnce();
  if (!cred) return;
  const emailField = $('#acc-email');
  const passwordField = $('#acc-password');
  if (!emailField || !passwordField) return;
  // get() può restare in sospeso per il tempo di un tocco sul selettore
  // nativo, e questa funzione viene richiamata ad ogni ridisegno: nel
  // frattempo l'utente potrebbe aver già iniziato a scrivere di suo, un
  // campo toccato non va mai sovrascritto.
  ...
  try {
    await signInWithPassword(cred.id, cred.password);
    toast('Accesso automatico effettuato');
    renderHomeIdentityBox();
  } catch (e) { /* credenziale non più valida: i campi restano pre-compilati, basta premere Accedi */ }
}
...
/* Audit Play Store, punto 5 — due gate distinti per Google:
   - googleProviderAvailable: il provider è attivo su Supabase → gli utenti
     GIÀ entrati con e-mail e password possono collegarlo ("Collega account
     Google", linkIdentity sulla loro sessione);
   - googlePublicSignInEnabled: in più, il pulsante pubblico "Accedi con
     Google" in copertina.
   Durante la migrazione il provider va acceso per il solo collegamento:
   se bastasse il provider a mostrare anche l'accesso pubblico, un utente
   legacy potrebbe usarlo prima di collegarsi e ritrovarsi con un secondo
   account (e-mail Google diversa). L'accesso pubblico resta quindi spento
   qui finché la migrazione non è verificata: accenderlo è una modifica a
   sé (vedi docs/play-readiness/AUTH_DEEP_LINKS.md). */
const GOOGLE_PUBLIC_SIGNIN_ENABLED = false;
function isGooglePublicSignInEnabled() { return GOOGLE_PUBLIC_SIGNIN_ENABLED; }
...
async function getAuthCapabilities() {
  ...
  try {
    ...
    const googleProvider = !!(json.external && json.external.google);
    authCapabilitiesCache = {
      anonymous: !!(json.external && json.external.anonymous_users),
      googleProviderAvailable: googleProvider,
      googlePublicSignInEnabled: googleProvider && isGooglePublicSignInEnabled(),
      apple: !!(json.external && json.external.apple),
      passkey: !!json.passkeys_enabled
    };
  } catch (e) {
    authCapabilitiesCache = { anonymous: false, googleProviderAvailable: false, googlePublicSignInEnabled: false, apple: false, passkey: false };
  }
...
/* Login social (Google): Google blocca l'OAuth dentro una WebView embedded
   (policy "no embedded user-agent"): sull'app nativa l'URL di accesso si
   apre nel browser di sistema (Chrome Custom Tabs, @capacitor/browser),
   mai nella WebView dell'app. Audit Play Store, punto 5: il flusso è ora
   PKCE (client dedicato, vedi startOAuthSignIn/getOAuthPkceClient in
   js/supabase-client.js) — il ritorno porta solo un codice monouso,
   scambiabile solo con il code_verifier rimasto su questo dispositivo; lo
   gestisce handleAuthCallbackUrl (schema rolemakers:// o App Link HTTPS,
   avvio a freddo, annullamento, duplicati, URL non ammessi). Il pulsante
   pubblico compare solo se il provider è attivo E il gate
   GOOGLE_PUBLIC_SIGNIN_ENABLED è acceso (getAuthCapabilities): durante la
   migrazione il provider serve solo al collegamento. */
async function signInWithProvider(provider) {
  // Difesa in profondità: anche con un pulsante rimasto nel DOM o una
  // chiamata diretta, nessun accesso pubblico Google finché il gate è spento.
  if (provider === 'google' && !isGooglePublicSignInEnabled()) {
    const e = new Error('L\'accesso con Google non è ancora disponibile: entra con e-mail e password e collega Google dalla pagina Account.');
    e.code = 'OAUTH_PUBLIC_SIGNIN_DISABLED';
    throw e;
  }
  await startOAuthSignIn(provider);
}

/* Migrazione graduale verso Google (audit Play Store, punto 5): chi ha
   già un account email/password lo collega a Google da qui, restando lo
   stesso account (stessi personaggi e storie) — mai un secondo account.
   Compare solo se Google è davvero attivo sul progetto; l'accesso con
   e-mail e password non viene mai rimosso. */
function hasGoogleIdentity(user) {
  return !!(user && Array.isArray(user.identities) && user.identities.some(i => i && i.provider === 'google'));
}
function linkedIdentitiesHtml(user, caps) {
  if (!caps || !caps.googleProviderAvailable || !user) return '';
  if (hasGoogleIdentity(user)) {
    return '<p class="helper-text" style="margin:0;">Account Google collegato: puoi accedere anche con Google. L\'accesso con e-mail e password resta valido.</p>';
  }
  return '<button class="btn btn-ghost btn-sm" id="acc-link-google" style="align-self:flex-start;">Collega account Google</button>'
    + '<p class="helper-text" style="margin:0;">Collega Google a questo account per accedere anche con Google, senza creare un secondo account. L\'accesso con e-mail e password resta valido.</p>';
}
async function renderLinkedIdentities(session) {
  const box = $('#acc-linked-identities');
  if (!box || !session || isGuestUser(session)) return;
  try {
    const caps = await getAuthCapabilities();
    box.innerHTML = linkedIdentitiesHtml(session.user, caps);
  } catch (e) { /* Google non verificabile: niente pulsante, l'account funziona comunque */ }
}
...
async function signOutCloud() {
  // Audit Play Store, punto 4: revoca del token push di questo dispositivo
  // PRIMA del logout, finché il JWT è ancora valido (dopo verrebbe
  // respinta e il telefono continuerebbe a ricevere le notifiche
  // dell'account uscito). Best-effort: non blocca mai l'uscita.
  if (typeof revokePushTokenBeforeSignOut === 'function') await revokePushTokenBeforeSignOut();
  await withTimeout(sb.auth.signOut(), 'Uscita');
  // Un'uscita esplicita può significare "voglio entrare con un altro
  // account": riarma sia la richiesta al sistema sia il tentativo di
  // accesso automatico, così un'altra credenziale salvata può essere
  // riproposta invece di restare bloccati sulla prima.
  savedCredentialPromise = null;
...
async function createCampaign(name, icon) {
  ...
  // Il chiamante mostra già il modale (ensureUgcTermsAccepted): qui il gate
  // copre anche i percorsi che non lo fanno e il caso "termini cambiati
  // nel frattempo" (RMU01 dal trigger server-side).
  return withUgcTermsGate(async () => {
...
async function setCampaignListed(campaignId, listed) {
  // Pubblicare nell'elenco pubblico è UGC: gate server-side (RMU01) +
  // modale di accettazione. Il ritiro (listed=false) non passa dal gate.
...
/* Prima i metadati, poi il file: se la rimozione del PDF fallisce la storia
   non dichiara più una premessa (nessun link rotto) e il file resta
   rilevabile come orfano (list_storage_orphans), mai il contrario. */
async function removeCampaignPremise(campaignId) {
  ...
  if (error) throw error;
  try { await withTimeout(sb.storage.from('premises').remove([premisePath(campaignId)]), 'Rimozione PDF'); }
  catch (e) { /* orfano rilevabile lato server */ }
}
...
async function getMyProfile(userId) {
  const { data, error } = await withTimeout(
    sb.from('profiles').select('display_name, account_role, avatar_url, plan_key, country, date_of_birth, suspended_at, suspended_reason').eq('id', userId).single(),
    'Profilo'
  );
  if (error) throw error;
  return data;
...
const LIBRARY_STATIC_ITEMS = [
  ...
  { id: 'static-setting-icaro', kind: 'setting', title: 'Icaro', access_status: 'sempre_disponibile', cover_asset_path: 'img/settings/icaro/icaro-copertina.png', staticAction: 'setting', settingKey: 'icaro' },
  { id: 'static-setting-ich', kind: 'setting', title: 'Ich', access_status: 'sempre_disponibile', cover_asset_path: 'img/settings/ich/ich-copertina.png', staticAction: 'setting', settingKey: 'ich' },
  { id: 'static-setting-eidos', kind: 'setting', title: 'Eidos', access_status: 'sempre_disponibile', cover_asset_path: 'img/settings/eidos/eidos-copertina.png', staticAction: 'setting', settingKey: 'eidos' }
];
let libraryItemsCache = [];
// Due schede, non tre filtri: 'sourcebook' = Biblioteca (manuali),
// 'setting' = Avventure (ambientazioni giocabili). Non esiste piu' un
// "Tutti" che mescolava le due cose — era proprio la confusione che rendeva
// Compendio e Biblioteca indistinguibili.
let libraryActiveFilter = 'sourcebook';
...
function libraryItemCardHtml(item) {
  ...
  const coverPath = item.cover_asset_path;
  const icon = item.kind === 'sourcebook'
    ? '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 6.5c-2.5-1.8-5.3-2.1-9-1.7v13c3.7-.4 6.5-.1 9 1.7 2.5-1.8 5.3-2.1 9-1.7v-13c-3.7-.4-6.5-.1-9 1.7Z"/><path d="M12 6.5v13"/></svg>'
    : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 2.5 2.8 6.7 6.7 2.8-6.7 2.8L12 21.5l-2.8-6.7L2.5 12l6.7-2.8L12 2.5Z"/><circle cx="12" cy="12" r="1.5"/></svg>';
  const cover = coverPath ? `<img src="${escapeHtml(coverPath)}" alt="" loading="lazy" decoding="async">` : '';
  const actionLabel = item.staticAction === 'manual' ? 'Apri il manuale' : (isOpenable ? 'Apri' : 'Bloccato');
  return `<div class="char-card library-item-card">
    <div class="avatar library-item-art" aria-hidden="true">${icon}${cover}</div>
    <div class="info">
...
function renderLibraryItems() {
  ...
  wrap.innerHTML = errorNote + `<p class="helper-text" style="margin:0;">${libraryItemsCache.length
    ...
    : 'Il catalogo dei contenuti non è ancora popolato: qui compariranno le avventure di Role Makers non appena saranno definiti contenuto, diritto di accesso e offerta commerciale. Il regolamento di base resta comunque sempre incluso, nella scheda Biblioteca.'}</p>`;
}
...
function renderCustomContentsList(campaignNameById) {
  ...
  if (!wrap) return;
  // Le annotazioni testuali della versione precedente restano visibili
  // solo se esistono: il nuovo flusso di creazione produce oggetti.
  const legacy = $('#customcontents-legacy');
  if (legacy) legacy.classList.toggle('hidden', !customContentsCache.length);
  wrap.innerHTML = customContentsCache.length
    ? customContentsCache.map(item => customContentCardHtml(item, campaignNameById || {})).join('')
    : `<p class="helper-text" style="margin:0;">Non hai annotazioni salvate in precedenza.</p>`;
}

/* Borsa del Narratore dentro "Contenuti personalizzati": stessa tabella,
   stesse righe (narratorBagItemRowHtml) e stesse RPC della Borsa nel tab
   PNG della scheda storia. Storie elencate: quelle in cui l'utente è
   owner/narratore/co_narratore (stesso criterio di is_campaign_master). */
let customContentsBagCampaigns = [];
let customContentsBagCampaignId = null;

async function listMyMasterCampaigns() {
  try {
    if (typeof listMyCampaignMemberships === 'function') {
      const ms = await listMyCampaignMemberships();
      return ms.filter(m => ['owner', 'narratore', 'co_narratore'].includes(m.role)).map(m => ({ id: m.campaignId, name: m.name }));
    }
  } catch (e) { /* ripiego sotto */ }
  return ((await listMyCampaigns().catch(() => [])) || []).map(c => ({ id: c.id, name: c.name }));
}

async function renderCustomContentsBag() {
  const wrap = $('#customcontents-bag');
  const newBtn = $('#btn-customcontent-new');
  if (!wrap) return;
  if (!customContentsBagCampaigns.length) {
    if (newBtn) newBtn.classList.add('hidden');
    wrap.innerHTML = `<div class="section-title" style="margin-top:0;"><span class="dot neutral"></span>🎒 Borsa del Narratore</div>
      <p class="helper-text" style="margin:0;">La creazione di oggetti è del Narratore: diventa disponibile quando sei Narratore di una storia (la crei da «Le tue storie»).</p>`;
    return;
  }
  if (!customContentsBagCampaigns.some(c => c.id === customContentsBagCampaignId)) customContentsBagCampaignId = customContentsBagCampaigns[0].id;
  if (newBtn) newBtn.classList.remove('hidden');
  const id = customContentsBagCampaignId;
  const opts = customContentsBagCampaigns.map(c => `<option value="${c.id}" ${c.id === id ? 'selected' : ''}>${escapeHtml(c.name || 'Storia')}</option>`).join('');
  const head = `<div class="section-title" style="margin-top:0;"><span class="dot neutral"></span>🎒 Borsa del Narratore</div>
    <div class="field"><label for="customcontents-bag-campaign">Storia</label><select id="customcontents-bag-campaign">${opts}</select></div>`;
  wrap.innerHTML = head + '<p class="helper-text" style="margin:0;">Verifica in corso…</p>';
  let items = [], chars = [];
  try {
    [items, chars] = await Promise.all([fetchNarratorBagItems(id), listCampaignCharacters(id).catch(() => [])]);
  } catch (e) {
    if (id !== customContentsBagCampaignId) return;
    wrap.innerHTML = head + `<p class="helper-text" style="margin:0;">Errore nel caricare la borsa: ${escapeHtml(describeError(e))}</p>`;
    return;
  }
  if (id !== customContentsBagCampaignId) return;
  const playerChars = chars.filter(ch => !ch.isMasterOwned);
  wrap.innerHTML = head + `
    <div class="box"><div class="box-bar"></div><div class="box-pad" style="display:flex;flex-direction:column;gap:6px;">
      ${items.length ? items.map(it => narratorBagItemRowHtml(it, playerChars)).join('') : '<p class="helper-text" style="margin:0;">La borsa di questa storia è vuota: crea il primo oggetto.</p>'}
      <button type="button" class="btn btn-primary btn-sm" id="btn-customcontents-bag-create" style="margin-top:6px;align-self:flex-start;">+ Crea oggetto</button>
    </div></div>`;
}
...
async function renderCustomContentsView() {
  ...
  const bagPromise = listMyMasterCampaigns().catch(() => []).then(list => { customContentsBagCampaigns = list || []; return renderCustomContentsBag(); });
  try {
...
const NOTIFICATION_TYPE_INFO = {
  ...
  contact_request_received: { icon: '👋', label: 'Richiesta di contatto' },
  contact_accepted: { icon: '🤝', label: 'Contatto accettato' }
};
...
// Solo notifiche DA LEGGERE (read_at null): quelle già lette non formano
// uno storico visibile — né qui, né nel contatore, né in Home — su ogni
// dispositivo, perché il filtro è lato server sulla stessa colonna
// read_at scritta da markNotificationRead/markAllNotificationsRead (nessun
// dato cancellato: la riga resta nel database, solo fuori dalla vista).
...
function notificationDetailText(n) {
  ...
  switch (n.type) {
    ...
    case 'contact_request_received': return `${p.requester_name || 'Un utente'} ti ha inviato una richiesta di contatto.`;
    case 'contact_accepted': return `${p.addressee_name || 'Un utente'} ha accettato la tua richiesta di contatto.`;
    default: return 'Aggiornamento della tua storia.';
  }
}

// Nota su contact_request_received: leggere la notifica NON risponde alla
// richiesta (resta in attesa nella sezione Contatti finché non la accetti
// o rifiuti lì). Le altre notifiche riportano esiti già decisi altrove.
const NOTIFICATION_ACTION_HINT = {
  contact_request_received: 'Per accettarla o rifiutarla apri i Contatti: segnarla come letta non risponde alla richiesta.'
};

function notificationRowHtml(n) {
  const info = NOTIFICATION_TYPE_INFO[n.type] || { icon: '🔔', label: 'Attività' };
  const unread = !n.read_at;
  const hint = NOTIFICATION_ACTION_HINT[n.type];
  return `<div class="box notification-row${unread ? ' unread' : ''}" data-notificationid="${n.id}" style="display:flex;align-items:flex-start;gap:10px;width:100%;text-align:left;margin-bottom:8px;padding:12px;background:var(--antracite-2);border:1px solid var(--bordo-scuro);border-radius:var(--radius);">
    ...
    <span style="flex:1;min-width:0;">
      ...
      ${hint ? `<span class="helper-text" style="margin:2px 0 0;display:block;font-style:italic;">${escapeHtml(hint)}</span>` : ''}
    ...
    ${unread ? `<button type="button" class="btn btn-ghost btn-sm" data-notificationread="${n.id}" style="flex:none;" aria-label="Segna come letta: ${escapeHtml(info.label)}">Segna come letta</button>` : ''}
  </div>`;
}
...
function renderNotificationsList() {
  ...
  if (!wrap) return;
  // Difesa in profondità: anche se una riga già letta restasse in cache
  // (segnata in questa sessione), non viene mai mostrata.
  const unread = notificationsCache.filter(n => !n.read_at);
  const markAll = $('#notifications-mark-all-read');
  if (markAll) markAll.classList.toggle('hidden', !unread.length);
  if (!unread.length) {
    wrap.innerHTML = `<p class="helper-text" style="margin:0;">Nessuna notifica da leggere. Quando succede qualcosa nelle tue storie o nei tuoi contatti, la trovi qui.</p>`;
    return;
  }
  const muted = getMutedNotificationTypes();
  const visible = unread.filter(n => !muted.has(n.type));
  wrap.innerHTML = visible.length
    ? visible.map(n => notificationRowHtml(n)).join('')
    : `<p class="helper-text" style="margin:0;">Le notifiche da leggere sono tutte di tipi che hai silenziato: cambia le preferenze qui sotto per tornare a vederle.</p>`;
}
...
function renderNotificationPreferences() {
  ...
  if (!wrap) return;
  const muted = getMutedNotificationTypes();
  // Anche i tipi silenziati senza notifiche da leggere restano elencati,
  // altrimenti non si potrebbero più riattivare.
  const types = Array.from(new Set(notificationsCache.map(n => n.type).concat(Array.from(muted))));
...
function setNotificationsBadgeCount(count) {
...
// Aggiornamento immediato del contatore dopo "Segna come letta"/"Segna
// tutte": dalla cache locale, senza attendere un nuovo giro di rete.
function updateNotificationsBadgeFromCache() {
  const muted = getMutedNotificationTypes();
  setNotificationsBadgeCount(notificationsCache.filter(n => !n.read_at && !muted.has(n.type)).length);
}
...
async function updateNotificationsBadge() {
  ...
  try {
    ...
    const muted = getMutedNotificationTypes();
    setNotificationsBadgeCount(items.filter(n => !n.read_at && !muted.has(n.type)).length);
...
/* Esiti della Edge Function (supabase/functions/delete-account/core.ts):
   solo `ok: true` + code 'DELETED' significa credenziale cancellata e
   verificata assente. AUTH_DELETE_FAILED (data_removed: true) è lo STATO
   PARZIALE: dati già rimossi, credenziale ancora presente — mai presentato
   come "account eliminato", sempre ripetibile. L'errore lanciato porta
   `code` e `dataRemoved` per il chiamante. */
async function deleteMyAccount(campaignResolutions) {
  ...
  // Compatibilità con la versione precedente della Edge Function (ancora
  // distribuita finché non viene ripubblicata): { ok: true } senza `code`
  // veniva restituito solo dopo un deleteUser() riuscito; il suo stato
  // parziale era un 500 con il messaggio "I tuoi dati sono stati rimossi".
  if (!error && body && body.ok === true && (body.code === 'DELETED' || body.code === undefined)) return { code: 'DELETED' };
  const legacyPartial = !(body && body.code) && /I tuoi dati sono stati rimossi/.test(message || (body && body.error) || '');
  const err = new Error(message || (body && body.error) || 'Eliminazione non confermata dal server: riprova');
  err.code = (body && body.code) || (legacyPartial ? 'AUTH_DELETE_FAILED' : 'UNKNOWN');
  err.dataRemoved = !!(body && body.data_removed) || legacyPartial;
  throw err;
}
  ...
  return `
      <div class="field"><label>Tipo</label>
        <select data-loottype>
          ...
          <option value="chiave">Oggetto generico</option>
        </select>
      </div>
...
function assistenzaHtml() {
  // Le domande frequenti hanno un blocco proprio nella vista Assistenza:
  // qui resterebbero un secondo ingresso alla stessa sezione.
...
/* Informativa e termini sono pubblicati come pagine statiche sullo stesso
   dominio dell'app (generate da PRIVACY.md/TERMINI.md, vedi
   scripts/build-legal-pages.js). Si aprono FUORI dalla WebView: farvi
   navigare la WebView significherebbe abbandonare la SPA senza un modo
   ovvio di tornare indietro, perche' quelle pagine non hanno la barra di
   un browser. Sul web e' un normale target="_blank"; nell'app nativa il
   click viene intercettato e passato al browser di sistema (vedi il
   listener su #account-legal-box). */
const LEGAL_PRIVACY_URL = 'https://rolemakers.it/privacy.html';
const LEGAL_TERMS_URL = 'https://rolemakers.it/termini.html';

function legalLinksHtml() {
  return `<div style="display:flex;flex-direction:column;gap:8px;">
    <a class="legal-link" data-external href="${LEGAL_PRIVACY_URL}" target="_blank" rel="noopener noreferrer" style="color:var(--testo-secondario-dark-2);">Informativa sulla privacy</a>
    <a class="legal-link" data-external href="${LEGAL_TERMS_URL}" target="_blank" rel="noopener noreferrer" style="color:var(--testo-secondario-dark-2);">Termini di servizio</a>
    <p class="helper-text" style="margin:0;">Si aprono nel browser. Nell'informativa trovi anche cosa viene cancellato e cosa resta in forma anonima quando elimini l'account.</p>
  </div>`;
}

/* ---- Repertorio delle storie -------------------------------------------
   Tratti, Tecniche e Incantesimi in gioco nelle storie a cui partecipo.

   Una sola chiamata: list_my_story_repertoire() unisce lato server tre
   fonti che stanno in posti diversi (campaign_known_traits per i tratti,
   characters.data->'tecniche' e ->'abilita' per il resto). Farlo dal
   client avrebbe significato una richiesta per storia piu' una per
   personaggio, e leggere schede altrui che le policy non espongono.

   Il filtro sulle storie NON e' un parametro: la funzione deriva l'insieme
   da auth.uid(). Non esiste quindi un identificativo da manipolare per
   farsi restituire il repertorio di un tavolo a cui non si appartiene. */
const REPERTOIRE_KINDS = {
  tratto: { titolo: 'Tratti', vuoto: 'Nessun tratto approvato nelle tue storie.' },
  tecnica: { titolo: 'Tecniche', vuoto: 'Nessuna tecnica nelle schede delle tue storie.' },
  incantesimo: { titolo: 'Incantesimi e magie', vuoto: 'Nessun incantesimo nelle schede delle tue storie.' }
};
const REPERTOIRE_LIST_LABEL = {
...
let repertoireActiveKind = 'tratto';

async function listMyStoryRepertoire() {
  const { data, error } = await withTimeout(sb.rpc('list_my_story_repertoire'), 'Repertorio delle storie');
  if (error) throw error;
  return data || [];
}

function repertoireRowHtml(row) {
  // La seconda riga dice da dove arriva la voce: senza, un elenco di nomi
  // in un gruppo con piu' storie e' inutilizzabile.
  const parti = [];
  if (row.campaign_name) parti.push(escapeHtml(row.campaign_name));
  if (row.owner_name) parti.push(escapeHtml(row.owner_name));
  if (row.list_key) parti.push(escapeHtml(REPERTOIRE_LIST_LABEL[row.list_key] || row.list_key));
  if (row.tipo) parti.push(escapeHtml(row.tipo));
  if (row.lv) parti.push('Lv ' + escapeHtml(row.lv));
  return `<div class="char-card">
    <div class="info">
      <div class="name">${escapeHtml(row.name || '')}</div>
      <div class="meta">${parti.join(' · ')}</div>
    </div>
  </div>`;
}

function renderRepertoireItems() {
  const wrap = $('#repertoire-items');
  if (!wrap) return;
  const titolo = $('#repertoire-title');
  const meta = REPERTOIRE_KINDS[repertoireActiveKind] || REPERTOIRE_KINDS.tratto;
  if (titolo) titolo.textContent = meta.titolo;

  if (repertoireError) {
    wrap.innerHTML = `<p class="helper-text" style="margin:0;">Non riesco a leggere il repertorio: ${escapeHtml(describeError(repertoireError))}</p>`;
    return;
  }
  if (repertoireCache === null) {
    wrap.innerHTML = `<p class="helper-text" style="margin:0;">Carico…</p>`;
    return;
  }
  const q = (($('#repertoire-search') || {}).value || '').trim().toLowerCase();
  const righe = repertoireCache
    .filter(r => r.kind === repertoireActiveKind)
    .filter(r => !q || [r.name, r.campaign_name, r.owner_name].some(v => String(v || '').toLowerCase().includes(q)));

  if (!righe.length) {
    wrap.innerHTML = `<p class="helper-text" style="margin:0;">${q ? 'Nessun risultato per questa ricerca.' : escapeHtml(meta.vuoto)}</p>`;
    return;
  }
  wrap.innerHTML = `<p class="helper-text" style="margin:0 0 10px;">${righe.length} ${righe.length === 1 ? 'voce' : 'voci'}</p>`
    + righe.map(repertoireRowHtml).join('');
}

async function renderRepertoireView(kind) {
  if (kind && REPERTOIRE_KINDS[kind]) repertoireActiveKind = kind;
  const search = $('#repertoire-search');
  if (search) search.value = '';
  $$('#repertoire-tabs .tab-btn').forEach(b => b.classList.toggle('active', b.dataset.reptab === repertoireActiveKind));
  repertoireCache = null;
  repertoireError = null;
  renderRepertoireItems();
  try {
    repertoireCache = await listMyStoryRepertoire();
  } catch (e) {
    // Mai un elenco vuoto quando la rete non risponde: "nessun tratto" e
    // "non ho potuto chiedere" sono due informazioni diverse.
    repertoireCache = [];
    repertoireError = e;
  }
  renderRepertoireItems();
...
/* ---- Vista "Assistenza" -------------------------------------------------
   Il tondo "?" in copertina porta qui. Raccoglie contatto, domande
   frequenti, documenti legali e versione: le quattro cose che si cercano
   quando qualcosa non va.

   Questi blocchi VIVEVANO in Account, dove nessuno li cercava. Sono stati
   spostati, non copiati: in Account non ci sono piu', e un test lo
   verifica. Due posti che dicono la stessa cosa divergono sempre, e per i
   documenti legali divergere significa dichiarare il falso. */
function renderSupportView() {
  const contact = $('#support-contact-box');
  if (contact) contact.innerHTML = assistenzaHtml();
  const faq = $('#support-faq-box');
  if (faq) faq.innerHTML = supportFaqHtml();
  const legal = $('#support-legal-box');
  if (legal) legal.innerHTML = legalLinksHtml();
  const info = $('#support-info-box');
  if (info) info.innerHTML = diagnosticInfoHtml();
}

/* Le domande frequenti non sono una pagina a parte: sono una sezione del
   manuale ("FAQ", vedi js/rules.js). Riusata quella, senza una seconda
   copia del testo da tenere allineata. */
function supportFaqHtml() {
  return `<p class="helper-text" style="margin:0 0 8px;">Le risposte alle domande più comuni su personaggi, storie e combattimento.</p>
    <button type="button" class="btn btn-ghost btn-sm" id="support-open-faq" style="align-self:flex-start;">Apri le domande frequenti</button>`;
}
...
function accountStatusHtml(session, caps, profile) {
  if (!session) {
    ...
    return `
      ...
      ${caps.googlePublicSignInEnabled ? '<button class="btn btn-ghost btn-sm" id="acc-google">Accedi con Google</button>' : ''}
...
function accountIdentityControlsHtml(profile, entitlements, consents) {
  return `
    ...
    <div class="section-title" style="margin-top:14px;"><span class="dot neutral"></span>Novità</div>
    ...
    <div class="box"><div class="box-bar"></div><div class="box-pad" style="display:flex;flex-direction:column;gap:8px;">
      ...
      <div id="acc-linked-identities"></div>
...
/* ---------------------------------------------- Fase 10: UGC/moderazione
   Superficie UGC reale del prodotto: nome di una storia pubblica, richieste
   di ingresso da estranei, log di sessione e tratti proposti dentro una
   campagna (vedi supabase/migrations/20260916000000_ugc_moderation.sql).
   La chat (di campagna e privata), le richieste di contatto, la
   pubblicazione nell'elenco pubblico e la condivisione di contenuti/diario
   passano dallo stesso gate server-side (trigger rm_ugc_gate_trigger,
   20261001000000_ugc_terms_server_authoritative.sql). */

// Versione delle regole UGC: UNA sola fonte, il server
// (current_ugc_terms_version(), vedi
// supabase/migrations/20261001000000_ugc_terms_server_authoritative.sql).
// Il client non ne tiene più una copia — prima una costante qui era rimasta
// a '2026-09-05' mentre il database era già a '2026-09-15', e chi aveva
// accettato la versione vecchia non vedeva mai il modale di riaccettazione.
// Lo stato arriva da my_ugc_terms_status() ('accepted' | 'outdated' |
// 'legacy_unknown'); l'accettazione registra sempre e solo la versione
// corrente decisa dal server (accept_current_ugc_terms(), nessun parametro).
let lastUgcTermsStatus = null;

async function fetchUgcTermsStatus() {
  const { data, error } = await withTimeout(sb.rpc('my_ugc_terms_status'), 'Verifica termini');
  if (error) throw error;
  lastUgcTermsStatus = data || null;
  return lastUgcTermsStatus;
}

async function hasAcceptedUgcTerms() {
  const status = await fetchUgcTermsStatus();
  return !!(status && status.status === 'accepted');
}

async function acceptUgcTerms() {
  const { data, error } = await withTimeout(sb.rpc('accept_current_ugc_terms'), 'Accettazione termini');
  if (error) throw error;
  lastUgcTermsStatus = data || null;
  if (!lastUgcTermsStatus || lastUgcTermsStatus.status !== 'accepted') {
    throw new Error('Accettazione non registrata: riprova');
  }
}

/* Errori del gate server-side (trigger rm_ugc_gate_trigger): RMU01 =
   regole della versione corrente non accettate, RMU02 = account sospeso. */
function isUgcTermsRequiredError(err) { return !!(err && err.code === 'RMU01'); }

/* Esegue un'azione che crea/condivide UGC; se il server la respinge perché
   mancano le regole correnti (RMU01), mostra il modale e — solo dopo
   un'accettazione esplicita — riprova UNA volta. Mai un'accettazione
   implicita, mai un secondo tentativo senza il consenso. */
async function withUgcTermsGate(action) {
  try {
    return await action();
  } catch (err) {
    if (!isUgcTermsRequiredError(err)) throw err;
    if (!(await ensureUgcTermsAccepted())) {
      throw new Error('Per continuare devi accettare le regole della community.');
    }
    return await action();
  }
}

/* Gate riusabile prima di ogni azione che crea/condivide UGC (creare una
   storia, chiedere di entrare in una storia altrui...): se i termini sono
   già accettati non mostra nulla, altrimenti apre il modale e attende la
   scelta dell'utente. Puramente lato UX — can_create_ugc() lato server
   applica comunque lo stesso controllo indipendentemente da questo, quindi
   un eventuale bypass qui non aprirebbe alcun buco, solo un errore meno
   chiaro al posto del modale. Risolve a true (azione da proseguire) o
   false (utente ha annullato, o il modale non è disponibile in questa vista). */
async function ensureUgcTermsAccepted() {
  const session = await currentCloudSession();
  if (!session || isGuestUser(session)) throw new Error('Serve un account per questa azione');
  try {
    if (await hasAcceptedUgcTerms()) return true;
  } catch (e) { /* verifica fallita (rete): si mostra comunque il modale, mai un blocco silenzioso */ }
  const modal = $('#ugc-terms-modal');
  if (!modal) return false;
  // Versione mostrata = quella che il server registrerà (nessuna copia locale).
  const versionEl = $('#ugc-terms-modal-version');
  if (versionEl) {
    const v = lastUgcTermsStatus && lastUgcTermsStatus.current_version;
    versionEl.textContent = v ? `Versione ${v}` : '';
    versionEl.classList.toggle('hidden', !v);
  }
  return new Promise(resolve => {
    const errorEl = $('#ugc-terms-modal-error');
    const yesBtn = $('#ugc-terms-modal-yes');
    const noBtn = $('#ugc-terms-modal-no');
    ...
    const onNo = () => { cleanup(); modal.classList.add('hidden'); resolve(false); };
    function cleanup() {
...
async function submitUgcReport(targetType, targetId, category, details) {
  const { data, error } = await withTimeout(
    sb.rpc('submit_report', { p_target_type: targetType, p_target_id: targetId, p_category: category, p_details: details || '' }),
    'Invio segnalazione'
  );
  if (error) throw error;
  return data;
...
async function submitUgcAppeal(message) {
  const { data, error } = await withTimeout(sb.rpc('submit_moderation_appeal', { p_message: message || '' }), 'Invio ricorso');
  if (error) throw error;
  return data;
}

async function blockUser(targetId) {
  if (typeof invalidateForegroundNoticeCache === 'function') invalidateForegroundNoticeCache();
  const { error } = await withTimeout(sb.rpc('block_user', { p_target_id: targetId }), 'Blocco utente');
  if (error) throw error;
}
async function unblockUser(targetId) {
  if (typeof invalidateForegroundNoticeCache === 'function') invalidateForegroundNoticeCache();
  const { error } = await withTimeout(sb.rpc('unblock_user', { p_target_id: targetId }), 'Sblocco utente');
  if (error) throw error;
}

/* Elenco utenti bloccati dall'account corrente: letto direttamente dalla
   tabella (RLS: solo le proprie righe come blocker sono leggibili), stesso
   schema di lettura già in uso altrove (vedi listPendingJoinRequests). */
async function listMyBlocks() {
  const session = await currentCloudSession();
  if (!session) return [];
  const { data, error } = await withTimeout(
    sb.from('user_blocks').select('blocked_id, created_at').eq('blocker_id', session.user.id).order('created_at', { ascending: false }),
    'Utenti bloccati'
  );
  if (error) throw error;
  if (!data.length) return [];
  const names = await fetchDisplayNames(data.map(b => b.blocked_id));
  return data.map(b => ({ id: b.blocked_id, name: names[b.blocked_id] || 'Avventuriero' }));
}

async function listModerationQueue() {
  const { data, error } = await withTimeout(sb.rpc('list_pending_reports'), 'Coda moderazione');
  if (error) throw error;
  return data || [];
}
async function resolveModerationReport(reportId, resolution, note) {
  const { data, error } = await withTimeout(
    sb.rpc('resolve_report', { p_report_id: reportId, p_resolution: resolution, p_note: note || '' }),
...
/* ---- modale di segnalazione, condiviso da ogni pulsante "🚩 Segnala"
   dell'app (storie pubbliche in js/cloud-character.js, richieste di
   ingresso qui sotto): un solo stato in memoria per il bersaglio corrente,
   wiring una volta sola in wireCloudAccountEvents (vedi sotto), niente
   listener aggiunti/rimossi ad ogni apertura come per ugc-terms-modal
   (qui l'utente può riaprire il modale più volte nella stessa sessione,
   niente rischio di listener duplicati che invierebbero la segnalazione
   più volte). */
let ugcReportTarget = null;
function openReportModal(targetType, targetId, label) {
  const modal = $('#ugc-report-modal');
  if (!modal) return;
  ugcReportTarget = { type: targetType, id: targetId };
  const targetEl = $('#ugc-report-modal-target');
  if (targetEl) targetEl.textContent = label || 'Segnala questo contenuto';
  const categorySel = $('#ugc-report-category');
  if (categorySel) categorySel.value = 'spam';
  const detailsInput = $('#ugc-report-details');
  if (detailsInput) detailsInput.value = '';
  const errorEl = $('#ugc-report-modal-error');
  if (errorEl) errorEl.textContent = '';
  modal.classList.remove('hidden');
}

const REPORT_CATEGORY_LABELS = {
  spam: 'Spam', harassment: 'Molestie', illegal_content: 'Contenuto illegale',
  sexual_content: 'Contenuto sessuale', violence: 'Violenza', copyright: 'Violazione di copyright', other: 'Altro'
};
const REPORT_TARGET_TYPE_LABELS = {
  campaign: 'Storia', user: 'Utente', session_log: 'Log di sessione', known_trait: 'Tratto proposto', appeal: 'Ricorso'
};

/* Riga della coda moderazione (solo admin, vedi list_pending_reports):
   "Rimuovi contenuto" mostrato solo dove resolve_report lo accetta
   davvero (campaign/session_log/known_trait — vedi la funzione SQL),
   "Riammetti" solo su un ricorso, "Sospendi autore" altrimenti. */
function moderationReportRowHtml(r) {
  const created = r.created_at ? new Date(r.created_at).toLocaleString('it-IT') : '';
  return `<div style="padding:8px 0;border-top:1px solid var(--bordo-scuro);display:flex;flex-direction:column;gap:6px;">
    <div class="helper-text" style="margin:0;"><strong>${escapeHtml(REPORT_TARGET_TYPE_LABELS[r.target_type] || r.target_type)}</strong> · ${escapeHtml(REPORT_CATEGORY_LABELS[r.category] || r.category)}${created ? ` · ${created}` : ''}</div>
    ${r.details ? `<div class="helper-text" style="margin:0;">${escapeHtml(r.details)}</div>` : ''}
    <div class="helper-text" style="margin:0;font-family:var(--font-mono);font-size:11px;">id bersaglio: ${escapeHtml(r.target_id)}</div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
      <button class="btn btn-sm btn-ghost" data-modresolve="${r.id}::dismiss">Respingi</button>
      ${['campaign', 'session_log', 'known_trait'].includes(r.target_type) ? `<button class="btn btn-sm btn-ghost" data-modresolve="${r.id}::remove_content">Rimuovi contenuto</button>` : ''}
      ${r.target_type === 'appeal'
        ? `<button class="btn btn-sm btn-primary" data-modresolve="${r.id}::lift_suspension">Riammetti</button>`
        : `<button class="btn btn-sm btn-primary" data-modresolve="${r.id}::suspend_user" style="color:var(--fisico-forte);">Sospendi autore</button>`}
    </div>
  </div>`;
}

async function renderModerationQueue() {
  const list = $('#ugc-moderation-queue-list');
  if (!list) return;
  list.innerHTML = '<p class="helper-text" style="margin:0;">Caricamento…</p>';
  try {
    const reports = await listModerationQueue();
    list.innerHTML = reports.length
      ? reports.map(moderationReportRowHtml).join('')
      : '<p class="helper-text" style="margin:0;">Nessuna segnalazione in attesa.</p>';
  } catch (e) {
    list.innerHTML = `<p class="helper-text" style="margin:0;">Errore: ${escapeHtml(describeError(e))}</p>`;
  }
}

function blockedUserRowHtml(b) {
  return `<div class="row-between" style="padding:4px 0;">
    <span>${escapeHtml(b.name)}</span>
    <button class="btn btn-sm btn-ghost" data-unblockuser="${b.id}">Sblocca</button>
  </div>`;
}

/* Sezione "Comunità e moderazione" (Account): norme UGC sempre visibili,
   elenco utenti bloccati con sblocco, coda moderazione solo per admin,
   avviso + ricorso quando l'account risulta sospeso (suspended_at). */
function communityBoxHtml(profile, accountRole, blocks) {
  const suspended = profile && profile.suspended_at;
  const parts = [];
  if (suspended) {
    parts.push(`
      <p class="helper-text" style="margin:0;color:var(--fisico-forte);"><strong>Account sospeso.</strong>${profile.suspended_reason ? ` Motivo: ${escapeHtml(profile.suspended_reason)}.` : ''} Finché la sospensione è attiva non puoi creare o condividere nuovi contenuti pubblici (storie, richieste di ingresso, log di sessione, tratti).</p>
      <button class="btn btn-primary btn-sm" id="acc-ugc-appeal" style="align-self:flex-start;">Presenta ricorso</button>
    `);
  }
  parts.push(`<button class="btn btn-ghost btn-sm" id="acc-ugc-policy" style="align-self:flex-start;">📜 Norme sui contenuti della community</button>`);
  parts.push(`
    <div>
      <div class="helper-text" style="margin:0 0 4px;">Utenti bloccati${blocks && blocks.length ? '' : ': nessuno'}</div>
      ${blocks && blocks.length ? blocks.map(blockedUserRowHtml).join('') : ''}
    </div>
  `);
  if (accountRole === 'admin') {
    parts.push(`<button class="btn btn-ghost btn-sm" id="acc-ugc-modqueue" style="align-self:flex-start;">🛡 Coda moderazione</button>`);
  }
  return parts.join('');
}
...
function campaignCardHtml(c) {
  ...
  const iconStyle = c.icon ? ` style="background-image:url(${escapeHtml(c.icon)})"` : '';
...
function campaignIconHtml(campaignId, icon) {
  const preview = icon
    ? `<img src="${escapeHtml(icon)}" alt="" style="width:56px;height:56px;border-radius:var(--radius-sm);object-fit:cover;">`
...
function joinRequestRowHtml(r) {
  return `<div class="row-between" style="padding:4px 0;flex-wrap:wrap;gap:6px;">
    ...
    <span style="display:flex;gap:4px;flex-wrap:wrap;">
      ...
      <button class="btn btn-sm btn-ghost" data-reportuser="${r.requested_by}" data-reportuserlabel="${escapeHtml(r.playerName)}" title="Segnala ${escapeHtml(r.playerName)}">🚩</button>
      <button class="btn btn-sm btn-ghost" data-blockuser="${r.requested_by}" data-blockuserlabel="${escapeHtml(r.playerName)}" title="Blocca ${escapeHtml(r.playerName)}">⛔</button>
    </span>
  </div>`;
}
...
function narratorBagItemRowHtml(it, playerChars) {
  ...
  const typeLabel = { arma: 'Arma', scudo: 'Scudo', armatura: 'Armatura', consumabile: 'Consumabile', chiave: 'Oggetto generico' }[it.item_type] || it.item_type;
...
/* Stesso modale sia dalla scheda storia (tab PNG) sia da "Contenuti
   personalizzati": il contesto dice in quale borsa creare l'oggetto e cosa
   ridisegnare dopo. Senza contesto: la storia aperta nella scheda. */
let npcBagAddContext = null;
function openNpcBagAddModal(campaignId, onDone) {
  npcBagAddContext = campaignId ? { campaignId, onDone: onDone || null } : null;
  $('#npc-bag-add-fields').innerHTML = npcBagAddFormHtml();
  $('#npc-bag-add-source').value = '';
  $('#npc-bag-add-modal').classList.remove('hidden');
}
function closeNpcBagAddModal() {
  $('#npc-bag-add-modal').classList.add('hidden');
  npcBagAddContext = null;
}

/* Azioni di una riga della Borsa (assegna/rimuovi), condivise da scheda
   storia e "Contenuti personalizzati": stesse RPC, poi refresh() della
   schermata che le ha scatenate. Ritorna true se il click era suo. */
async function handleNarratorBagRowAction(target, refresh) {
  if (target.dataset.bagassignbtn) {
    const bagItemId = target.dataset.bagassignbtn;
    const sel = $(`[data-bagassignselect="${bagItemId}"]`);
    const charId = sel && sel.value;
    if (!charId) { toast('Scegli un giocatore'); return true; }
    try {
      await assignNarratorBagItemCloud(bagItemId, charId);
      toast('Oggetto assegnato: il giocatore lo riceve da accettare');
      refresh();
    } catch (err) { toast('Errore: ' + describeError(err)); }
    return true;
  }
  if (target.dataset.bagremovebtn) {
    if (!confirm('Rimuovere questo oggetto dalla borsa?')) return true;
    try {
      await removeNarratorBagItemCloud(target.dataset.bagremovebtn);
...
function activateCampaignSheetTab(name) {
  ...
  // Bug reale segnalato dall'utente: renderCampaignMapPanel() (chiamata da
  // renderCampaignSheet() ad ogni aggiornamento della scheda, non solo
  // quando si apre questo tab) può creare la mappa Leaflet mentre il tab
  // "Mappa" non è ancora quello attivo — il contenitore è display:none in
  // quel momento, Leaflet ne blocca lì le dimensioni interne (compreso lo
  // strato che intercetta i click) e passare a display:flex più tardi non
  // le ricalcola da solo. Va rifatto qui, ogni volta che "Mappa" diventa
  // il tab davvero visibile — sia per il Narratore (editor) sia per un
  // giocatore (vista di sessione), qualunque dei due sia quello attivo.
  if (name === 'mappa') {
    if (typeof refreshMapEditorCanvasSize === 'function') refreshMapEditorCanvasSize();
    if (typeof refreshMapSessionCanvasSize === 'function') refreshMapSessionCanvasSize();
  }
}
...
async function renderCampaignSheet() {
  ...
  try {
    ...
    if (typeof renderCampaignMapPanel === 'function') renderCampaignMapPanel(id);
    ...
    const bagItemsHtml = bagItems.length
      ...
      : '<p class="helper-text" style="margin:0;">Vuota: raccogli il bottino di un PNG segnato come morto in combattimento, oppure crea un oggetto.</p>';
    ...
    if (npcPanel) npcPanel.innerHTML = `
      ...
      <p class="helper-text">Armi, scudi, armature, consumabili e oggetti generici pronti da assegnare a un giocatore come bottino — bottino di un PNG morto o creati qui (anche da "Contenuti personalizzati"). Restano invisibili ai giocatori finché non li assegni.</p>
      ${bagItemsHtml}
      <button type="button" class="btn btn-ghost btn-sm" id="btn-open-npc-bag-add" style="margin-top:6px;align-self:flex-start;">+ Crea oggetto</button>
    `;
    ...
    const isMaster = await isCampaignSheetMaster(id);
    if (id !== activeCampaignSheetId) return;
    riassuntoPanel.innerHTML = isMaster
      ? campaignSessionLogsHtml(id, logs)
      : playerRecapHtml(id, logs, activeCampaignSheetName);
...
let campaignAssetsGalleryIndex = new Map();
let pendingReplaceCampaignAssetId = null;
...
async function renderCampaignAssetsGallery() {
  ...
  catch (err) { body.innerHTML = `<p class="helper-text" style="margin:16px;">Errore: ${escapeHtml(describeError(err))}</p>`; return; }
      ...
      ${assets.map(a => `
        <div class="asset-gallery-tile">
          ...
          <div class="asset-gallery-thumb" data-assetgallerythumb="${a.id}">
            ...
            <button type="button" class="ag-del ag-replace" style="left:4px;right:auto;" data-replacegalleryasset="${a.id}" title="Sostituisci immagine" aria-label="Sostituisci immagine">↻</button>
          </div>
        </div>
      `).join('')}
    </div>
  `;
  campaignAssetsGalleryIndex = new Map(assets.map(a => [a.id, a]));
  assets.forEach(a => {
...
async function renderHomeIdentityBox() {
  ...
  const noCaps = { googleProviderAvailable: false, googlePublicSignInEnabled: false, apple: false, passkey: false };
  let session;
  try {
    session = await currentCloudSession();
  } catch (e) {
  ...
  if (!session && !pendingRecoveryEmail) tryAutofillSavedCredential();
  // Qui stava il salto automatico alla Home "Il tuo tavolo" appena una
  // sessione reale risultava attiva sulla copertina (S1). Rimosso: la
  // sessione si risolve un istante DOPO il primo disegno, quindi il salto
  // arrivava a schermo già pieno e si vedeva la copertina sostituita da
  // sola — il "caricamento disordinato" segnalato dall'utente durante
  // l'accesso. Ora che "Home" è la copertina e il tavolo ha una voce
  // propria nella barra, quel salto contraddirebbe anche la voce appena
  // toccata: si resta dove si è, e al tavolo ci si va quando lo si
  // sceglie.
  ...
  if (!session) {
    try {
      ...
      if ((caps.googlePublicSignInEnabled || caps.apple) && untouched) box.innerHTML = accountStatusHtml(session, caps, profile);
...
async function renderAccountArea() {
  ...
  const communityWrap = $('#account-community-wrap');
  const communityBox = $('#account-community-box');
  ...
  try {
    ...
    if (communityWrap) communityWrap.classList.add('hidden');
  ...
  // profile sta al livello della funzione, non dentro il blocco di
  // identityBox: serve anche piu' sotto, alla sezione "Comunita' e
  // moderazione", che da profile.suspended_at ricava se mostrare l'avviso
  // di sospensione e il ricorso. Una seconda getMyProfile() sarebbe una
  // chiamata di rete in piu' per un dato gia' in mano.
  let profile = null;
  if (identityBox) {
    if (session && !isGuestUser(session)) {
      try { profile = await getMyProfile(session.user.id); } catch (e) { /* nickname non essenziale: il campo resta vuoto */ }
      ...
      renderLinkedIdentities(session);
    } else {
  ...
  if (communityWrap && communityBox) {
    if (session && !isGuestUser(session)) {
      communityWrap.classList.remove('hidden');
      let blocks = [];
      try { blocks = await listMyBlocks(); } catch (e) { /* elenco non essenziale: la sezione resta comunque utilizzabile */ }
      communityBox.innerHTML = communityBoxHtml(profile, accountRole, blocks);
    } else {
...
/* Recap lato giocatore: riassunti pubblicati dal Narratore, in sola
   lettura, nel tab "Recap" della scheda storia (non più una vista globale
   dell'indice). La RLS di campaign_session_logs restituisce a un giocatore
   solo le righe visibility='gruppo' della storia aperta: nessun filtro
   aggiuntivo lato client necessario. Modifica/eliminazione restano
   riservate al Narratore (campaignSessionLogsHtml). */
...
function playerRecapHtml(campaignId, logs, storyName) {
  lastPreviouslyOnLogs = logs;
  return `
    <div class="section-title" style="margin-top:0;"><span class="dot neutral"></span>Recap</div>
    ...
    ${logs.length ? `<div class="field"><label>Cerca nei riassunti</label><input type="text" data-searchplayerlogs="${campaignId}" placeholder="Cerca per titolo o testo..."></div>` : ''}
    <div data-playerlogslist="${campaignId}">${logs.length ? logs.map(previouslyOnEntryHtml).join('') : '<p class="helper-text" style="margin:0;">Il Narratore non ha ancora pubblicato nessun riassunto.</p>'}</div>
  `;
}

/* Il tab Recap distingue Narratore (registro modificabile) e giocatore
   (sola lettura) con lo stesso criterio già usato dalla Mappa della
   storia (isCampaignMapMaster: owner/narratore/co_narratore). In caso di
   errore si mostra la vista in sola lettura: la RLS resta comunque
   l'unica vera autorità su lettura e scrittura. */
async function isCampaignSheetMaster(campaignId) {
  try {
    return typeof isCampaignMapMaster === 'function' ? await isCampaignMapMaster(campaignId) : false;
  } catch (e) { return false; }
}

/* Apre il Recap nel contesto della storia: scheda storia + tab Recap.
   Usato da Home ("Leggi il recap") per la storia del personaggio attivo. */
function openCampaignRecap(campaignId) {
  openCampaignSheet(campaignId);
  activateCampaignSheetTab('riassunto');
}
function openActiveCharacterStoryRecap() {
  const c = typeof getActive === 'function' ? getActive() : null;
  if (!c || !c.cloudCampaignId) {
    toast('Il personaggio attivo non fa parte di nessuna storia: il recap si trova dentro la storia, quando ci entri.');
    return;
  }
  openCampaignRecap(c.cloudCampaignId);
}
...
function wireCloudAccountEvents() {
  ...
  // Se una copertina non carica, l'icona vettoriale sottostante resta visibile.
  $('#library-items').addEventListener('error', e => {
    if (e.target.matches('.library-item-art img')) e.target.remove();
  }, true);
  ...
  $('#notifications-list').addEventListener('click', async e => {
    // Solo il bottone esplicito "Segna come letta": la lettura non esegue
    // mai alcuna azione (approvare/rifiutare/completare una richiesta).
    const btn = e.target.closest('[data-notificationread]');
    if (!btn) return;
    const id = btn.dataset.notificationread;
    const n = notificationsCache.find(x => x.id === id);
    if (!n || n.read_at) return;
    // Ottimistico: la riga sparisce subito e il contatore scende; se il
    // salvataggio fallisce la riga torna, per non far credere persistito
    // uno stato che alla riapertura ricomparirebbe.
    n.read_at = new Date().toISOString();
    renderNotificationsList();
    renderNotificationPreferences();
    updateNotificationsBadgeFromCache();
    try {
  ...
  // "+" in alto e "+ Crea oggetto": creano un oggetto nella Borsa della
  // storia scelta (non più un'annotazione testuale).
  const openCustomContentsBagCreate = () => {
    if (!customContentsBagCampaignId) { toast('Serve una storia di cui sei Narratore'); return; }
    openNpcBagAddModal(customContentsBagCampaignId, renderCustomContentsBag);
  };
  $('#btn-customcontent-new').addEventListener('click', openCustomContentsBagCreate);
  $('#customcontents-bag').addEventListener('change', e => {
    if (e.target.id !== 'customcontents-bag-campaign') return;
    customContentsBagCampaignId = e.target.value;
    renderCustomContentsBag();
  });
  $('#customcontents-bag').addEventListener('click', async e => {
    if (e.target.id === 'btn-customcontents-bag-create') { openCustomContentsBagCreate(); return; }
    await handleNarratorBagRowAction(e.target, renderCustomContentsBag);
  });
  ...
  $('#delete-account-confirm-yes').addEventListener('click', async () => {
    ...
    if (btn.disabled) return; // doppio tocco: una sola richiesta alla volta
    btn.disabled = true;
    try {
      ...
      // Stato parziale: il modale resta aperto e la sessione attiva, così
      // "Elimina" può essere ripetuto (operazione idempotente lato server).
      // Mai "Account eliminato" finché la credenziale esiste ancora.
      if (err && err.code === 'AUTH_DELETE_FAILED') {
        toast('Eliminazione incompleta: i tuoi dati sono stati rimossi ma l\'account di accesso no. Tocca di nuovo "Elimina" per completare.');
      } else {
        toast('Errore: ' + describeError(err));
      }
    } finally {
  ...
  $('#home-status-box').addEventListener('click', async e => {
    ...
    if (e.target.id === 'acc-google') {
      signInWithProvider('google').catch(err => toast(err && /^OAUTH_/.test(String(err.code || '')) ? err.message : 'Errore: ' + describeError(err)));
      return;
    }
  ...
  $('#account-identity-controls').addEventListener('click', async e => {
    ...
    if (e.target.id === 'acc-link-google') {
      linkGoogleToCurrentAccount().catch(err => toast(err && /^OAUTH_/.test(String(err.code || '')) ? err.message : 'Errore: ' + describeError(err)));
      return;
    }
  ...
  // ---- Assistenza: apertura delle domande frequenti ----
  // Delegato da #view-support, che e' statico in index.html. Il bottone
  // stava in Account ed era servito dal listener di
  // #account-identity-controls: spostando il blocco senza spostare anche il
  // listener sarebbe rimasto un bottone che non fa nulla.
  $('#view-support').addEventListener('click', e => {
    if (e.target && e.target.id === 'support-open-faq') openRuleSection('FAQ');
  });

  // ---- "Privacy e termini": apertura fuori dalla WebView ----
  // Sul web il browser gestisce da solo target="_blank" e questo listener non
  // fa nulla. Nell'app nativa la WebView non ha barra indirizzi ne' pulsante
  // indietro visibile: lasciarla navigare su una pagina statica lascerebbe
  // l'utente bloccato fuori dalla SPA. Il browser di sistema, invece, si
  // chiude e restituisce il controllo all'app.
  // Delegato da #view-account, che e' statico in index.html: il box dei
  // collegamenti viene invece rigenerato a ogni renderAccountArea(), quindi
  // agganciarsi direttamente a lui significherebbe cablare un elemento che
  // al momento del cablaggio non esiste ancora.
  $('#view-support').addEventListener('click', async e => {
    const link = e.target.closest && e.target.closest('#support-legal-box a[data-external]');
    if (!link) return;
    if (typeof isNativeApp !== 'function' || !isNativeApp()) return;
    const browser = (typeof browserPlugin === 'function') ? browserPlugin() : null;
    if (!browser) return; // nessun plugin: resta il comportamento predefinito
    e.preventDefault();
    try {
      await browser.open({ url: link.href });
    } catch (err) {
      // Se il browser di sistema non si apre, meglio dirlo che restare muti:
      // questi due documenti sono l'unico posto in cui l'utente puo' leggere
      // cosa facciamo dei suoi dati.
      console.warn('Apertura del documento fallita:', err && err.message);
      toast('Non riesco ad aprire il browser. Il documento e\' su ' + link.href);
    }
  });

  // ---- "Comunità e moderazione" (Fase 10): norme, blocchi, ricorso, coda ----
  $('#account-community-box').addEventListener('click', async e => {
    if (e.target.id === 'acc-ugc-policy') {
      const modal = $('#ugc-policy-modal');
      if (modal) modal.classList.remove('hidden');
      return;
    }
    if (e.target.id === 'acc-ugc-appeal') {
      const modal = $('#ugc-appeal-modal');
      const messageInput = $('#ugc-appeal-message');
      const errorEl = $('#ugc-appeal-modal-error');
      if (messageInput) messageInput.value = '';
      if (errorEl) errorEl.textContent = '';
      if (modal) modal.classList.remove('hidden');
      return;
    }
    if (e.target.id === 'acc-ugc-modqueue') {
      const modal = $('#ugc-moderation-queue-modal');
      if (modal) modal.classList.remove('hidden');
    ...
    if (e.target.dataset.unblockuser) {
      try {
        await unblockUser(e.target.dataset.unblockuser);
  ...
  // ---- modale "Segnala" (condiviso: storie pubbliche, richieste di ingresso) ----
  $('#ugc-report-modal').addEventListener('click', async e => {
    if (e.target.id === 'ugc-report-modal-cancel' || e.target.id === 'ugc-report-modal') {
      $('#ugc-report-modal').classList.add('hidden');
      ugcReportTarget = null;
      return;
    }
    if (e.target.id === 'ugc-report-modal-send') {
      if (!ugcReportTarget) { $('#ugc-report-modal').classList.add('hidden'); return; }
      const btn = e.target;
      const errorEl = $('#ugc-report-modal-error');
      const category = $('#ugc-report-category').value;
      const details = $('#ugc-report-details').value.trim();
      btn.disabled = true;
      if (errorEl) errorEl.textContent = '';
      try {
        await submitUgcReport(ugcReportTarget.type, ugcReportTarget.id, category, details);
        $('#ugc-report-modal').classList.add('hidden');
        ugcReportTarget = null;
        toast('Segnalazione inviata: grazie, verrà esaminata');
      } catch (err) {
        if (errorEl) errorEl.textContent = describeError(err);
      }
      btn.disabled = false;
  ...
  // ---- modale "Presenta ricorso" (account sospeso) ----
  $('#ugc-appeal-modal').addEventListener('click', async e => {
    if (e.target.id === 'ugc-appeal-modal-cancel' || e.target.id === 'ugc-appeal-modal') {
      $('#ugc-appeal-modal').classList.add('hidden');
      return;
    }
    if (e.target.id === 'ugc-appeal-modal-send') {
      const btn = e.target;
      const errorEl = $('#ugc-appeal-modal-error');
      const message = $('#ugc-appeal-message').value.trim();
      if (!message) { if (errorEl) errorEl.textContent = 'Scrivi un messaggio prima di inviare il ricorso.'; return; }
      btn.disabled = true;
      if (errorEl) errorEl.textContent = '';
      try {
        await submitUgcAppeal(message);
        $('#ugc-appeal-modal').classList.add('hidden');
        toast('Ricorso inviato: verrà esaminato dal team di moderazione');
      } catch (err) {
        if (errorEl) errorEl.textContent = describeError(err);
      }
      btn.disabled = false;
  ...
  // ---- modale "Norme sui contenuti della community" (solo lettura) ----
  $('#ugc-policy-modal').addEventListener('click', e => {
    if (e.target.id === 'ugc-policy-modal-close' || e.target.id === 'ugc-policy-modal') {
      $('#ugc-policy-modal').classList.add('hidden');
    }
  });

  // ---- coda moderazione (solo admin) ----
  $('#ugc-moderation-queue-modal').addEventListener('click', async e => {
    if (e.target.id === 'ugc-moderation-queue-close' || e.target.id === 'ugc-moderation-queue-modal') {
      $('#ugc-moderation-queue-modal').classList.add('hidden');
      return;
    }
    if (e.target.dataset.modresolve) {
      const [reportId, resolution] = e.target.dataset.modresolve.split('::');
      let note = '';
      if (resolution === 'suspend_user') {
        note = prompt('Motivo della sospensione (visibile all\'utente):') || '';
        if (note === null) return;
      } else if (resolution === 'remove_content') {
        if (!confirm('Rimuovere il contenuto segnalato?')) return;
      } else if (resolution === 'lift_suspension') {
        if (!confirm('Riammettere questo utente, annullando la sospensione?')) return;
      }
      try {
        await resolveModerationReport(reportId, resolution, note);
        toast('Segnalazione evasa');
        renderModerationQueue();
      } catch (err) { toast('Errore: ' + describeError(err)); }
      return;
    }
  ...
  $('#account-campaigns-box').addEventListener('click', async e => {
    if (e.target.id === 'acc-create-campaign') {
      ...
      if (!(await ensureUgcTermsAccepted())) return;
      try {
  ...
  $('#view-campaignsheet').addEventListener('click', async e => {
    ...
    if (e.target.dataset.reportuser) {
      openReportModal('user', e.target.dataset.reportuser, `Segnali ${e.target.dataset.reportuserlabel || 'questo utente'}`);
      return;
    }
    if (e.target.dataset.blockuser) {
      const label = e.target.dataset.blockuserlabel || 'questo utente';
      if (!confirm(`Bloccare ${label}? Non vedrai più le sue storie pubbliche e lui non potrà più mandarti richieste di ingresso.`)) return;
      try {
        await blockUser(e.target.dataset.blockuser);
        toast(`${label} bloccato`);
        renderCampaignSheet();
      } catch (err) { toast('Errore: ' + describeError(err)); }
      return;
    }
    ...
    if (await handleNarratorBagRowAction(e.target, renderCampaignSheet)) return;
  });
  ...
  $('#account-campaigns-box').addEventListener('change', async e => {
    ...
    if (e.target.id === 'acc-new-campaign-icon-input') {
      ...
      try {
        ...
        info.innerHTML = `<img src="${escapeHtml(pendingNewCampaignIcon)}" alt="" style="width:48px;height:48px;border-radius:var(--radius-sm);object-fit:cover;">`;
      } catch (err) { toast('Errore: ' + describeError(err)); }
      return;
    }
  });
  ...
  // nessuna nuova query — solo il tab "Recap" della scheda storia.
  $('#view-campaignsheet').addEventListener('input', e => {
    // Recap in sola lettura del giocatore: stessa ricerca client-side.
    if (e.target.dataset.searchplayerlogs) {
      const q = e.target.value.trim().toLowerCase();
      const list = $(`[data-playerlogslist="${e.target.dataset.searchplayerlogs}"]`);
      if (!list) return;
      const filtered = q
  ...
  $('#campaign-assets-popup-body').addEventListener('click', e => {
    ...
    const replaceBtn = e.target.closest('[data-replacegalleryasset]');
    if (replaceBtn) {
      pendingReplaceCampaignAssetId = replaceBtn.dataset.replacegalleryasset;
      $('#campaign-assets-replace-file').click();
      return;
    }
  ...
  $('#campaign-assets-replace-file').addEventListener('change', async e => {
    const file = e.target.files[0];
    e.target.value = '';
    const asset = campaignAssetsGalleryIndex.get(pendingReplaceCampaignAssetId);
    pendingReplaceCampaignAssetId = null;
    if (!file || !asset) return;
    try {
      await replaceCampaignAssetImage(asset, file);
  ...
  $('#npc-bag-add-modal').addEventListener('click', async e => {
    ...
    if (e.target.id === 'npc-bag-add-submit') {
      ...
      const ctx = npcBagAddContext;
      const campaignId = ctx ? ctx.campaignId : activeCampaignSheetId;
      if (!campaignId) { toast('Scegli prima una storia'); return; }
      try {
        await addNarratorBagItemCloud(campaignId, itemType, item, sourceLabel);
        toast(`Creato nella borsa: ${item.name || item.nome}`);
        closeNpcBagAddModal();
        if (ctx && ctx.onDone) ctx.onDone(); else renderCampaignSheet();
      } catch (err) { toast('Errore: ' + describeError(err)); }
      return;
    }
    if (e.target.closest('[data-lootbonusadd]')) {
        ...
        (async () => {
          try {
            ...
            if (!error && (!data || data.deleted_at)) {
              // deleted_at valorizzato con la credenziale ancora valida =
              // STATO PARZIALE (dati rimossi, auth.users no: la riga del
              // profilo sparisce solo con la credenziale). Prima veniva
              // annunciato "account eliminato", falso: ora si completa
              // l'eliminazione già richiesta dall'utente, e lo si dice solo
              // se il server la conferma.
              let message = 'Questo account è stato eliminato.';
              if (data && data.deleted_at) {
                try {
                  await deleteMyAccount([]);
                  message = 'Eliminazione dell\'account completata.';
                } catch (e) {
                  message = 'L\'eliminazione di questo account non è ancora completa: accedi di nuovo per riprovare o scrivi all\'assistenza.';
                }
              }
              await signOutCloud();
              toast(message);
