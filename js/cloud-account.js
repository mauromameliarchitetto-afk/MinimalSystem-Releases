...
/* Login social (Google): Google blocca l'OAuth se rileva che gira dentro
   una WebView embedded (policy ufficiale "no embedded user-agent" per app
   native, vedi fonti nel piano di pubblicazione store) — prima di questa
   correzione signInWithOAuth faceva navigare la WebView dell'app stessa
   verso l'URL di accesso Google, esattamente il caso che la policy vieta.
   Ora, sull'app nativa, si chiede a supabase-js solo l'URL di accesso
   (skipBrowserRedirect: true, nessuna navigazione automatica) e lo si apre
   nel browser di sistema (Chrome Custom Tabs) con il plugin Capacitor
   Browser: la WebView dell'app non viene mai coinvolta nel login. Il
   ritorno funziona come prima (redirect a rolemakers://auth-callback,
   intercettato da completeSessionFromDeepLink in supabase-client.js), che
   chiude anche il browser di sistema appena l'app lo riceve. Sul web resta
   il comportamento di sempre (nessuna WebView, nessun blocco Google). */
async function signInWithProvider(provider) {
  const options = {};
  if (AUTH_REDIRECT_URL) options.redirectTo = AUTH_REDIRECT_URL;
  if (isCapacitorNative()) {
    options.skipBrowserRedirect = true;
    const { data, error } = await withTimeout(sb.auth.signInWithOAuth({ provider, options }), 'Accesso');
    if (error) throw error;
...
/* ---------------------------------------------- Fase 10: UGC/moderazione
   ...
   Nessuna chat/messaggistica esiste: niente da moderare lì. */

// Deve combaciare esattamente con current_ugc_terms_version() lato SQL:
// un bump lì richiede lo stesso bump qui, altrimenti chi ha già accettato
// la versione precedente continuerebbe a passare il gate lato client pur
// non avendo accettato quella nuova (la RPC lato server userebbe comunque
// la versione vera, quindi l'azione fallirebbe con un errore RLS invece
// che con il modale dedicato — nessun buco di sicurezza, solo una UX peggiore).
const UGC_TERMS_VERSION = '2026-09-05';

async function hasAcceptedUgcTerms(uid) {
  const { data, error } = await withTimeout(
    sb.rpc('has_accepted_terms', { p_user_id: uid, p_version: UGC_TERMS_VERSION }),
    'Verifica termini'
  );
  if (error) throw error;
  return !!data;
...
async function acceptUgcTerms() {
  const { error } = await withTimeout(sb.rpc('accept_terms', { p_version: UGC_TERMS_VERSION }), 'Accettazione termini');
  if (error) throw error;
}
...
async function ensureUgcTermsAccepted() {
  ...
  try {
    if (await hasAcceptedUgcTerms(session.user.id)) return true;
