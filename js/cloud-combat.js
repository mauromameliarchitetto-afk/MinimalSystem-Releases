...
/* Registro attacchi (S16, piano UX "confronto D&D Beyond"): a differenza
   di get_combat_board, qui è una SELECT diretta — la RLS "attacchi:
   lettura membri campagna" (20260803120000_combat_tables.sql) già
   permette a ogni membro di leggere la riga intera. Non serve
   redazione lato server: combat_attacks non contiene MAI uno stat block
   (solo id personaggio, etichetta arma/tecnica testuale, tiri e danno
   finale) — le stesse cifre che chiunque guardi il combattimento vede già
   scorrere dal vivo nel pannello d'attacco, mai un dato nuovo rispetto a
   quello. Il nome del personaggio si risolve lato client con
   combatCharNameById/combatParticipantName (js/app.js), che ricadono già
   su "???" per un PNG non ancora rivelato — stessa resa esatta della vista
   dal vivo, nessuna seconda logica di redazione da mantenere allineata. */
// FC5 (Registro di gioco condiviso): select estesa con i campi di dettaglio
// di ogni tiro (attacco/danno/difesa/salvezza) — prima non richiesti dalla
// sola riga sommario del registro, ora servono per "richiamare il
// dettaglio del tiro" senza una seconda query dedicata.
async function listCombatAttackLog(encounterId) {
  const { data, error } = await withTimeout(
    sb.from('combat_attacks')
      .select(`id, round_number, attacker_character_id, target_character_id, source_label, source_kind,
        is_environmental, is_surprise_attack, defense_type, defense_success, final_damage, status, created_at,
        attack_roll_total, attack_roll_detail, damage_roll_total, damage_roll_detail,
        defense_roll_total, defense_roll_detail, robustezza_roll_total, robustezza_roll_detail`)
      .eq('encounter_id', encounterId).eq('status', 'resolved')
...
/* Ciclo vita esteso (FC4, FASE FINALE): rinomina/note/archivio/
   eliminazione/duplicazione di un incontro, oltre a avvio/fine sopra —
   stesso schema RPC (SECURITY DEFINER, gated su is_campaign_master), vedi
   20260930010000_combat_encounter_lifecycle_extras.sql. */
async function renameCombatEncounter(encounterId, label) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_rename_combat_encounter', { p_encounter_id: encounterId, p_label: label }),
    'Rinomina combattimento'
  );
  if (error) throw error;
  return data;
}
async function setCombatEncounterNotes(encounterId, notes) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_set_combat_encounter_notes', { p_encounter_id: encounterId, p_notes: notes }),
    'Note combattimento'
  );
  if (error) throw error;
  return data;
}
async function archiveCombatEncounter(encounterId) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_archive_combat_encounter', { p_encounter_id: encounterId }),
    'Archiviazione combattimento'
  );
  if (error) throw error;
  return data;
}
async function deleteCombatEncounter(encounterId) {
  const { error } = await withTimeout(
    sb.rpc('narratore_delete_combat_encounter', { p_encounter_id: encounterId }),
    'Eliminazione combattimento'
  );
  if (error) throw error;
}
async function duplicateCombatEncounter(encounterId) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_duplicate_combat_encounter', { p_encounter_id: encounterId }),
    'Duplicazione combattimento'
  );
  if (error) throw error;
  return data;
}
/* Correzioni puntuali durante l'esecuzione (FC4): iniziativa manuale di un
   singolo partecipante, turno precedente (solo dentro lo stesso round),
   rinomina di un partecipante nel tabellone (etichetta locale, non tocca
   characters.name). */
async function setManualInitiative(encounterId, characterId, value, detail) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_set_manual_initiative', { p_encounter_id: encounterId, p_character_id: characterId, p_value: value, p_detail: detail || null }),
    'Iniziativa manuale'
  );
  if (error) throw error;
  return data;
}
async function combatPreviousTurn(encounterId, characterId) {
  const { data, error } = await withTimeout(
    sb.rpc('combat_previous_turn', { p_encounter_id: encounterId, p_character_id: characterId }),
    'Turno precedente'
  );
  if (error) throw error;
  return data;
}
async function renameCombatParticipant(encounterId, characterId, displayName) {
  const { data, error } = await withTimeout(
    sb.rpc('narratore_rename_combat_participant', { p_encounter_id: encounterId, p_character_id: characterId, p_display_name: displayName }),
...
/* Combattimenti da riprendere (FC2, macrofase completamento funzionale,
   card "Combattimento da riprendere" in Home): un incontro in 'staging' o
   'active' in una campagna dell'utente, sia come Narratore (proprietario
   della campagna) sia come giocatore (personaggio membro della campagna).
   Sola lettura diretta — stessa RLS "membri campagna" già usata da
   fetchCombatEncounterStatuses qui sopra, nessuna RPC nuova necessaria. */
async function listMyResumableEncounters(campaignIds) {
  if (!campaignIds || !campaignIds.length) return [];
  const { data, error } = await withTimeout(
    sb.from('combat_encounters')
      .select('id, campaign_id, label, status, created_at')
      .in('campaign_id', campaignIds)
      .in('status', ['staging', 'active'])
      .order('created_at', { ascending: false }),
    'Combattimenti da riprendere'
  );
  if (error) throw error;
  return data || [];
}

/* Elenco di TUTTI gli incontri di una campagna, qualunque stato (compresi
   quelli già 'ended'): a differenza di listMyResumableEncounters qui sopra
   (solo da riprendere), serve al Diario della campagna (FC3) per popolare
   il collegamento opzionale "incontro" — un giocatore deve poter linkare
   una voce di diario anche a una battaglia già conclusa, non solo a una in
   corso. Stessa RLS "membri campagna", nessuna nuova policy necessaria.
   Riusata anche per l'elenco di gestione nel tab Combattimento (FC4:
   rinomina/note/archivio/eliminazione/duplicazione) — stesse righe già
   qui, select estesa con round_number/notes/ended_at invece di una
   seconda query identica. */
async function listCampaignEncountersForLinking(campaignId) {
  if (!campaignId) return [];
  const { data, error } = await withTimeout(
    sb.from('combat_encounters')
      .select('id, label, status, round_number, notes, created_at, ended_at')
...
async function fetchCampaignAssets(campaignId) {
  const { data, error } = await withTimeout(
    sb.from('campaign_assets').select('id, label, storage_path, created_at, updated_at')
...
async function uploadCampaignAsset(campaignId, file, label) {
  ...
  if (error) {
    // Nessun file senza record: l'oggetto appena caricato viene rimosso.
    try { await withTimeout(sb.storage.from('campaign-assets').remove([path]), 'Rimozione immagine'); } catch (e) { /* resta rilevabile da list_storage_orphans */ }
    throw error;
  }
  return data;
}

/* Sostituisce UNA immagine della libreria: stesso record, stesso oggetto
   (storage_path invariato, upsert). Le altre immagini restano distinte.
   Se l'upload fallisce, immagine e metadati precedenti restano intatti;
   solo dopo un upload riuscito si aggiorna updated_at (cache-busting). */
async function replaceCampaignAssetImage(asset, file) {
  if (!asset || !asset.id || !asset.storage_path) throw new Error('Immagine non trovata');
  if (file.size > CAMPAIGN_ASSET_MAX_BYTES) {
    throw new Error(`Immagine troppo grande (${(file.size / (1024 * 1024)).toFixed(1)} MB): il limite è 15 MB`);
  }
  if (!/^image\/(png|jpeg|webp)$/.test(file.type || '')) throw new Error('Formato non supportato: usa JPG, PNG o WebP');
  const { error: upErr } = await withTimeout(
    sb.storage.from('campaign-assets').upload(asset.storage_path, file, { upsert: true, contentType: file.type }),
    'Sostituzione immagine'
  );
  if (upErr) throw upErr;
  const { data, error } = await withTimeout(sb.rpc('narratore_touch_campaign_asset', { p_asset_id: asset.id }), 'Aggiornamento immagine');
  if (error) throw error;
  return data;
}

/* Prima il record, poi il file: se la rimozione del file fallisce resta un
   oggetto non referenziato (in coda di pulizia lato server), mai un record
   che punta a un file inesistente. */
async function removeCampaignAsset(assetId, storagePath) {
  ...
  if (error) throw error;
  try { await withTimeout(sb.storage.from('campaign-assets').remove([storagePath]), 'Rimozione immagine'); }
  catch (e) { /* in coda di pulizia lato server (trigger su campaign_assets) */ }
}
...
async function applyDannoFisso(encounterId, attackerCharacterId, targetCharacterId, source, statusAttackerRoll, statusAttackerDetail, statusDefenderRoll, statusDefenderDetail, extraEffects) {
  const { data, error } = await withTimeout(
    sb.rpc('apply_danno_fisso', {
      ...
      // Effetti di stato AGGIUNTIVI, entrambi i tiri già pronti (vedi
      // resolveDannoFisso) — array di {nome, tratto, bonusPct,
      // attackerRollTotal?, attackerRollDetail?, defenderRollTotal?,
      // defenderRollDetail?}.
      p_extra_effects: extraEffects || []
    }),
    'Danno fisso'
  );
  if (error) throw error;
...
async function submitCombatAttackRolls(attackId, attackRollTotal, attackRollDetail, damageRollTotal, damageRollDetail, statusRollTotal, statusRollDetail, perforareTaglioRollTotal, perforareTaglioRollDetail, weaponStatusRollTotal, weaponStatusRollDetail, damageRollTotal2, damageRollDetail2, counterRollTotal, counterRollDetail, extraStatusRolls) {
  const { data, error } = await withTimeout(
    sb.rpc('submit_attack_rolls', {
      ...
      // Tiro d'ingresso di ciascun effetto AGGIUNTIVO (combatComputeAttackRoll
      // extraStatusRolls, stesso ordine di atk.extra_effects): un array di
      // {total, detail} (o null dove l'effetto non è del catalogo, nessun
      // dado tirato) — vedi submit_attack_rolls/combat_resolve_extra_status_entry.
      p_extra_status_rolls: extraStatusRolls || []
    }),
    'Tiro di attacco'
  );
  if (error) throw error;
...
async function submitCombatDefenseRoll(attackId, defenseType, rollTotal, rollDetail, auto1, auto1Detail, auto2, auto2Detail, statusRollTotal, statusRollDetail, weaponStatusRollTotal, weaponStatusRollDetail, extraStatusRolls) {
  const { data, error } = await withTimeout(
    sb.rpc('submit_attack_defense_roll', {
      ...
      // stesso ordine di atk.extra_effects — vedi combatRollDefense.
      p_extra_defender_status_rolls: extraStatusRolls || []
    }),
    'Schivata/blocco'
  );
  if (error) throw error;
...
/* RIFERIMENTO, NON PIÙ INVIATO AL SERVER (audit Play Store, punto 3): il
   server ricalcola le stesse perdite con la stessa logica
   (durability_perdite_attacco, 20261001020000_durability_server_authoritative.sql)
   e ignora qualunque valore del client. Questa funzione resta come
   specifica eseguibile della regola, coperta dai test unitari
   (equipment-usable-callers-integration.spec.js).
...
/* Il danno HP applicato è sempre final_damage già calcolato dal server nello
   step precedente, e la perdita di Durabilità è calcolata dal server
   (audit Play Store, punto 3: 20261001020000_durability_server_authoritative.sql):
   il client invia SOLO l'id dell'attacco, nessun importo. Le perdite
   applicate tornano in data.durability_losses_applied. */
async function applyCombatAttackDamage(attackId) {
  const { data, error } = await withTimeout(
    sb.rpc('apply_combat_attack_damage', { p_attack_id: attackId }),
    'Applica danno'
  );
  if (error) throw error;
  return data;
