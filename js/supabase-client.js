...
const AUTH_REDIRECT_URL = isCapacitorNative() ? 'rolemakers://auth-callback' : undefined;
...
function completeSessionFromDeepLink(url) {
  try {
    // Il browser di sistema aperto per il login Google (vedi
    // signInWithProvider) va richiuso appena si torna nell'app: innocuo per
    // gli altri link (email di conferma/recupero), che non l'hanno mai
    // aperto — Browser.close() non fa nulla se non c'e' nulla da chiudere.
