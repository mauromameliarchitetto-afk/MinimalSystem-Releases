...
function characterCloudPayload(c) {
  ...
  // cloudSyncPending: come narratorEditGuest, puramente locale/effimero (solo
  // "un'altra scansione in background ha visto una versione più nuova non
  // ancora scaricata QUI") — mai un dato di gioco, mai utile ad altri
  // dispositivi. Senza questa esclusione applyFullCloudSnapshot lo avrebbe
  // ereditato dal blob del device che ha pushato per ultimo (vedi il
  // commento lì su cloudVersion/cloudDirty, stessa classe di bug).
  delete payload.cloudSyncPending;
  // Ritratto (audit Play Store, punto 1): mai nel JSON cloud. Il file vive
  // nel bucket privato character-portraits (js/character-portraits.js), in
  // PostgreSQL solo portrait_path/portrait_updated_at: così non viaggia più
  // a ogni salvataggio né viene copiato in ogni riga di character_versions.
  // portraitPos (posizione del ritaglio, pochi byte) resta nel JSON.
  delete payload.portrait;
  delete payload.portraitDirty;
  delete payload.portraitPath;
  delete payload.portraitUpdatedAt;
  delete payload.portraitCacheKey;
  return payload;
}

/* Dopo ogni push riuscito: porta nel cloud l'eventuale modifica locale del
   ritratto. Un errore qui non annulla il salvataggio della scheda: la
   modifica resta "da caricare" (portraitDirty) e riprova al push successivo. */
async function pushCharacterPortraitAfterSave(c) {
  if (typeof syncCharacterPortraitToCloud !== 'function') return;
  try { await syncCharacterPortraitToCloud(c); }
  catch (e) { if (typeof toast === 'function') toast('Ritratto non ancora caricato nel cloud: verrà riprovato al prossimo salvataggio.'); }
}
...
async function pushCharacterToCloud(c) {
  ...
  if (c.cloudCharacterId) {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116' && expectedVersion !== null) {
        const { data: fresh, error: freshErr } = await withTimeout(
          sb.from('characters').select('level, campaign_id, data, current_version, portrait_path, portrait_updated_at').eq('id', c.cloudCharacterId).single(),
          'Verifica conflitto'
        );
    ...
    c.cloudDirty = false;
    await pushCharacterPortraitAfterSave(c);
...
async function resolveVersionConflict(c, freshRow, pushFn) {
  ...
  if (choice === 'cloud') {
    applyFullCloudSnapshot(c, freshRow); // azzera anche cloudSyncPending
  } else {
...
async function deleteCharacterCloud(cloudCharacterId) {
  ...
  if (error) throw error;
  // Il ritratto non deve restare orfano: rimosso subito (proprio prefisso);
  // se non riesce, il server l'ha già messo in coda di pulizia.
  if (typeof removeCharacterPortraitObject === 'function') await removeCharacterPortraitObject(cloudCharacterId);
}
...
async function listMyCloudCharacters() {
  ...
  const { data, error } = await withTimeout(
    sb.from('characters').select('id, name, level, campaign_id, data, current_version, updated_at, portrait_path, portrait_updated_at').eq('owner_user_id', session.user.id).eq('is_npc', false),
...
async function syncMyCharactersFromCloud() {
  ...
  cloudChars.forEach(row => {
    ...
    characters.push(c);
    importedAny = true;
    newlyImported.push([c, row]);
  });
  // Ritratti dei personaggi appena importati: scaricati dal bucket privato
  // (non viaggiano più nel JSON).
  if (typeof hydrateCharacterPortraitFromCloud === 'function') {
    for (const [c, row] of newlyImported) {
      try { await hydrateCharacterPortraitFromCloud(c, row); } catch (e) { /* resta senza immagine finché non riesce */ }
    }
  }
...
/* Uscita volontaria da una storia (leave_campaign, RPC dedicata: prima
   l'unica strada era chiedere al Narratore di rimuoverti). Non tocca i
   campi cloudCampaignId/cloudCampaignName/ecc. qui: il chiamante deve
   comunque fare syncCharacterFromCloud(c) subito dopo, che li azzera già
   da sé confrontando il campaign_id server (tornato null) con quello
   locale — stessa identica reazione già scritta per il caso "il Narratore
   mi ha rimosso", un solo percorso invece di due che potrebbero divergere. */
async function leaveCampaignCloud(c) {
  if (!c.cloudCharacterId) throw new Error('Personaggio non salvato nel cloud');
  const { error } = await withTimeout(
    sb.rpc('leave_campaign', { p_character_id: c.cloudCharacterId }),
    'Uscita dalla storia'
  );
  if (error) throw error;
}
...
async function syncCharacterFromCloud(c, allowConflictPrompt = true) {
  ...
  if (!localVersionKnown || serverVersion > Number(c.cloudVersion)) {
    if (!allowConflictPrompt) {
      ...
      applyFullCloudSnapshot(c, data); // azzera anche cloudSyncPending
  ...
  if (typeof hydrateCharacterPortraitFromCloud === 'function') {
    try {
      if (await hydrateCharacterPortraitFromCloud(c, data)) {
        changed = true;
        if (typeof getActive === 'function' && getActive() === c && typeof renderPortrait === 'function') { renderPortrait(c); if (typeof renderHeader === 'function') renderHeader(c); }
      }
    } catch (e) { /* ritratto non essenziale alla sincronizzazione */ }
  }
...
function applyFullCloudSnapshot(c, cloudRow) {
  ...
  Object.assign(c, cloudData, preserved, {
    ...
    cloudDirty: false,
    // Come cloudVersion/cloudDirty sopra: mai ereditato da cloudData (il
    // valore che aveva il DISPOSITIVO che ha pushato per ultimo quel blob).
    // Uno snapshot completo appena applicato è per definizione allineato,
    // nessun conflitto rimandato da segnalare più.
...
function exportCharacterJson(c) {
  // L'export personale include il ritratto (i dati sono dell'utente): è solo
  // il JSON sincronizzato nel cloud a non contenerlo più.
  const exported = Object.assign(characterCloudPayload(c), c.portrait ? { portrait: c.portrait } : {});
...
/* "Condivisioni e permessi" (S6, piano UX "confronto D&D Beyond"): il
   giocatore era già informato DI ESSERE in una storia (nome storia,
   Narratore, stato sessione, qui sopra), ma non di COSA questo comporta
   per la propria scheda — un vuoto di trasparenza reale segnalato
   dall'audit S6, non una funzione nuova da inventare: descrive solo
   permessi che esistono già nel codice (isLevelLocked in app.js — il
   livello resta bloccato al giocatore mentre è in una storia — ed
   editing Narratore libero via openCharacterForNarratorEdit/
   narratoreUpdateCharacterDataCloud). Il bottone "Esci dalla storia"
   (leaveCampaignCloud, RPC leave_campaign) è stato aggiunto SUBITO dopo
   la prima versione di questo box: la bozza iniziale si limitava a
   spiegare che uscire richiedeva contattare il Narratore, un paragrafo
   più lungo del necessario per dire "non puoi farlo da solo" — la vera
   correzione era costruire la funzione mancante, non descriverne meglio
   l'assenza. */
function cloudPermissionsDetailsHtml(c) {
  return `<details class="tec-effetto-details">
      <summary class="tec-effetto-summary">🔒 Condivisioni e permessi</summary>
      <p class="helper-text" style="margin:6px 0;">Il Narratore di questa storia può assegnare il tuo livello e correggere la tua scheda; nessun altro giocatore può vederla o modificarla.</p>
      <button type="button" class="btn btn-ghost btn-sm" id="cs-leave-campaign" style="color:var(--fisico-forte);">🚪 Esci dalla storia</button>
    </details>`;
}
function cloudStoryBoxHtml(c) {
  ...
  return `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <button class="btn btn-primary btn-sm" id="cs-request-join-select">Invia richiesta di partecipazione</button>
      <button class="btn btn-ghost btn-sm" id="cs-report-campaign">🚩 Segnala questa storia</button>
    </div>
    ${shareCharacterTriggerHtml()}
  `;
}
...
function wireCloudCharacterEvents() {
  $('#cloud-story-box').addEventListener('click', async e => {
    ...
    if (e.target.id === 'cs-report-campaign') {
      const select = $('#cs-campaign-select');
      const campaignId = select ? select.value : '';
      if (!campaignId) { toast('Scegli prima una storia dall\'elenco'); return; }
      const campaignName = (select.options[select.selectedIndex] && select.options[select.selectedIndex].dataset.name) || select.options[select.selectedIndex].textContent;
      openReportModal('campaign', campaignId, `Segnali la storia «${campaignName}»`);
      return;
    }
    ...
    if (e.target.id === 'cs-leave-campaign') {
      if (!confirm(`Uscire da «${c.cloudJoinCampaignName || c.cloudCampaignName || 'questa storia'}»? La tua scheda resta tua, solo scollegata dalla storia.`)) return;
      try {
        await leaveCampaignCloud(c);
        // syncCharacterFromCloud rileva da sé campaign_id tornato null e
        // azzera cloudCampaignId/cloudCampaignName/ecc. — lo stesso identico
        // ramo già scritto per "il Narratore mi ha rimosso" (vedi sopra),
        // messaggio di conferma incluso: nessuna logica duplicata qui.
