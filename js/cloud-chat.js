/* Chat di campagna (FC6, FASE FINALE — completamento funzionale
   ...
   Correzioni mirate (pacchetto v1, punto 11): identità reale nei
   messaggi (nickname + ritratto personaggio, Narratore = emblema+nome
   campagna, mai "Tu") tramite hydrateChatIdentity()/list_campaign_member_
   characters, markup .chatp-* condiviso con Tomo/anteprima, azioni
   (rispondi/modifica/elimina/segnala/blocca) nascoste dietro un menu "…"
   invece che sempre visibili. */
...
async function sendChatMessage(campaignId, channel, body, replyToId) {
  ...
  // Chat = UGC: gate server-side sulle regole correnti (RMU01), vedi
  // withUgcTermsGate in cloud-account.js.
...
/* Coda di moderazione UNICA: le segnalazioni di chat non vanno piu' in una
   tabella propria (rm_chat_reports, eliminata) ma in content_reports
   insieme a quelle su storie pubbliche, log di sessione e tratti — e'
   quello che gli store intendono per "strumenti di moderazione", e una
   coda divisa in due non lo e'.

   La categoria resta 'other' con il motivo libero in details: la chat
   raccoglie un testo, non una categoria, e inventarne una qui sarebbe
   peggio che dichiararla generica. Il campaignId non serve piu': il
   bersaglio e' il messaggio, e la campagna si risale da li'. */
async function reportChatMessage(messageId, _campaignId, reason) {
  const { error } = await withTimeout(
    sb.rpc('submit_report', {
...
/* Blocco UNICO (decisione dell'utente): bloccare una persona vale ovunque,
   non solo nella chat di UNA campagna. La vecchia tabella rm_chat_blocks,
   con la sua chiave (campagna, bloccante, bloccato), non esiste piu': i
   blocchi vivono in user_blocks e non hanno campagna
   (20260930050000_ugc_moderation.sql).

   Il parametro campaignId resta nella firma perche' i chiamanti lo hanno
   sottomano e toglierlo obbligherebbe a toccarli tutti senza guadagnarci
   nulla — ma qui non viene usato, ed e' bene che si veda dalla firma
   stessa invece che scoprirlo leggendo il corpo.

   Scrittura via RPC e non via insert diretta: user_blocks non ha policy di
   insert/delete: block_user/unblock_user sono l'unica via, e validano
   l'auto-blocco. */
async function blockChatUser(_campaignId, blockedUserId) {
  const { error } = await withTimeout(sb.rpc('block_user', { p_target_id: blockedUserId }), 'Blocco utente');
  if (error) throw error;
}
async function unblockChatUser(_campaignId, blockedUserId) {
  const { error } = await withTimeout(sb.rpc('unblock_user', { p_target_id: blockedUserId }), 'Sblocco utente');
  if (error) throw error;
}
async function listChatBlockedUserIds(_campaignId) {
  const session = await currentCloudSession();
  if (!session) return [];
  const { data, error } = await withTimeout(
    sb.from('user_blocks').select('blocked_id').eq('blocker_id', session.user.id),
    'Blocchi'
  );
  if (error) throw error;
...
// Correzioni mirate (pacchetto v1): identità reale nei messaggi — ruolo
// per stabilire chi mostra l'identità Narratore (emblema+"Narratore"),
// personaggio attivo per volto/nome di chi invece è un giocatore. Mai
// più la parola "Tu" per il proprio messaggio (il nickname reale/volto
// del proprio personaggio compaiono come per chiunque altro).
let chatMemberRoles = {};      // user_id -> role
let chatMemberCharacters = {}; // user_id -> { name, portrait }
let chatCampaignName = '';
let chatCampaignIcon = null;
...
// Ruoli (chi è Narratore) + personaggio attivo (volto/nome) di ciascun
// membro della campagna, ed emblema/nome della campagna per l'identità
// Narratore — stessa RPC/pattern già in uso per la mappa (get_combat_board,
// list_map_session_participants): characters ha RLS "solo proprietario o
// Narratore", quindi il nome/ritratto altrui passa da una RPC dedicata
// (list_campaign_member_characters), mai una query diretta.
async function hydrateChatIdentity(campaignId) {
  const [membersRes, charsRes, campaignRes] = await Promise.all([
    withTimeout(sb.from('campaign_members').select('user_id, role').eq('campaign_id', campaignId), 'Membri campagna'),
    withTimeout(sb.rpc('list_campaign_member_characters', { p_campaign_id: campaignId }), 'Identità personaggi'),
    withTimeout(sb.from('campaigns').select('name, icon').eq('id', campaignId).maybeSingle(), 'Campagna')
  ]);
  if (membersRes.error) throw membersRes.error;
  if (charsRes.error) throw charsRes.error;
  if (campaignRes.error) throw campaignRes.error;
  chatMemberRoles = {};
  (membersRes.data || []).forEach(r => { chatMemberRoles[r.user_id] = r.role; });
  chatMemberCharacters = {};
  (charsRes.data || []).forEach(r => { chatMemberCharacters[r.user_id] = { name: r.character_name, portrait: r.portrait_url }; });
  chatCampaignName = (campaignRes.data && campaignRes.data.name) || 'Storia';
  chatCampaignIcon = (campaignRes.data && campaignRes.data.icon) || null;
}
function chatAuthorIsNarrator(userId) {
  const role = userId ? chatMemberRoles[userId] : null;
  return role === 'owner' || role === 'narratore' || role === 'co_narratore';
}

function chatMessageRowHtml(m, ctx) {
  if (m.author_user_id && ctx.blockedUserIds.includes(m.author_user_id)) {
    return `<div class="chatp-row" data-chatmsgrow="${m.id}"><p class="helper-text" style="margin:0;">Messaggio nascosto (hai bloccato questo utente). <button type="button" class="btn btn-ghost btn-sm" data-chatunblock="${m.author_user_id}">Sblocca</button></p></div>`;
  }
  const isMine = !!m.author_user_id && m.author_user_id === ctx.myId;
  const isRemoved = !!m.removed_reason;
  const isNarratorMsg = chatAuthorIsNarrator(m.author_user_id);
  ...
  const bodyHtml = isRemoved
    ? `<p class="chatp-body helper-text" style="font-style:italic;">${escapeHtml(removedLabel)}</p>`
  ...
  const replyHtml = m.reply_to_id
    ...
    : '';
  // Azioni dietro un menu "…" (correzioni mirate 5.3.5): mai visibili di
  // default, un tocco/click le mostra — stesso principio richiesto per
  // long press/hover desktop, qui reso con un semplice toggle (robusto
  // su ogni dispositivo, senza dover distinguere il tipo di puntatore).
  const actions = [];
  ...
  const actionsMenu = actions.length
    ? `<div class="chatp-actions-wrap"><button type="button" class="chatp-actions-toggle" data-chatactionstoggle aria-label="Altre azioni" aria-haspopup="true">⋯</button><div class="chatp-actions-menu hidden">${actions.join('')}</div></div>`
    : '';

  // Narratore: emblema reale della campagna + "Narratore" — SEMPRE,
  // anche per i propri messaggi se il Narratore è il viewer (mai
  // confuso con l'accento "messaggio proprio", che resta riservato ai
  // giocatori: due significati diversi, mai lo stesso trattamento).
  if (isNarratorMsg) {
    const emblemHtml = chatCampaignIcon
      ? `<img src="${escapeHtml(chatCampaignIcon)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
      : `<span>${escapeHtml((chatCampaignName || '?').trim().charAt(0).toUpperCase())}</span>`;
    return `<div class="chatp-row chatp-row-narrator" data-chatmsgrow="${m.id}">
      <div class="chatp-avatar chatp-avatar-narrator">${emblemHtml}</div>
      <div class="chatp-bubble chatp-bubble-narrator">
        <div class="chatp-head"><span class="chatp-badge-narrator">Narratore</span><span class="chatp-time">${escapeHtml(time)}${m.edited_at ? ' · modificato' : ''}</span></div>
        ${replyHtml}
        ${bodyHtml}
        ${actionsMenu}
      </div>
  ...
  const nickname = m.author_user_id ? (ctx.authorNames[m.author_user_id] || 'Un membro della campagna') : 'Utente eliminato';
  const identity = m.author_user_id ? ctx.memberCharacters[m.author_user_id] : null;
  const portrait = identity && identity.portrait;
  const initial = (nickname || '?').trim().charAt(0).toUpperCase();
  const color = isMine ? 'var(--magico-forte)' : (typeof chatDeterministicColor === 'function' ? chatDeterministicColor(m.author_user_id || nickname) : 'var(--fisico-forte)');
  const avatarHtml = `<div class="chatp-avatar" style="border-color:${color};">${portrait ? `<img src="${escapeHtml(portrait)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">` : `<span>${escapeHtml(initial)}</span>`}</div>`;
  const mineClass = isMine ? ' chatp-row-mine' : '';
  const bubbleClass = isMine ? 'chatp-bubble chatp-bubble-mine' : 'chatp-bubble';
  return `<div class="chatp-row${mineClass}" data-chatmsgrow="${m.id}">
    ${isMine ? '' : avatarHtml}
    <div class="${bubbleClass}" style="${isMine ? '' : 'border-left-color:' + color + ';'}">
      <div class="chatp-head"><span class="chatp-nickname" style="color:${color};">${escapeHtml(nickname)}</span><span class="chatp-time">${escapeHtml(time)}${m.edited_at ? ' · modificato' : ''}</span></div>
      ${replyHtml}
      ${bodyHtml}
      ${actionsMenu}
    </div>
...
function renderChatMessageListDom() {
  ...
  const ctx = { myId: chatMyUserId, authorNames: chatAuthorNamesCache, memberCharacters: chatMemberCharacters, isMaster: chatIsMaster, blockedUserIds: chatBlockedUserIds, byId };
...
async function openCampaignChatModal(campaignId) {
  ...
  // Identità reale (ritratto personaggio, emblema/nome Narratore) MAI a
  // bloccare il primo rendering dei messaggi: parte qui in background,
  // senza essere atteso. Il nickname (via authorNames, non toccato da
  // questa chiamata) è già disponibile subito — il fallback finché
  // l'identità non arriva è quindi "nickname reale, iniziale al posto del
  // ritratto, mai marcato Narratore per errore", mai un dato inventato.
  // Se arriva mentre il modale è ancora aperto sulla stessa campagna,
  // un secondo rendering aggiorna le righe già mostrate: stesso array
  // chatMessages, nello stesso ordine — nessuna riga duplicata, riordinata
  // o persa, solo l'HTML di ciascuna riga ricalcolato con l'identità piena.
  hydrateChatIdentity(campaignId).then(function () {
    if (campaignId === chatModalCampaignId) renderChatMessageListDom();
  }).catch(function () {});
...
function wireCampaignChatModal() {
  ...
  $('#chat-message-list').addEventListener('click', async e => {
    const toggleBtn = e.target.closest('[data-chatactionstoggle]');
    if (toggleBtn) {
      const menu = toggleBtn.nextElementSibling;
      const wasOpen = menu && !menu.classList.contains('hidden');
      $$('.chatp-actions-menu').forEach(m => m.classList.add('hidden'));
      if (menu && !wasOpen) menu.classList.remove('hidden');
      return;
    }
    // Qualunque azione scelta dentro il menu "…" lo richiude subito dopo
    // (Rispondi/Modifica/Elimina/Segnala/Blocca gestiti più sotto).
    const chosenInMenu = e.target.closest('.chatp-actions-menu');
    if (chosenInMenu) chosenInMenu.classList.add('hidden');
  ...
  // Click fuori da un menu "…" aperto: lo richiude (comportamento atteso
  // per un menu a comparsa, non catturato dal listener sopra perché quello
  // ascolta solo i click dentro #chat-message-list).
  document.addEventListener('click', e => {
    if (e.target.closest('.chatp-actions-wrap')) return;
    $$('.chatp-actions-menu').forEach(m => m.classList.add('hidden'));
  });
}
